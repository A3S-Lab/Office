...
- Ribbon and selection popovers escape a clipped `[data-a3s-office]` ancestor
  onto `document.body` so task-workspace overflow does not cut them off.
...
- Grow the permanent R0 no-clobber corpus (`bun run test:corpus:r0`) with
  representative fixtures for complex (fldChar) first-page header PAGE
  live-field identity, covering Word-authored title-page chrome without
  art-border breadth.
...
- Grow the permanent R0 no-clobber corpus (`bun run test:corpus:r0`) with
  representative fixtures for even-page footer NUMPAGES live-field identity,
  covering Traditional Office facing-page page-of-total chrome without art-border
  breadth.
...
- Grow the permanent R0 no-clobber corpus (`bun run test:corpus:r0`) with
  representative fixtures for first-page footer NUMPAGES live-field identity,
  covering Traditional Office title-page page-of-total chrome without art-border
  breadth.
...
- Grow the permanent R0 no-clobber corpus (`bun run test:corpus:r0`) with
  representative fixtures for distinct first-page vs default header and footer
  text, covering Traditional Office title-page chrome without art-border
  breadth.
...
- Grow the permanent R0 no-clobber corpus (`bun run test:corpus:r0`) with
  representative fixtures for header SECTION and footer SECTIONPAGES live-field
  identity, covering multi-section report chrome without art-border breadth.
...
- Grow the permanent R0 no-clobber corpus (`bun run test:corpus:r0`) with
  representative fixtures for body SECTIONPAGES and NUMWORDS field
  kind/instruction identity, covering section page-count and live word-count
  workflows without art-border breadth.
...
- Grow the permanent R0 no-clobber corpus (`bun run test:corpus:r0`) with
  representative fixtures for endnote reference/body identity and inline text
  content-control alias/tag/text, covering academic notes and contract form
  fields without art-border breadth.
...
- Browser document PDF export now paints Writer `checkered` art paragraph
  borders as repeating 2×2 tile grids with alternating diagonal fills along
...
- Browser document PDF export now paints Writer `checkedBarBlack` art
  paragraph borders as repeating checkered bar segments along the measured
  edge. Remaining decorative art borders and full PDF/UA certification stay
  out of scope.
...
- Browser document PDF export now paints Writer `certificateBanner` art
  paragraph borders as repeating notched ribbon motifs along the measured edge.
  Remaining decorative art borders and full PDF/UA certification stay out of
  scope.
...
- Browser document PDF export now paints Writer `cakeSlice` art paragraph
  borders as repeating cake-slice wedges along the measured edge. Remaining
  decorative art borders and full PDF/UA certification stay out of scope.
...
- Browser document PDF export now paints Writer `balloons3Colors` art paragraph
  borders as repeating balloon silhouettes along the measured edge. Remaining
  decorative art borders and full PDF/UA certification stay out of scope.
...
- Browser document PDF export now paints Writer `babyRattle` art paragraph
  borders as repeating rattle silhouettes along the measured edge. Remaining
  decorative art borders and full PDF/UA certification stay out of scope.
...
- Browser document PDF export now paints Writer `babyPacifier` art paragraph
  borders as repeating pacifier silhouettes along the measured edge. Remaining
  decorative art borders and full PDF/UA certification stay out of scope.
...
- Browser document PDF export now paints Writer `apples` art paragraph borders
  as repeated apple silhouettes along the measured edge. Remaining decorative
  art borders and full PDF/UA certification stay out of scope.
...
- Whole-paragraph mark, paragraph-break, and text-move revision bodies now
  also admit bounded `w:endnoteReference` glyphs that carry exactly one
  Word-ns numeric `w:id`, mirroring `w:footnoteReference` from `0.187.0`.
  Attributed `footnoteRef` / `endnoteRef` and malformed markers stay
  fail-closed.
...
- Browser document PDF export nests page-matched vector-run Span MCIDs under
  outline-derived H1–H6/P StructElem `/K` (last outline role on that page;
  unmatched Spans stay Document kids). Remaining decorative art borders and
  full PDF/UA certification remain out of scope.
...
- Browser document PDF export paints Writer `basicThinLines` art
  paragraph borders as three parallel hairlines along the measured edge.
  Remaining decorative art borders and full PDF/UA certification remain out of
  scope.
...
- Browser document PDF export paints Writer `triangles` / `triangle1` /
  `triangle2` art paragraph borders as explicit closed isosceles triangles.
  Remaining decorative art borders and full PDF/UA certification remain out of
  scope.
...
- Browser document PDF export paints Writer `wave` and `doubleWave` paragraph
  borders as explicit sine polylines (not silent straight-line approximations).
  Art and 3D borders and full PDF/UA certification remain out of scope.

## 0.155.0 - 2026-09-11

### Writer

- Browser document PDF export links vector-run `/Span` ActualText through
  page-local `/MCID`, page `/StructParents`, and StructTreeRoot `/ParentTree`
  Span kids. Outline role stubs keep empty `/K`. Full PDF/UA certification
...
- Attribute-free empty `w:annotationRef` CT_Empty glyphs are now admitted
  inside whole-paragraph mark, paragraph-break, and text-move revision
  bodies. Attributed `annotationRef` and separators stay fail-closed.

## 0.145.0 - 2026-09-11

### Writer

- Attribute-free empty `w:endnoteRef` CT_Empty glyphs are now admitted
  inside whole-paragraph mark, paragraph-break, and text-move revision
  bodies. Attributed `endnoteRef`, `endnoteReference` with id,
...
- Eligible whole-paragraph mark revision bodies now admit untracked supported
  inline DrawingML picture siblings (`wp:inline` + resolved image `r:embed`)
  beside matching `w:ins` / `w:del` wrappers, mirroring the 0.130 text-sibling
  path. Empty or malformed drawings, floating anchors, unresolved embeds, and
  picture-only mark bodies stay fail-closed. Paragraph-break bodies already
  admitted inline pictures through `0.133.0`.
...
- Eligible paragraph-break merge/split, whole-paragraph mark, and text-move
  revision bodies now admit relationship-free empty `w:dayShort`,
  `w:monthShort`, and `w:yearShort` glyphs alongside the existing soft-break /
  tab / carriage-return / last-rendered page break / page-number / long
  date-field / hyphen / hyperlink / bookmark set. Attributed or non-empty short
  date-field glyphs stay fail-closed.
...
- Eligible paragraph-break merge/split, whole-paragraph mark, and text-move
  revision bodies now admit relationship-free empty `w:tab`, `w:noBreakHyphen`,
  and `w:softHyphen` run glyphs alongside the existing soft-break / hyperlink /
  bookmark set. Attributed or non-empty glyphs stay fail-closed.
...
- Bounded multi-level relationship-free `w:numberingChange` originals
  ...
  segments, and malformed originals stay fail-closed.
...
- Relationship-free `w:sectPrChange` priors that include bounded relationship-free
  empty `w:footnotePr` or children only (`w:pos` / `w:numFmt` / `w:numStart` /
  `w:numRestart`) are now reviewable as `section-formatting` with accept/reject,
  live track-changes, and native export (including current-section `w:footnotePr`
  on DOCX write). Unknown children/attributes, duplicates, grandchildren, and
  relationship bindings stay fail-closed; opaque section fixtures moved off
  `footnotePr` to `endnotePr`.
...
- Relationship-free `w:sectPrChange` priors that include relationship-free
  `w:textDirection` with required known `w:val` (`lrTb` / `tbRl` / `btLr` /
  `lrTbV` / `tbRlV` / `tbLrV`) are now reviewable as `section-formatting` with
  accept/reject, live track-changes, and native export (including current-section
  `w:textDirection` on DOCX write). Unknown attributes and values stay
  fail-closed; opaque section fixtures moved off `textDirection` to `bidi`.
...
- Relationship-free `w:sectPrChange` priors that include relationship-free empty
  or onOff `w:noEndnote` (`w:val` on/off/true/false/0/1) are now reviewable as
  `section-formatting` with accept/reject, live track-changes, and native
  export (including current-section `w:noEndnote` on DOCX write). Unknown
  attributes stay fail-closed; opaque section fixtures moved off `noEndnote` to
  `textDirection`.
...
- Relationship-free `w:tcPrChange` priors that include bounded `w:gridSpan`
  (positive integer `w:val`) are now reviewable as `cell-formatting` with
  accept/reject, live track-changes, and native export. Missing, non-positive,
  non-integer, or malformed values stay fail-closed; opaque cell fixtures moved
  off `gridSpan` to `cellIns`.
...
- Relationship-free `w:tcPrChange` priors that include bounded `w:hMerge`
  (`restart` / `continue`, omitted val = continue) are now reviewable as
  `cell-formatting` with accept/reject, live track-changes, and native export.
  Unknown merge values stay fail-closed; opaque cell fixtures moved off `hMerge`
  to `vMerge`.
...
- Relationship-free `w:sectPrChange` priors that include bounded relationship-free
  `w:docGrid` (`w:type` and `w:linePitch` only) are now reviewable as
  `section-formatting` with accept/reject, live track-changes, and native export.
  Unknown attributes such as `w:charSpace` stay fail-closed; opaque section
...
- Relationship-free `w:trPrChange` priors that include relationship-free
  ...
  Broader row property sets remain opaque or fail-closed; opaque fixtures stayed on
  `divId` (cell `cnfStyle` became reviewable in 0.102.0; cell opaque fixtures now `hMerge`).
...
- Relationship-free `w:tblPrChange` priors that include relationship-free
  `w:tblStyleRowBandSize` are now reviewable as `table-formatting`
  (`rowBandSize`) with accept/reject, live track-changes, and native export
  (including current-table `w:tblStyleRowBandSize` on DOCX write). Broader
...
- Relationship-free `w:tblPrChange` priors that include relationship-free
  ...
  table property sets remain opaque or fail-closed; opaque fixtures stayed on
  `tblStyleRowBandSize` (moved to `tblPrException` in 0.99.0).
...
- Relationship-free `w:tblPrChange` priors that include relationship-free
  `w:tblDescription` are now reviewable as `table-formatting` (`description`)
  with accept/reject, live track-changes, and native export (including
  current-table `w:tblDescription` on DOCX write). Broader table property sets
  remain opaque or fail-closed; opaque fixtures stayed on `tblStyleColBandSize` (moved to `tblStyleRowBandSize` in 0.98.0).
...
- Relationship-free `w:tblPrChange` priors that include relationship-free `w:tblCaption` are now
  ...
  fixtures stayed on `tblDescription` (moved to `tblStyleColBandSize` in 0.97.0).
...
- Relationship-free `w:tblPrChange` priors that include dxa `w:tblCellSpacing` are now
  reviewable as `table-formatting` (`cellSpacing`) with accept/reject, live
  track-changes, and native export (including current-table `w:tblCellSpacing` on
  DOCX write). Broader table property sets remain opaque or fail-closed; opaque
  fixtures stay on `tblBorders`.

## 0.93.0 - 2026-09-09

### Writer

- Relationship-free `w:tblPrChange` priors that include relationship-free `w:tblStyle` are now
  reviewable as `table-formatting` (`styleId`) with accept/reject, live
  track-changes, and native export (including current-table `w:tblStyle` on
  DOCX write). Broader table property sets remain opaque or fail-closed; opaque
  fixtures stay on `tblCellSpacing`.
...
- Relationship-free `w:tblPrChange` priors that include `w:tblLook` are now
  reviewable as `table-formatting` (`look`) with accept/reject, live
  track-changes, and native export (including current-table `w:tblLook` on DOCX
  write). Broader table property sets remain opaque or fail-closed; opaque
  fixtures stay on `tblOverlap`.
...
- Relationship-free `w:trPrChange` priors that include `w:wAfter` are now
  reviewable as `row-formatting` with accept/reject, live track-changes, and
  native export (including current-row `w:wAfter` on DOCX write).
  Broader row property sets remain opaque or fail-closed; opaque fixtures now
  use `cnfStyle` instead of `wAfter`.

## 0.85.0 - 2026-09-08

### Writer

- Relationship-free `w:trPrChange` priors that include `w:wBefore` are now
  reviewable as `row-formatting` with accept/reject, live track-changes, and
  native export (including current-row `w:wBefore` on DOCX write).
  Broader row property sets remain opaque or fail-closed; opaque fixtures now
  use `wAfter` instead of `wBefore`.
...
- Relationship-free `w:trPrChange` priors that include `w:gridBefore` are now
  reviewable as `row-formatting` with accept/reject, live track-changes, and
  native export (including current-row `w:gridBefore` on DOCX write).
  Broader row property sets remain opaque or fail-closed; opaque fixtures now
  use `gridAfter` instead of `gridBefore`.

## 0.82.0 - 2026-09-08

### Writer

- Relationship-free `w:tcPrChange` priors that include `w:hideMark` are now
  reviewable as `cell-formatting` with accept/reject, live track-changes, and
  native export (including current-cell `w:hideMark` on DOCX write).
  Broader cell property sets remain opaque or fail-closed; opaque fixtures now
  use `cnfStyle` instead of `hideMark`.

## 0.81.0 - 2026-09-08

### Writer

- Relationship-free `w:tcPrChange` priors that include `w:tcFitText` are now
  reviewable as `cell-formatting` with accept/reject, live track-changes, and
  native export (including current-cell `w:tcFitText` on DOCX write).
  Broader cell property sets remain opaque or fail-closed; opaque fixtures now
  use `hideMark` instead of `tcFitText`.
...
Five-surface WPS UI/UX alignment expands the shared evidence matrix to **103 ACL**
and **78 visual** contracts (up from 61 ACL / 56 visual in 0.76.0). Each surface
ships ribbon advertisements plus ACL / visual / unit evidence where noted.

### Writer

- Paragraph move (`Alt+Shift+↑/↓`), indent families (`Ctrl+M` / `Ctrl+Shift+M`,
  `Shift+Alt+.` / `Shift+Alt+,`, `Alt+Shift+→/←`), and alignment
  (`Ctrl+L/E/R/J`, distributed `Ctrl+Shift+J` with DOCX `w:jc=distribute`).
- List and style chords: default bullets (`Ctrl+Shift+L`), headings
  (`Ctrl+Alt+1..3`), Normal/正文 (`Ctrl+Shift+N`), clear formatting
  (`Ctrl+Space`), and `Shift+F3` case cycling.
- Character formatting: underline styles, double strikethrough (`Ctrl+Shift+X`),
  font-size steppers, and mutually exclusive sub/superscript.
- Find/replace, page break, hyperlink, word-count, Insert / Page Layout chrome,
  and field lock / unlink / code toggles (`Ctrl+F11`, `Ctrl+Shift+F11`,
  `Ctrl+Shift+F9`, `Alt+F9`, `Shift+F9`) with `MERGEFORMAT` field settings.

### Spreadsheet

- Workbook View chrome: formula bar, show formulas (`Ctrl` + backtick), gridlines,
  and headers with `data-*` state for operators.
- Sheet navigation via `Ctrl+PageUp` / `Ctrl+PageDown` with grid-focus restore.

### Presentation

- Slide management: new (`Ctrl+M` / `⌘⇧N`), duplicate (`Ctrl+D`), delete on the
  active thumbnail, notes pane, group/ungroup advertisements, and `Ctrl+F1`
  ribbon collapse with visual coverage.
- Slideshow: `F5` / `Shift+F5`, `Home` / `End`, digit+Enter jump, and blank-screen
  overlays (`B` / `.`, `W` / `,`); sorter Enter returns to normal view.

### Markdown

- Shared `Ctrl+F1` ribbon collapse with the other editors.
- Source formatting and link entry: `Ctrl+B`, `Ctrl+I`, `Ctrl+K`.

### PDF

- Zoom (`Ctrl+0/1/2`), page navigation (`PageUp`/`PageDown`, `Ctrl+Home`/`Ctrl+End`,
  `Space`/`Shift+Space`), and `Ctrl+F` search focus.

### Shared operator hardening

- Unified arrow-key / Home / End traversal across ribbons, quick access, and the
  PDF command bar while comboboxes keep native editing behavior.
- Windows A3S Test CDP adapter: auto-launch Playwright Chromium when the CDP
  port is empty, viewport via window bounds, and CSS-selector waits that keep
  spaced attributes intact.
...
- Added bounded same-paragraph move inference to Writer Compare. Deterministic
  lexical ranges that move within one simple paragraph or heading now become a
  single paired `move` review item, carrying separators with the moved range so
  accept, reject, Undo, and DOCX reopen reconstruct the exact original or
  revised text. Ambiguous duplicates, rich runs, relationship-bound content,
  cross-paragraph ranges, and over-limit inputs fail closed as ordinary
  revisions or explicit diagnostics.
- Updated the Review ribbon, Changes pane, author copy, toast counts, and
  bilingual documentation to explain the bounded Compare behavior. Desktop and
  390 px browser flows verify one move card, both source/destination marks,
  responsive containment, focus, accessibility, and empty console/page-error
  diagnostics.
- Recorded the WPS COM/UIA reference probe used for parity decisions. On the
  installed WPS 12.0 COM surface, `CompareDocuments` exposed the observed
  reorder as one delete and one insert record, so A3S keeps paired move
  inference deterministic and local rather than claiming an unobserved native
  WPS move type.
