...
reviewable bounded w:rPrChange character-formatting revision author/kind identity, reviewable jc-only w:pPrChange paragraph-formatting revision author/kind identity, reviewable whole-paragraph mark insertion revision author/kind identity, reviewable paragraph-break split revision author/kind identity, reviewable paired text-move revision author/kind identity, reviewable companion move-range bookmark text-move revision author/kind identity, reviewable jc-only w:tblPrChange table-formatting revision author/kind identity, reviewable tblW-only w:tblPrChange preferred-width table-formatting revision author/kind identity, reviewable tblInd-only w:tblPrChange table-indent revision author/kind identity, reviewable tblCellMar-only w:tblPrChange table cell-margin revision author/kind identity, reviewable tblLayout-only w:tblPrChange table-layout revision author/kind identity, reviewable bidiVisual-only w:tblPrChange RTL visual revision author/kind identity, reviewable solid-shd-only w:tblPrChange table-fill revision author/kind identity, reviewable tblOverlap-only w:tblPrChange overlap revision author/kind identity, reviewable tblLook-only w:tblPrChange table-look revision author/kind identity, reviewable tblStyle-only w:tblPrChange table-style revision author/kind identity, reviewable tblCellSpacing-only w:tblPrChange table cell-spacing revision author/kind identity, reviewable tblBorders-only w:tblPrChange table-border revision author/kind identity, reviewable tblStyleColBandSize-only w:tblPrChange column-band revision author/kind identity, reviewable tblStyleRowBandSize-only w:tblPrChange row-band revision author/kind identity, reviewable tblCaption-only w:tblPrChange table-caption revision author/kind identity, reviewable tblDescription-only w:tblPrChange table-description revision author/kind identity, reviewable tblpPr-only w:tblPrChange table-float revision author/kind identity, reviewable orientation-only w:sectPrChange section-formatting revision author/kind identity, reviewable pgMar-only w:sectPrChange section-margin revision author/kind identity, reviewable titlePg-only w:sectPrChange title-page revision author/kind identity, reviewable equal-width cols-only w:sectPrChange column revision author/kind identity, reviewable rtlGutter-only w:sectPrChange RTL gutter revision author/kind identity, reviewable paperSrc-only w:sectPrChange paper-source revision author/kind identity, reviewable vAlign-only w:sectPrChange section vertical-align revision author/kind identity, reviewable docGrid-only w:sectPrChange document-grid revision author/kind identity, reviewable bidi-only w:sectPrChange section bidi revision author/kind identity, reviewable type-only w:sectPrChange section-break revision author/kind identity, reviewable complete pgSz geometry-only w:sectPrChange page-geometry revision author/kind identity, reviewable unequal-width cols-only w:sectPrChange unequal-column revision author/kind identity, reviewable lnNumType-only w:sectPrChange line-numbering revision author/kind identity, reviewable pgNumType-only w:sectPrChange page-number format revision author/kind identity, reviewable formProt-only w:sectPrChange form-protection revision author/kind identity, reviewable noEndnote-only w:sectPrChange endnote-suppression revision author/kind identity, reviewable textDirection-only w:sectPrChange section text-direction revision author/kind identity, reviewable footnotePr-only w:sectPrChange footnote-properties revision author/kind identity, reviewable endnotePr-only w:sectPrChange endnote-properties revision author/kind identity, reviewable pgBorders-only w:sectPrChange page-border revision author/kind identity, reviewable cantSplit-only w:trPrChange row-formatting revision author/kind identity, reviewable tblHeader-only w:trPrChange header-row revision author/kind identity, reviewable trHeight-only w:trPrChange row-height revision author/kind identity, reviewable hidden-only w:trPrChange row-visibility revision author/kind identity, reviewable jc-only w:trPrChange row-alignment revision author/kind identity, reviewable gridBefore-only w:trPrChange leading-grid revision author/kind identity, reviewable gridAfter-only w:trPrChange trailing-grid revision author/kind identity, reviewable wBefore-only w:trPrChange leading-width revision author/kind identity, reviewable wAfter-only w:trPrChange trailing-width revision author/kind identity, reviewable cnfStyle-only w:trPrChange conditional-format revision author/kind identity, reviewable divId-only w:trPrChange HTML-div revision author/kind identity, reviewable tblCellSpacing-only w:trPrChange cell-spacing revision author/kind identity, reviewable vAlign-only w:tcPrChange cell-formatting revision author/kind identity, reviewable solid-shd-only w:tcPrChange cell-fill revision author/kind identity, reviewable tcW-only w:tcPrChange preferred-width cell revision author/kind identity, reviewable tcMar-only w:tcPrChange cell-margin revision author/kind identity, reviewable noWrap-only w:tcPrChange cell-wrap revision author/kind identity, reviewable textDirection-only w:tcPrChange text-direction revision author/kind identity, reviewable tcFitText-only w:tcPrChange fit-text revision author/kind identity, reviewable hideMark-only w:tcPrChange hide-mark revision author/kind identity, reviewable cnfStyle-only w:tcPrChange conditional-format cell revision author/kind identity, reviewable hMerge-only w:tcPrChange horizontal-merge revision author/kind identity, reviewable vMerge-only w:tcPrChange vertical-merge revision author/kind identity, reviewable gridSpan-only w:tcPrChange column-span revision author/kind identity, reviewable tcBorders-only w:tcPrChange cell-border revision author/kind identity, reviewable current-level bullet w:numberingChange (nfc 23) numbering revision author/kind identity, reviewable current-level decimal w:numberingChange numbering revision author/kind identity, reviewable multi-level w:numberingChange (sibling original levels) numbering revision author/kind identity,
default header/footer text, first-page vs default header/footer text, even-page vs default header/footer text, first-page header PAGE live fields (fldSimple and complex fldChar), even-page header PAGE live fields (fldSimple and complex fldChar), first-page footer NUMPAGES live fields (fldSimple and complex fldChar), even-page footer NUMPAGES live fields (fldSimple and complex fldChar), first-page header DATE (`\@` format) live fields (fldSimple and complex fldChar), first-page footer TIME live fields (fldSimple and complex fldChar), even-page header DATE (`\@` format) live fields (fldSimple and complex fldChar), even-page footer TIME live fields (fldSimple and complex fldChar), first-page header SECTION live fields (fldSimple and complex fldChar), first-page footer SECTIONPAGES live fields (fldSimple and complex fldChar), even-page header SECTION live fields (fldSimple and complex fldChar), even-page footer SECTIONPAGES live fields (fldSimple and complex fldChar), footnote and endnote reference/body identity,
...
NUMWORDS plus NUMCHARS plus FILENAME plus AUTHOR plus TITLE plus SUBJECT plus KEYWORDS plus LASTSAVEDBY plus COMMENTS plus CREATEDATE plus SAVEDATE plus PRINTDATE plus MERGEFIELD plus DATE (bare and `\@` format) plus TIME field
...
SECTION and footer SECTIONPAGES live-field identity (fldSimple and complex),
figure SEQ caption
...
entry title identity in addition to v1 bookmarks/links, track changes, reviewable
jc-only w:tblPrChange table-formatting revisions, reviewable orientation-only
...
also an eligible text-only sibling (including soft breaks, tabs, carriage returns, last-rendered page breaks, page-number and date-field glyphs, non-breaking
...
with an explicit seven-shape geometry model: rectangle, rounded rectangle,
ellipse, diamond, triangle, parallelogram, and hexagon. The shape value is shared by the contextual
...
The seventy-eighth Writer milestone adds one typed field-settings dialog for
the bounded live-field grammar. Page, section, and bookmark page-reference
fields expose Arabic, upper/lower Roman, upper/lower alphabetic, Ordinal,
date/time, hyperlink, and bookmark-target choices. WPS complex fields and
their automatic `MERGEFORMAT` tails stay editable through the same pagination
resolver; unknown imported date formats remain source-preserved and unsupported
switches stay cached with compatibility diagnostics. The Commander WPS COM
probe now has numeric and common profiles, while deterministic Playground and
