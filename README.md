...
| Sort and filter | **Partial** — Traditional Office-style current-region expansion warnings for partial selections; stable value and custom-list sorting with seven built-in month/weekday sequences, bounded session-authored sequences, header retention, up to 64 ordered keys, blanks-last behavior, relative-formula translation, one-step Undo; plus worksheet AutoFilter authoring and keyboard menus | Color/icon sorting, row-direction sorting, durable host-managed custom-list settings, advanced criteria, and table/AutoFilter-integrated custom sorting |
