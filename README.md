...
| Sort and filter | **Partial** — Traditional Office-style current-region expansion warnings for partial selections; stable value-based custom sorting with header retention, up to 64 ordered keys, blanks-last behavior, relative-formula translation, one-step Undo; plus worksheet AutoFilter authoring and keyboard menus | Color/icon and custom-list sorting, row-direction sorting, advanced criteria, and table/AutoFilter-integrated custom sorting |
