...
| Sort and filter | **Partial** — Traditional Office-style current-region expansion warnings for partial selections; stable value, custom-list, effective cell-color, effective font-color, and conditional-format icon sorting with seven built-in month/weekday sequences, bounded session-authored sequences, header retention, top/bottom appearance placement, up to 64 ordered keys, blanks-last value behavior, relative-formula translation, and one-step Undo; plus worksheet AutoFilter authoring and keyboard menus | Row-direction sorting, durable host-managed custom-list settings, advanced criteria, and table/AutoFilter-integrated custom sorting |
