...
版本 `0.86.0` 让行后置宽度（width after）属性修订可审阅：

- **行后置宽度** — 无关系的 `wAfter` 先验成为 `row-formatting`，支持接受/拒绝、实时修订跟踪与原生 DOCX 导出（含当前行写出）。
- **边界** — 更广的 `tcPr` / `trPr` 形态仍为不透明元数据或失败闭合；不透明行夹具已迁至 `cnfStyle`。
