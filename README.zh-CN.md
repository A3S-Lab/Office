...
版本 `0.213.0` 将演示对象对齐改为命令菜单：

- **对齐启动器** — 对象对齐使用 `Popover` 菜单与 `menuitem` 动作，不再使用
  卡在禁用 `none` 闭合值上的 `OfficeSelect`，闭合标签保持「对象对齐」，不依赖
  假选择状态。
- **覆盖** — 单元与 Playwright 覆盖菜单动作、Escape 关闭，以及字体/对齐触发器
  的中文闭合标签适配。对话框 Escape 归属（`0.211.0`）与未裁切对话框菜单
  （`0.212.0`）仍然有效。
