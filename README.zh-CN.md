...
版本 `0.210.0` 继续修复功能区下拉闭合标签的中文宽度：

- **文档表格** — 表格布局与边框目标 `OfficeSelect` 触发器加宽，适配
  `固定列宽`、`内部横框线` 等闭合标签。
- **演示文稿动画** — 触发字段加宽，适配 `与上一动画同时` / `上一动画之后`。
- **表格数字格式** — 数字格式栈与日期时间触发器加宽，适配 `会计专用` /
  `科学计数` / `日期和时间`；删除自定义排序残留的原生 `select` 样式。
