...
版本 `0.77.0` 深化五种编辑器的 WPS UI/UX 对齐，并将共享证据矩阵扩展到
**103** 个 ACL 契约与 **78** 个视觉契约：

- **Writer** — 段落移动与缩进族、含分散对齐的对齐、项目符号、标题、正文、大小写循环、
  清除格式、双删除线、查找替换、分页符、超链接，以及域锁定/取消链接。
- **Spreadsheet** — 工作簿视图 chrome，以及 `Ctrl+PageUp`/`Ctrl+PageDown` 工作表导航。
- **Presentation** — 新建/复制/删除幻灯片、备注、组合快捷键展示、功能区折叠，以及放映导航
  （含 Home/End 与数字+Enter）。
- **Markdown** — `Ctrl+F1` 功能区折叠，以及 `Ctrl+B` / `Ctrl+I` / `Ctrl+K`。
- **PDF** — 缩放、翻页与 `Ctrl+F` 搜索焦点。
