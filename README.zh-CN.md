...
版本 `0.81.0` 让单元格适应文字（fit text）属性修订可审阅：

- **单元格适应文字** — 无关系的 `tcFitText` 先验成为 `cell-formatting`，支持接受/拒绝、实时修订跟踪与原生 DOCX 导出（含当前单元格写出）。
- **边界** — 更广的 `tcPr` / `trPr` 形态仍为不透明元数据或失败闭合；不透明单元格夹具已迁至 `hideMark`。
