...
版本 `0.93.0` 让表 tblStyle 属性修订可审阅：

- **表样式 ID** — 无关系的 `tblStyle` 先验成为 `table-formatting`（`styleId`），支持接受/拒绝、实时修订跟踪与原生 DOCX 导出。
- **边界** — 更广的 `tblPr` 形态仍为不透明元数据或失败闭合；不透明表夹具仍为 `tblCellSpacing`。
