...
- Treat bounded character, paragraph-formatting, ordered-list numbering,
  ...
  `w:gridBefore` and/or `w:gridAfter` and/or `w:wBefore` snapshot
  is reviewable as `row-formatting` with accept/reject, live track-changes for
  cantSplit/repeat-header/row-height/hidden/alignment/gridBefore/gridAfter/width-before edits, and native export.
