...
Tip reviewed against `@a3s-lab/office@0.144.0`. Use this section as the active
...
5. Note-adjacent empty glyphs: attribute-free empty `w:footnoteRef` is
   ...
   revision bodies through `0.144.0`; attributed `footnoteRef`,
   `footnoteReference` with id, `endnoteRef`, `annotationRef`, and separators
...
- Treat bounded character, paragraph-formatting, ordered-list numbering,
  ...
  bodies are admitted through `0.144.0`; attributed `footnoteRef`,
  `footnoteReference` with id, `endnoteRef`, `annotationRef`, and separators
  stay fail-closed. Floating anchors, empty or malformed drawings, unresolved
