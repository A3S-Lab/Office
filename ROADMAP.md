...
3. Shared ribbon / shortcut / focus contracts across the five surfaces, with
   ...
   Through `0.227.0`, shared `Popover` outside pointer dismiss (and
   disable-while-open) restores focus to the toolbar trigger across L2/L3
   ...
   Through `0.232.0`, Document underline / strike L2 style picks (and underline
   color) stop calling `chain().focus()` so Popover restores the ribbon
   disclosure.
   ...
   Through `0.241.0`, Document font family/size, paragraph-style, and table
   border-apply selects stop stealing focus into the editor so Popover restores
   the ribbon combobox.
   ...
   Through `0.244.0`, Document Home paragraph alignment and page-chrome
...
7. Continuous no-clobber corpus growth for representative Traditional Office
   ...
   comments, default headers/footers, first-page vs default header/footer text, even-page vs default header/footer text, first-page header PAGE live fields (fldSimple and complex fldChar), even-page header PAGE live fields (fldSimple and complex fldChar), first-page footer NUMPAGES live fields (fldSimple and complex fldChar), even-page footer NUMPAGES live fields,
...
- Treat bounded character, paragraph-formatting, ordered-list numbering,
  ...
  revision bodies through `0.187.0`; bounded `w:endnoteReference` with the same
  id contract is admitted through `0.188.0`; attributed `footnoteRef` /
  `endnoteRef` / `annotationRef` with exactly one Word-ns numeric `w:val` are
  admitted through `0.189.0`. Malformed attributed refs stay fail-closed.
  Floating anchors, empty or malformed drawings, unresolved
