...
- Treat bounded character, paragraph-formatting, ordered-list numbering,
  ...
  (`w:fmt`/`w:start` only), `w:formProt`, `w:vAlign`, `w:noEndnote`, `w:textDirection`, `w:bidi`, bounded `w:footnotePr`, bounded `w:endnotePr`, `w:type` (`breakAfter`), and bounded `w:pgBorders` edits, and native export.
