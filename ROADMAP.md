...
| Sort and AutoFilter | **Partial** with shared dense/sparse current-region discovery, Traditional Office-style exact-or-expand warnings for partial sort selections, filter toggling, hidden-row state, keyboard menus, and stable value-based custom sorting with header retention, up to 64 ordered keys, blanks last in either direction, relative-formula translation, and one-step Undo | Color/icon and custom-list sorting, row-direction sorting, advanced criteria, table/AutoFilter-integrated custom sorting, and some active criteria remain incomplete | P1 |
