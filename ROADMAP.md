...
Tip reviewed against `@a3s-lab/office@0.145.0`. Use this section as the active
...
5. Note-adjacent empty glyphs: attribute-free empty `w:footnoteRef` is
   admitted through `0.144.0` and attribute-free empty `w:endnoteRef` is
   admitted inside whole-paragraph mark, paragraph-break, and text-move
   revision bodies through `0.145.0`; attributed `footnoteRef` /
   `endnoteRef`, `footnoteReference` / `endnoteReference` with id,
   `annotationRef`, and separators stay fail-closed until dedicated
   fixtures admit them.
...
- Treat bounded character, paragraph-formatting, ordered-list numbering,
  ...
  attributed `footnoteRef` / `endnoteRef`, `footnoteReference` /
  `endnoteReference` with id, `annotationRef`, and separators stay
