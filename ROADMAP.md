...
Tip reviewed against `@a3s-lab/office@0.146.0`. Use this section as the active
...
5. Note-adjacent empty glyphs: attribute-free empty `w:footnoteRef` is
   admitted through `0.144.0`, attribute-free empty `w:endnoteRef` through
   `0.145.0`, and attribute-free empty `w:annotationRef` inside
   whole-paragraph mark, paragraph-break, and text-move revision bodies
   through `0.146.0`; attributed `footnoteRef` / `endnoteRef` /
   `annotationRef`, `footnoteReference` / `endnoteReference` with id, and
   separators stay fail-closed until dedicated fixtures admit them.
...
- Treat bounded character, paragraph-formatting, ordered-list numbering,
  ...
  are admitted through `0.146.0`; attributed `footnoteRef` / `endnoteRef` /
  `annotationRef`, `footnoteReference` / `endnoteReference` with id, and
  separators stay fail-closed. Floating anchors, empty or malformed drawings, unresolved
