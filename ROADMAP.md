...
| Data validation | **Partial**: common list, numeric, date, and text-length rules; compact maximum-sheet ranges; complete common input and error-alert settings; blank/dropdown behavior; custom titles/messages; Stop blocking plus explicit Warning/Information confirmation branches for direct and formula-bar edits; and exact native XLSX round trips | Custom formulas, dependent lists, paste/object batch validation, and broader rule-specific browser affordances remain incomplete | P1 |
