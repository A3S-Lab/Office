...
Tip reviewed against `@a3s-lab/office@0.138.0`. Use this section as the active
...
2. Remaining nested-table and SDT move-range sandwiches (multi-cell
   table-spanning companion `w:move*Range*` bookmarks with untracked
   sibling-cell text through `0.138.0`; Compare / same-document text-only
   move-range generation through `0.137.0`; single-cell table companion
   `w:move*Range*` bookmarks for supported text-only moves through `0.136.0`;
   cross-section companions through `0.131.0`).
...
- Treat bounded character, paragraph-formatting, ordered-list numbering,
  ...
  table-spanning companion `w:move*Range*` bookmarks with untracked
  sibling-cell text are admitted through `0.138.0`. Floating anchors,
  empty or malformed drawings, unresolved embeds, remaining nested /
  SDT move-range sandwiches, and remaining complex numbering edge
