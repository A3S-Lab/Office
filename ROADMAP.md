...
- Treat bounded character, paragraph-formatting, ordered-list numbering,
  ...
  (`w:fmt`/`w:start` only), `w:formProt`, `w:vAlign`, `w:noEndnote`, `w:textDirection`, `w:bidi`, bounded `w:footnotePr`, bounded `w:endnotePr`, and `w:type` (`breakAfter`) edits, and native export.
