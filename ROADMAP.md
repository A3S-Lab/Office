...
Tip reviewed against `@a3s-lab/office@0.137.0`. Use this section as the active
...
2. Remaining multi-cell, nested-table, and SDT move-range sandwiches (Compare /
   same-document text-only move-range generation through `0.137.0`; single-cell
   table companion `w:move*Range*` bookmarks for supported text-only moves
   through `0.136.0`; cross-section companions through `0.131.0`).
...
- Treat bounded character, paragraph-formatting, ordered-list numbering,
  ...
  `w:move*Range*` bookmarks on export through `0.137.0`. Floating anchors,
  empty or malformed drawings, unresolved embeds, remaining multi-cell /
  nested / SDT move-range sandwiches, and remaining complex numbering edge
