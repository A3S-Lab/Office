...
Tip reviewed against `@a3s-lab/office@0.136.0`. Use this section as the active
...
2. Broader Compare / same-document move-range generation and remaining
   multi-cell, nested-table, and SDT move-range sandwiches (single-cell table
   companion `w:move*Range*` bookmarks for supported text-only moves are
   admitted through `0.136.0`; cross-section companions through `0.131.0`).
...
- Treat bounded character, paragraph-formatting, ordered-list numbering,
  ...
  `0.136.0`. Floating anchors, empty or malformed drawings, unresolved embeds,
  remaining multi-cell / nested / SDT move-range sandwiches, remaining complex
  numbering edge cases, and broader Compare move-range generation remain
  explicit follow-up work. Relationship-free `w:tblPrChange` with a prior `w:jc`
