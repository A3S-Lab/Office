...
| Sort and AutoFilter | **Partial** with shared dense/sparse current-region discovery, Traditional Office-style exact-or-expand warnings for partial sort selections, filter toggling, hidden-row state, keyboard menus, and stable value/custom-list sorting with seven built-in month/weekday sequences, bounded session-authored sequences, header retention, up to 64 ordered keys, blanks last, relative-formula translation, and one-step Undo | Color/icon sorting, row-direction sorting, durable host-managed custom-list settings, advanced criteria, table/AutoFilter-integrated custom sorting, and some active criteria remain incomplete | P1 |
