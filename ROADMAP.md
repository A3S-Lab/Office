...
Tip reviewed against `@a3s-lab/office@0.133.0`. Use this section as the active
...
1. Untracked drawing siblings beside mark wrappers and picture-only paragraph
   bodies (supported inline DrawingML pictures are admitted through `0.133.0`;
   safe relationship-bound external hyperlinks through `0.132.0`; untracked
   text-only sibling runs through `0.130.0`) with fail-closed ineligible shapes.
...
- Treat bounded character, paragraph-formatting, ordered-list numbering,
  ...
  `0.133.0`. Untracked drawing siblings, floating anchors, picture-only
  paragraphs, table-spanning move-range sandwiches, remaining complex numbering
  edge cases, and broader Compare move-range generation remain explicit
