...
| --- | --- | --- | --- |
| Text entry, character and paragraph formatting, styles, clipboard, format painter, undo/redo | **Supported**, including mutually exclusive native `w:caps` / `w:smallCaps`; all 18 native `w:u` values with direct or theme color identity and explicit resets; standard shortcuts; page chrome; formatting revisions; and exact DOCX reopen | Long-tail Word style and typography effects outside the declared typed models can still normalize | Maintain / P0 fidelity |
