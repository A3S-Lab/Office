...
- Treat bounded character, paragraph-formatting, ordered-list numbering,
  ...
  and/or `w:tblW` and/or `w:tblInd` and/or `w:tblCellMar` and/or `w:tblLayout`   and/or `w:bidiVisual` and/or solid direct-color `w:shd` and/or `w:tblLook` and/or `w:tblOverlap` and/or relationship-free `w:tblStyle` and/or dxa `w:tblCellSpacing` and/or direct-color `w:tblBorders`
  snapshot is reviewable as `table-formatting` with accept/reject, live
  track-changes for alignment/preferred-width/indent/default-cell-margin/layout-mode/bidiVisual/solid-fill/tblLook/tblOverlap/tblStyle/tblCellSpacing/tblBorders
