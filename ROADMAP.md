...
- Treat bounded character, paragraph-formatting, ordered-list numbering,
  ...
  `w:tblHeader`, and/or `w:trHeight` and/or `w:hidden` and/or `w:jc` snapshot
  is reviewable as `row-formatting` with accept/reject, live track-changes for
  cantSplit/repeat-header/row-height/hidden/alignment/gridBefore edits, and native export.
