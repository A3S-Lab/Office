...
- Treat bounded character, paragraph-formatting, ordered-list numbering,
  ...
  and/or `w:tcW` and/or `w:noWrap` and/or `w:textDirection` and/or `w:tcFitText` snapshot is reviewable as `cell-formatting`
  with accept/reject, live track-changes for
  vertical-align/solid-fill/cell-margin/preferred-width/noWrap/textDirection/tcFitText edits, and
