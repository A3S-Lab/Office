...
- Treat bounded character, paragraph-formatting, ordered-list numbering,
  ...
  `w:cnfStyle` and/or `w:divId` snapshot is reviewable as `row-formatting` with accept/reject, live
  track-changes for
  cantSplit/repeat-header/row-height/hidden/alignment/gridBefore/gridAfter/width-before/width-after/cnfStyle/divId edits, and native export.
