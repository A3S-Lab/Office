...
| Sort and AutoFilter | **Partial** with shared dense/sparse current-region discovery, Traditional Office-style exact-or-expand warnings for partial sort selections, filter toggling, hidden-row state, keyboard menus, and stable value/custom-list/effective-color/conditional-icon sorting with seven built-in month/weekday sequences, bounded session-authored sequences, header retention, top/bottom appearance placement, up to 64 ordered keys, blanks last for value orders, relative-formula translation, and one-step Undo | Row-direction sorting, durable host-managed custom-list settings, advanced criteria, table/AutoFilter-integrated custom sorting, and some active criteria remain incomplete | P1 |
