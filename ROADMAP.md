...
| Spreadsheet | Workbook editing, formulas, recalculation, native Tables/ListObjects, bounded structured-reference formulas, charts, pivots, conditional formatting, validation, protection, comments, print setup | Automatic calculated table columns/totals, advanced pivots/slicers, external data, macros/add-ins, advanced analysis, and broader formula parity | Stabilize calculation and native workbook semantics before adding more UI |
...
| Native spreadsheet tables | **Partial**: Insert Table and grid-scoped `Cmd/Ctrl+T`, semantic ID-keyed ListObjects, contextual Table Design, 60 built-in OOXML styles, sparse-safe Convert to Range, row/column reconciliation, supported filters, native XLSX table parts, browser/Yjs convergence, and bounded structured-reference calculation for table names/display names, contiguous columns, header/data/totals selectors, qualified references, and table-local formulas | Automatic calculated-column fill, complete totals-row authoring, slicers, external/query tables, and broader table integrations remain incomplete | P0/P1 |
...
| Data validation | **Partial**: common list, numeric, date, and text-length rules; compact maximum-sheet ranges; complete common input and error-alert settings; blank/dropdown behavior; custom titles/messages; and exact native XLSX round trips | Custom formulas, dependent lists, and distinct browser confirmation branches for Warning/Information remain incomplete | P1 |
...
- Extend the implemented native table model with automatic calculated-column
  fill, complete totals authoring, slicers, and external/query-table boundaries;
  continue broadening the bounded structured-reference formula corpus.
