...
Tip reviewed against `@a3s-lab/office@0.149.0`. Use this section as the active
...
6. Browser document PDF output toward searchable text, broader vector paint,
   ...
   broader borders and full PDF/UA structure trees remain open.
...
- Move browser document PDF output toward searchable text, vector content, and
  ...
  **Partial**: Latin vector paint + title/`lang`/heading outline bootstrap are
  ...
  `0.148.0`; Writer underlines paint as vector paths through `0.149.0`; optional
  host `registerWorkPdfCjkFont` TrueType enables searchable CJK vector text;
  broader borders, full PDF/UA structure trees, and bundled CJK fonts remain open.
