...
const HELP: &str = concat!(
    ...
    */
    ...
    "Document, Markdown, Spreadsheet, Presentation, and PDF find list 1-based matches. Pass matchCount as expectedMatches and optional occurrence to document-replace-text, markdown-replace-text, or presentation-replace-text. Under concurrent peers, also echo Document paragraphId/textId/indexUtf16, Markdown indexUtf16, or Presentation containerKind/containerId/elementId/indexUtf16 from the hit so a drifted span fails closed. Spreadsheet hits name the cell for spreadsheet-set-cell. PDF hits return fieldId for pdf-set-form-value, or annotationId/pageIndex/annotationType for FreeText pdf-update-annotation. PDF form values and FreeText annotations may also pass the hit indexUtf16 and matched search so only that span is replaced.\n",
