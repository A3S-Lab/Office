...
impl NativeOfficeMcpServer {
    ...
    #[tool(
        ...
        description = "List 1-based text matches in a durable Document, Markdown, Spreadsheet, Presentation, or PDF replica without writing. Document and Markdown use matchCount with replace-text. Spreadsheet hits return sheetId, row, and column for spreadsheet-set-cell. Presentation uses the same count with presentation-replace-text, which writes only the matched scene-element text. PDF hits return fieldId for form values or annotationId, pageIndex, and annotationType for FreeText contents; edit with pdf-set-form-value or pdf-update-annotation. Pass a hit's indexUtf16 and matched search to replace only that span in a form value or FreeText contents.",
        annotations(
            read_only_hint = true,
            destructive_hint = false,
            idempotent_hint = true,
    ...
    #[tool(
        ...
        description = "Idempotently apply one authorized format-aware Office collaboration mutation; supports Markdown UTF-16 edits (replace-text may pass find-hit indexUtf16), fail-closed stable Document text/paragraph/options, Spreadsheet cells (formula writes need cached v/m beside f; no recalculate-spreadsheet-formulas), Presentation scene elements and z-order, PDF form values and FreeText annotation span edits (optional search/indexUtf16), and append-only PDF redaction/page-operation review",
        annotations(
            read_only_hint = false,
            destructive_hint = false,
            idempotent_hint = true,
