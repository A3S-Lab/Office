...
use super::identity::{
    ...
    table_row_text_id_rotations, validate_paragraph_id_input, PARAGRAPH_ID_ATTRIBUTE,
...
pub(super) fn validate_replace_paragraph(
    paragraph_id: &str,
    expected_text_id: &str,
    expected_text: &str,
    replacement: &str,
    ...
    validate_paragraph_id_input(expected_text_id, "expectedTextId")?;
    validate_paragraph_text(expected_text, "expectedText")?;
    validate_paragraph_text(replacement, "replacement")
}
...
pub(super) fn replace_paragraph(
    ...
    if target.paragraph.tag().as_ref() != "paragraph" {
        return Err(document_structure_conflict(
            "Only a plain paragraph can be replaced by this mutation.",
        ));
    }
    let current_text_id =
        document_identity_attribute(&target.paragraph, &transaction, TEXT_ID_ATTRIBUTE)?
    ...
    if current_text_id != expected_text_id {
        return Err(paragraph_match_conflict(
            "The target Document paragraph text identity changed after it was inspected.",
            Some((&current_text_id, expected_text_id)),
        ));
    }
    let current = editable_paragraph_text(&target.paragraph, &transaction)?;
    ...
    let text_id_rotations =
        paragraph_text_id_rotations(std::slice::from_ref(&target.paragraph), &transaction)?;
    let table_rows = ancestor_table_rows(std::slice::from_ref(&target.paragraph))?;
    let row_text_id_rotations = table_row_text_id_rotations(&table_rows, &transaction)?;
    drop(transaction);
    ...
    match current.node {
        Some(text) => {
            if current.length_utf16 > 0 {
                text.remove_range(&mut transaction, 0, current.length_utf16);
            }
            if !replacement.is_empty() {
                match current.first_attributes {
                    Some(attributes) => {
                        text.insert_with_attributes(&mut transaction, 0, replacement, attributes)
                    }
                    None => text.insert(&mut transaction, 0, replacement),
                }
            }
        }
        None if !replacement.is_empty() => {
            target
                .paragraph
                .push_back(&mut transaction, XmlTextPrelim::new(replacement));
        }
        None => {}
    }
...
struct EditableParagraphText {
    text: String,
    node: Option<XmlTextRef>,
    first_attributes: Option<Attrs>,
    length_utf16: u32,
}

fn editable_paragraph_text<T: ReadTxn>(
    paragraph: &XmlElementRef,
    transaction: &T,
) -> UseResult<EditableParagraphText> {
    let mut text = String::new();
    let mut node = None;
    let mut first_attributes = None;
    for child in paragraph.children(transaction) {
        let XmlOut::Text(child) = child else {
            return Err(document_structure_conflict(
                "A paragraph containing inline atoms cannot be replaced by this mutation.",
            ));
        };
        if node.is_some() {
            return Err(document_structure_conflict(
                "A paragraph containing multiple collaborative text nodes cannot be replaced as one guarded paragraph.",
            ));
        }
        for chunk in child.diff(transaction, |_| ()) {
            if chunk
            ...
            {
                return Err(document_structure_conflict(
                    "A paragraph containing comment or tracked-change marks cannot be replaced by this mutation.",
                ));
            }
            match chunk.insert {
                Out::Any(Any::String(value)) => {
                    if first_attributes.is_none() && !value.is_empty() {
                        first_attributes = chunk.attributes.map(|attributes| *attributes);
                    }
                    text.push_str(&value);
                }
                _ => {
                    return Err(document_structure_conflict(
                        "A paragraph containing inline embeds cannot be replaced by this mutation.",
                    ))
                }
            }
        }
    ...
    let length_utf16 = utf16_len(&text)?;
    if length_utf16 > MAX_DOCUMENT_PARAGRAPH_TEXT_UTF16 {
        return Err(document_structure_conflict(
            "The target paragraph is too large for a bounded structural mutation.",
        ));
    }
    Ok(EditableParagraphText {
        text,
        node,
        first_attributes,
        length_utf16,
...
fn paragraph_match_conflict(
    message: &str,
    text_ids: Option<(&str, &str)>,
) -> a3s_use_core::UseError {
    let mut error = collaboration_error("office.collaboration.mutation_match_conflict", message)
        .with_suggestion(
        "Read the latest native collaboration projection before retrying the paragraph mutation.",
    );
    if let Some((current, expected)) = text_ids {
        error = error
            .with_detail("currentTextId", current.to_owned())
            .with_detail("expectedTextId", expected.to_owned());
    }
    error
}
