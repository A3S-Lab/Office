...
character-formatting revisions from `edit` clients, preserves them across Yrs
restart, and protects them from `suggest` clients.
...
| Authorization | `edit` publishes content/review updates including validated character-formatting revisions and decisions; Document `comment` publishes semantically validated review records; authenticated Document `suggest` publishes semantically validated text proposals while preserving formatting revisions; other non-edit pairs receive only |
...
| --- | --- |
| `edit` | Accept canonical content and review updates, including bounded character-formatting revisions and their decisions, after protocol validation. |
...
when comparing canonical state. Existing formatting revisions remain in that
comparison and pass the same bounded validator, so a `suggest` connection
cannot add, remove, or rewrite them. Native projection recognizes formatting
decision records, while the closed `document-suggestion-create` and
`document-suggestion-decide` mutation payloads remain insertion/deletion-only.
