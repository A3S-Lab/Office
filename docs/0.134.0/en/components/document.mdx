...
`r:embed`) are admitted alongside visible text. Untracked supported inline
DrawingML picture siblings may sit beside matching body wrappers, mirroring
untracked text siblings. Floating anchors, empty or malformed drawings,
unresolved embeds, picture-only paragraphs, unresolved or unsafe hyperlinks,
mismatched kind/author/date, malformed or namespace-spoofed attributes,
unsupported nodes, and over-limit input stay on the structural compatibility
path with explicit diagnostics.
...
destination lives in a later section than the source. Reopening therefore
returns the same one-card review identity without `data-document-change`
leakage or transient IDs. The bounded import/export budget is 65,536 move sides
and 1,000,000 characters per move. Rich runs outside ordinary text,
table-spanning or section-sandwiching range markers, relationship-bound
