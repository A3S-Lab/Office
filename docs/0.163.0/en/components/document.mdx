...
unambiguous single-level and bounded multi-level decimal, letter, and Roman
definitions at the current `w:ilvl`. Sibling levels in `w:original` may carry
other `ST_NumberFormat` values as opaque prior text. Consecutive
...
removes every private marker. Malformed, duplicated, conflicting, unsupported
current-level bullet/picture formats, or namespace-spoofed records remain
