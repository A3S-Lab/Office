...
commands own the familiar WPS keyboard aliases while the live worksheet grid
has focus:
...
Home and Font exposes Underline as a WPS-style split control. The primary
...
The same command catalog owns the common WPS/Excel font-emphasis aliases:

| Intent | Primary shortcut | WPS alias | Direct Fortune field |
| --- | --- | --- | --- |
...
Home and Alignment exposes six WPS-familiar text-orientation choices through
...
next primary-button command. Outside and No Border items also show their WPS
shortcut hints without changing their accessible names. Successful commands
close the dialog, return focus to the workbook grid, emit one controlled
`onChange`, and create one undoable workbook operation.
