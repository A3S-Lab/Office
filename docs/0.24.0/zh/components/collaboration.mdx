...
| Spreadsheet | 工作表、命名区域、原生表格记录与冲突局部化单元格 | 工作表、区域与活动单元格 | 单元格创建、更新与删除；原生文件支持封闭的表格添加、设置和删除，浏览器协同编辑中的表格修改由浏览器绑定执行 |
...
字段的记录保存单元格和原生表格。公式、样式、数字格式、超链接、备注及独立的表格设计
字段可以分别合并：
...
筛选、标题/汇总标记、内置样式身份及首末列和行列条纹选项。修改或移动一个表格不会替换
同级表格或工作表记录。
