...
Format Cells remains a cell-wide command. A family, size, color, bold, italic,
underline, or strikethrough change is applied immutably to every visible run
through one controlled workbook update and one Undo record. A number-format
change retains the inline-string type instead of flattening the rich cell.
Unchanged semantic color origins are emitted as their native palette reference;
an edited run color falls back to explicit RGB rather than exporting stale
theme or indexed metadata.
...
enter the rich-run model. This slice preserves, renders, and applies cell-wide
font edits to existing runs; caret-level partial-run authoring remains outside
the public Spreadsheet surface.
