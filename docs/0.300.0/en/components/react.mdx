...
Disabled actions remain readable but are skipped by arrow-key and edge focus.
Opening the menu with the keyboard places focus on the first enabled action;
Escape restores focus to the **File** trigger.
