...
| --- | --- | --- | --- |
| Document | ProseMirror 结构、页面选项、评论、文字、字符与段落格式修订、最终决定和参考文献 | 文本光标与选区 | 文本替换、段落增删、页面颜色、修订模式、选区评论、署名文字建议与原子文字建议决定；Yrs 还会保护格式修订并读取其审计记录 |
| Markdown | 共享 Markdown 源码 | 源码或可视化编辑器中的光标与选区 | UTF-16 安全的替换与 splice |
| Spreadsheet | 工作表、命名区域、原生表格记录（含已校验的计算列规则）与冲突局部化单元格 | 工作表、区域与活动单元格 | 单元格创建、更新与删除；原生文件支持封闭的表格添加、设置和删除，浏览器协同编辑中的表格修改由浏览器绑定执行 |
...
1. Provider 使用经过认证的 `edit` 房间票据发送有大小限制的标准 Yjs v1 更新；
2. A3S Boot 在持久房间锁内把更新应用到候选 Yrs 文档；
3. Office 校验器只接受已知 Mark 字段、有界的作者/日期/ID，以及只包含受支持格式类型
   和标量属性的旧 Mark 快照；
4. 服务先持久化，再确认并广播；
5. 浏览器与原生副本通过下一次状态向量交换收敛，包括重启、重复或乱序投递之后。
...
在同步和审核后重新导出成原生 OOXML。在 Playground 中依次选择**体验格式修订**、
**审阅**、**查看修订（2）**；确定性 A3S Test 场景会证明拒绝后文字保留，粗体和字符
...
**段落格式**卡、拒绝该修订，并证明旧对齐、缩进、间距与行距恢复，同时正文和独立的
字符格式修订保留。
...
筛选、标题/汇总标记、内置样式身份、计算列公式及首末列和行列条纹选项。修改或移动一个
...
字节。前几项/后几项数量范围为 1–500，百分比范围为 1–100，动态筛选只能使用文档列出的
枚举，并且只有启用标题行时才能设置筛选。
...
交换与最终内容完全收敛；计算列规则也按字段同步，不会把缓存值误当作用户输入。
...
| 会话模式 | `edit` 可以修改规范内容、创建或决定文字与字符格式修订并更新审核数据。Document `comment` 可以创建选区评论、回复、解决/重开，并且只能删除当前 Actor 自己的审核记录，正文保持只读。经过认证的 Document `suggest` 可以创建署名插入/删除/替换建议，并且只能撤回自己的建议；不能作出最终决定，也不能修改规范正文、结构、格式修订、选项、评论或他人建议。`view`、非 Document `suggest` 与非 Document `comment` 只能接收。 |
...
- Document 已开放 `comment` 模式的持久评论操作，以及 `suggest` 模式的署名插入、删除
  与替换建议。`edit` 还可以创建和决定字符格式修订。只有 `edit` 参与者可以接受或拒绝，
