...
Date evaluation accepts imported `Date` values and finite Excel 1900 serials
whose native cell type is `ct.t='d'`; an ordinary unformatted number is never
silently treated as a date. Relative conditions use the user's local civil
date, Sunday-start weeks, inclusive starts, and exclusive ends. Year to Date
includes the current local day. The clock is an explicit pure-model input for
deterministic tests, while editor commands supply the current local time. Source
workbook retention for the 1904 date system remains a separate interoperability
boundary, so dynamic date authoring currently follows the normalized 1900
model.
...
value/color filter rows by the stable sort permutation. Source-level 1904
