...
| --- | --- | --- | --- |
| Document | ProseMirror 结构、页面选项、评论、文字、字符格式、段落格式与有序列表编号修订、最终决定和参考文献 | 文本光标与选区 | 文本替换、段落增删、页面颜色、修订模式、选区评论、署名文字建议与原子文字建议决定；Yrs 还会保护格式与编号修订并读取其审计记录 |
...
严格与过渡 DOCX `w:numberingChange` 对明确的单层十进制、字母和罗马数字列表使用同一
模型。只有身份与旧编号序列一致时，连续的原生逐项记录才会合并；格式错误、冲突、多层、
不支持或命名空间伪造的形式继续进入结构诊断。确定性
...
| 会话模式 | `edit` 可以修改规范内容、创建或决定文字、字符格式、段落格式与编号修订并更新审核数据。Document `comment` 可以创建选区评论、回复、解决/重开，并且只能删除当前 Actor 自己的审核记录，正文保持只读。经过认证的 Document `suggest` 可以创建署名插入/删除/替换建议，并且只能撤回自己的建议；不能作出最终决定，也不能修改规范正文、结构、格式或编号修订、选项、评论或他人建议。`view`、非 Document `suggest` 与非 Document `comment` 只能接收。 |
