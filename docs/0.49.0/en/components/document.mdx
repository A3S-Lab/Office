...
The current Compare workflow does not infer move pairs from arbitrary source
and destination edits; it continues to emit deterministic insertion/deletion
and formatting revisions. Use a native paired move or an explicit browser move
mark when exact move-review semantics are required.
...
paragraph-formatting revisions. The selected author is attached to every
generated revision, the source name is bounded transaction metadata, the whole
result publishes through one controlled `onChange`, and Undo restores the
