...
reviewable insertion, deletion, character-formatting, and
paragraph-formatting revisions in one transaction and one Undo record. Combine
admits a reviewed copy only when rejecting all of its revisions exactly
reconstructs the current baseline. The public **文档比较** Playground template,
...
targets only the destination, browser/Yrs records one immutable `move` decision,
