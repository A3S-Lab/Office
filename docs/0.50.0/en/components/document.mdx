...
Compare can infer a move only inside one simple paragraph or heading when a
unique lexical range appears once in each aligned delete/insert chunk, with
matching marks and safely carried separators. Arbitrary source/destination
edits, duplicate candidates, rich runs, and cross-paragraph ranges continue to
produce deterministic insertion/deletion or diagnostic results. Use a native
paired move or an explicit browser move mark when broader exact move-review
...
heading, Compare additionally emits one paired `move` review item. Matching
marks and safely carried separators are required; ambiguous duplicates, rich
runs, cross-paragraph ranges, and over-limit text remain ordinary revisions or
explicit diagnostics. The selected author is attached to every generated
...
deleted block removes the whole container. That container identity does not yet
map to a native paragraph-mark revision, so callers that need exact structural
accept/reject semantics should resolve those block revisions before DOCX export.
The run-level native revisions still survive reopen, and this boundary is
reported rather than presented as complete structural comparison fidelity.

Open the Playground's **文档比较** template, choose Review → **比较文档**,
and import a revised file to exercise the public path. Focused component tests,
DOCX reopen coverage, responsive Playwright, and the local-only
`word-document-comparison.acl` A3S Test suite cover controlled publication,
review decisions, focus restoration, accessibility, and empty browser
diagnostics. A3S Test is not installed or invoked by Actions or Pages.
...
and top/center/bottom vertical alignment. Every accepted change is one typed
document update and one Undo step. A text box keeps its rich inline text and
line breaks, remains selectable as an isolated block, and is kept together by
the pagination resolver so a box is not split across physical pages.

The native file boundary is deliberately small and explicit. Supported DOCX
WordprocessingML text boxes use `wps:wsp` with `txBox="1"`; geometry, inline/floating
placement, offsets, fill, outline, body padding, and vertical anchor round-trip
through the same editable model. The generated shape is also consumed by the
live page, read-only preview, and PDF capture projections. An isolated
text-box drawing in a paragraph is converted to editable content. A paragraph
that mixes the drawing with other runs, malformed shape content, or an
unsupported DrawingML branch stays on the normal compatibility path and is
reported by `docx.text-boxes` diagnostics rather than being silently presented
as a lossless edit.
