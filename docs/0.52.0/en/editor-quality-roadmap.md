...
## Shared controlled IME integrity

Document, visual Markdown, and Presentation text now share one
composition-aware controlled-state boundary. Pinyin and other pre-edit text
stays editor-local; only the committed value is published, and an authoritative
host replacement waits until composition settles. Ordinary typing takes no
additional timer path. Focused component regressions plus a dedicated WebKit
Playwright gate make this a permanent P0 text-entry contract.
...
layout, wrapping, crop, bounded rotation/reflection, and layer metadata through
public import/export. Export
...
The body Writer surface now exposes a deliberately smaller editable slice on
top of that package-preservation boundary: direct paragraph inline controls can
be authored as plain text or rich text with aliases, tags, multiline state,
appearance, color, and independent content or shell locks. Selection, preview,
pagination, and Undo use the same inline node, and generic transactions cannot
silently bypass a lock. Transitional and strict `w:sdt` import/export allocate
collision-free native IDs and preserve supported run formatting. Data binding,
placeholders, repeating regions, date/dropdown/picture/form controls, block or
nested controls, and relationship-bound semantics remain explicit follow-up
boundaries with compatibility diagnostics.
...
Writer Office 2010 OpenType typography now shares one closed semantic model
between ordinary text and structured equations. The advanced Font dialog
authors all 16 ligature combinations, number form and spacing, stylistic sets
1-20, and contextual alternates with independent mixed states, Follow style,
partial per-run patches, and one-step Undo. Body and page-chrome stories,
notes, comments, Format Painter, formatting revisions and rejection, exact
`w14` namespace export, reopen, diagnostics, and browser-authoritative shaping
measurement are permanent local gates.
...
Malformed or namespace-spoofed properties fail closed. No shortcut is invented.
Writer hidden text now retains inherited, native hidden, and explicit visible
reset states through one `w:vanish` model. The shared advanced font dialog
keeps mixed state independent, `Cmd/Ctrl+Shift+H` uses the standard scoped
toggle, and one action remains one Undo record. Body and page chrome, notes,
Format Painter, formatting revisions, strict/transitional style inheritance,
exact export, and reopen retain the value without replacing an existing
character style. Hidden text is suppressed by default, in preview, and in PDF;
the View ribbon reveals it only while editing with a dotted underline.
Malformed or namespace-spoofed values fail closed, unchanged comment XML stays
source-preserved, and affected paragraphs use browser-authoritative line
measurement.
Writer native outline, shadow, emboss, and imprint now retain four independent
nullable states through exact `w:outline`, `w:shadow`, `w:emboss`, and
`w:imprint` properties. Missing values inherit and explicit false values remain
direct resets. Outline plus shadow is valid; emboss and imprint stay mutually
exclusive with every other enabled effect. The shared advanced font dialog
keeps each mixed state independent, atomically clears conflicts, and commits
one Undo record. Every editable Word story, document defaults, inherited
styles, Format Painter, formatting revisions, strict/transitional import,
diagnostics, exact export, and reopen share the typed model. Bounded CSS and PDF
projections remain paint-only, so eligible paragraphs stay on Worker/WASM
layout. Focused Rstest, desktop/compact Playwright, and local-only A3S Test 1.0.0
coverage exercise the public Playground template without adding A3S Test to
GitHub Actions or Pages.
Writer character borders now retain one exact native `w:bdr` value across all
25 visible line styles plus `nil` and `none`, direct or theme colors with
tint/shade, 2–96 eighth-point widths, 0–31 point spacing, and explicit shadow
and frame flags. The Home Font group exposes a direct toggle, while the shared
advanced font dialog keeps mixed selections safe and distinguishes Follow
style, explicit no-border, and a complete editable border. Line style, color,
width, spacing, shadow, and frame commit with every other touched character
property through one transaction and one Undo record; Format Painter uses the
same mark and no non-standard shortcut is claimed. Document defaults, styles,
every editable Word story, formatting revisions, strict/transitional import,
diagnostics, exact export, and reopen share the typed model. Visible borders
use browser-authoritative line measurement because CSS border and padding
change inline geometry; explicit resets remain eligible for Worker/WASM, and
browser/PDF line styles stay bounded visual approximations. Focused Rstest,
...
Writer Table of Contents now uses one selectable, non-editable typed block
with semantic heading/native outline levels 1–9, hyperlinks, page-number
visibility and alignment,
dot/dash/underline/no leaders, stable heading targets, and at most 512 cached
entries. Insert, selected-block customization, and explicit all-TOC refresh
each create one transaction and one Undo record. The shared outline collector
is the only heading source and the live pagination field resolver is the only
page-number source. Included items without paragraph identities receive them
inside the same insert/update transaction. Native DOCX export, import, and a
second export retain a
real `w:sdt`/`TOC` field, cached entries, heading bookmarks, and exact leader
semantics. Common lossless switches reopen as editable state; custom style
mappings, partial page ranges, malformed ranges, and unsupported separators
are diagnosed instead of approximated. Focused Rstest, responsive UI coverage,
the public **可更新目录** Playground template, and a local-only A3S Test gate
cover discoverability, editing, refresh, focus recovery, Undo, accessibility,
and empty browser diagnostics without adding A3S Test to Actions or Pages.
Writer native indexes now use stable selectable `documentIndexEntry` atoms and
one selectable generated `documentIndex` block. Primary and secondary terms,
cross-references, bold/italic page intent, 1–4 columns, indented/run-in layout,
alignment, and four leader modes stay typed. The shared pagination field
resolver is the only page source; normalized duplicate terms on the same page
merge without discarding marker identities. Each mark/edit, block
insert/customize, and all-index refresh is one transaction and one Undo record.
At most 2,048 markers contribute to at most 512 cached rows, with explicit
truncation beyond either bound. Native DOCX export, import, reopen, and second
export retain `w:fldSimple` `XE` fields, a real `w:sdt`/`INDEX` field, cached
`Index1`/`Index2` rows, cross-references, page emphasis, columns, and leaders.
Only the lossless common switch subset becomes editable; range markers, custom
types, locale controls, malformed structures, and unknown switches remain
diagnosed. Focused Rstest, responsive UI coverage, the public **原生索引**
Playground template, and a local-only A3S Test gate cover discovery, marking,
refresh, focus recovery, Undo, native reopen, accessibility, console, and page
errors without adding A3S Test to Actions or Pages.
Writer document compare/combine now forms one completed bounded vertical slice.
Review can import DOCX, HTML, or TXT against the current controlled baseline,
deterministically align same-layout paragraphs and headings, and generate
reviewable insertion, deletion, character-formatting, paragraph-formatting, and
bounded same-section text-move revisions in one transaction and one Undo
record. A move is admitted only for a unique lexical range with matching marks
and safely carried separators inside one simple paragraph or heading, or across
aligned simple text blocks in the same section; duplicate, mark-mismatched,
rich, section-boundary, table, and over-limit candidates remain ordinary
revisions or diagnostics. Combine admits a reviewed copy only when rejecting all
of its revisions exactly reconstructs the current baseline. The public
**文档比较** Playground template, bilingual references, focused Rstest,
responsive Playwright, native `w:ins`/`w:del`/`w:rPrChange`/`w:pPrChange` reopen
coverage, and local-only A3S Test gate cover the shipped path. Exact text-only
whole-paragraph insertions and deletions now also round-trip through matching
native `w:pPr/w:rPr` paragraph-mark records and retain one atomic block
decision. Isolated paragraph-break merge/split revisions, mixed-content marks,
changed complex structures, cross-section/range move generation, and multi-copy
structural conflict resolution remain explicit fail-closed follow-up work.
Existing strict or transitional text-only
`w:moveFrom`/`w:moveTo` pairs still import as one atomic review item and
round-trip natively; rich, range-marker, relationship-bound, or unpaired moves
remain diagnosed instead of being flattened.
...
reference frames, four-edge percentage cropping, quarter-turn rotation,
horizontal/vertical reflection, and alternative text. No-wrap images use a
one-pixel pagination anchor instead of
...
The ribbon and dialog expose the same bounded 90-degree rotation and reflection
model; DOCX export/reopen verifies `a:xfrm` `rot`, `flipH`, and `flipV`, while
arbitrary-angle or malformed values receive a visible compatibility diagnostic
and normalize safely.
...
diagnostics.
Body `PAGE`, `NUMPAGES`, `SECTION`, `SECTIONPAGES`, `DATE`, `TIME`, `NUMWORDS`,
and `NUMCHARS` fields now resolve from the measured Worker/WASM page containing
each atom, including physical-page membership for continuous sections. Numeric
statistics exclude generated field results, preserve word boundaries around
inline atoms, and count spaces separately from line-break separators. Pagination
refreshes numeric results without adding history or advancing clock fields;
scoped F9 refreshes all field kinds as one undoable action. The cross-reference
picker can create a bookmark-backed `PAGEREF` that stores stable target identity,
follows bookmark normalization, and reports a missing target instead of serving
stale semantics. Transaction mapping preserves moved field identities, gives
copied atoms fresh redo-stable identities, and keeps insertion plus its initial
refresh in one history step. Native DOCX simple and complete inline complex
fields round-trip twice; only the bounded common switch grammar is atomized,
while unsupported, nested, cross-paragraph, deleted, or instructionless
structures remain cached text with explicit compatibility diagnostics. Unit
coverage exercises live page contexts, statistics, bookmark retargeting, copy
and undo/redo identity, clock-safe automatic refresh, structural diagnostics,
and native DOCX output; a deterministic phone workflow covers common fields,
bookmark page references, physical PAGE and NUMPAGES results, F9, atomic
undo/redo, accessibility, and browser diagnostics.
...
calculated-column fill on row insertion is now supported for empty cells in
new table body rows. The rule is inferred only from one consistent current-row
formula, persists in table metadata, and preserves manual values, formulas,
styles, and exceptions; conflicts remove the rule. Dense and sparse sheets,
Yjs validation, and native XLSX `<calculatedColumnFormula>` round trips are
covered. Common totals-row authoring is now also covered: the Table Design
dialog, per-column aggregate/label/custom formula state, filter-aware
`SUBTOTAL`, direct-edit reconciliation, structural movement, rename safety,
dense/sparse projections, Yjs validation, and native XLSX totals metadata all
have focused evidence. Slicers, external/query tables, and advanced
...
The thirty-first slice preserves native XLSX rich-text cells from shared and
inline strings through import, cell-wide formatting, export, and reopen.
Fortune's existing `ct.s` runs retain font family, size, color, bold, italic,
strikethrough, four underline variants, and exact edge whitespace. Matching
theme, indexed, automatic, and tint identities remain semantic; changed colors
export as RGB. One Format Cells intent immutably updates every run while
retaining the inline-string type, one controlled revision, and one Undo record.
Fail-closed budgets cap a cell at 32,767 characters and 512 runs, and an import
...
or Pages. The following slices complete same-column AND/OR, wildcard,
Top/Bottom, dynamic evaluation, and table/AutoFilter-owned Custom Sort.
...
to Actions or Pages. The following slices complete wildcard, Top/Bottom,
dynamic evaluation, and table/AutoFilter-owned Custom Sort.
...
application, Undo, accessibility, XLSX reopen, and clean diagnostics. Later
slices complete wildcard, dynamic evaluation, and table/AutoFilter-owned
Custom Sort; large-rank Worker/WASM offload remains open.
...
diagnostics. The following slices complete dynamic evaluation and
...
accessibility, and clean diagnostics. Large aggregate/rank Worker/WASM offload
remains open; the fifty-first slice below completes table/AutoFilter-owned
Custom Sort and the fifty-second completes source-level 1904 retention.

The fiftieth Spreadsheet slice completes bounded custom-list preference
management inside Custom Sort. One responsive, keyboard-accessible dialog keeps
the seven built-in sequences read-only and stages creation, editing, deletion,
and reordering for at most 32 user lists before one atomic confirmation.
Duplicate and size validation reuse the canonical list model; unchanged
confirmation does not write the typed store; cancellation discards the draft;
and a rejected write preserves the complete changed set for the mounted session
with visible feedback. Active sort keys follow edited sequences and return to
ascending value order when their sequence is deleted. Focused Rstest and
desktop/compact Playwright cover bounds, persistence order, remounts, responsive
containment, focus restoration, accessibility, and clean diagnostics.

The fifty-first Spreadsheet slice integrates Custom Sort with native table and
worksheet AutoFilter ownership. Any contained selection resolves to the exact
sortable owner; table totals are excluded, structural headers remain fixed, and
left-to-right movement is disabled. The request carries an owner fingerprint
that is derived again immediately before mutation, so table replacement,
header/totals changes, and moved filters fail closed. Filter-active sorts emit
one controlled workbook change: typed worksheet/table conditions are evaluated
against the sorted dense or sparse cells, opaque Fortune value/color filters
remap hidden-row ownership through the stable sort permutation, and independent
manual hiding remains intact. Missing hidden rows from the native range reader
fall back to controlled cells, while sort area and filter rescans stay under
explicit one-million-cell budgets. Focused Rstest, a local-only A3S Test gate,
and desktop/compact Playwright cover table formulas, native filter visibility,
single-step Undo, focus, responsive containment, accessibility, and clean
diagnostics.

The fifty-second Spreadsheet slice makes the workbook date system an explicit
controlled-model field. XLSX import retains the native 1900 or 1904 epoch and
exact date-typed numeric serials; export restores `workbookPr date1904` and
numeric formula caches without a JavaScript `Date` conversion. Dynamic date
filters, menu profiling, post-edit and post-sort reconciliation, and `Ctrl+;`
authoring consume the same epoch. Collaboration validates and merges the field
independently from calculation settings. Focused unit, real-package reopen,
and local A3S Test browser coverage verify 1904 serial zero, filter ownership,
controlled reopen, accessibility, and clean diagnostics.
...
- Keep the bounded entrance/exit animation model composable across the object-
  centric ribbon, controlled history, clipboard, collaboration, slideshow,
  reduced motion, and native PPTX timing-tree reopen. Reject duplicate classes,
  overlapping same-object cue intervals, inconsistent transitions, and
  untrusted timing structures before they enter editable state.
...
- Retain the completed first page-organization slice: insert, delete, rotate,
  reorder, extract, merge, and split run in a dedicated Worker; every mutation
  replaces one Blob and creates one Undo record; native history is consumed
  first; and 256 MiB primary, 128 MiB merge, and 4,096-page limits fail closed.
- Add form authoring, signatures, document-level catalog rewriting, and
  explicit redaction review while preserving the implemented search, history,
  save, and page-organization paths.
...
- Verify saved annotations, forms, and page mutations by reopening the emitted
  Blob in PDFium and an independent binary parser. Keep signed, encrypted, and
  unsafe catalog structures inside the documented fail-closed boundary.

Exit evidence: large files render a bounded page window, page exports retain
their declared count/order/rotation, edits survive reopen, and destructive
operations always show a review step before save.
...
- PDF: `loadSource`, `onSave`, and `onPageExport`.
