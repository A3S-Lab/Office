...
The next WPS drawing boundary is explicit as well. The installed WPS 12.0 COM
probe writes `Shapes.AddConnector` as a legacy VML `v:shape` with
`o:spt="32"`/`#_x0000_t32`; it is not a text-bearing `wps:wsp`. A connector-only
`w:pict` therefore produces `docx.connectors`, keeps endpoint, routing,
arrowhead, and floating-anchor semantics on the compatibility path, and is not
misreported as a text box or an image. The deterministic connector fixture and
`word-wps-connector-boundary.acl` prove that the editor remains usable and
capture accessibility, screenshot, console, and page-error evidence. A typed
editable connector model is intentionally still open.
