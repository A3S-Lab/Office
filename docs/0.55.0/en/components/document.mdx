...
unambiguous single-level decimal, letter, and Roman definitions. Consecutive
per-item records become one atomic list intent only when author, date, numbering
identity, level, format, suffix, and original sequence agree. Export writes
native sequential records back under `w:numPr`, enables `w:trackRevisions`, and
removes every private marker. Malformed, duplicated, conflicting, multi-level,
unsupported, or namespace-spoofed records remain structural diagnostics.
...
so it is not guessed as a whole-paragraph change. Mixed or relationship-bound
content, multiple body wrappers, mismatched kind/author/date, malformed or
...
positive ID, author, and UTC date. Reopening therefore returns the same one-card
review identity without `data-document-change` leakage or transient IDs. The
bounded import/export budget is 65,536 move sides and 1,000,000 characters per
move. Rich runs outside ordinary text, range markers, relationship-bound
objects, malformed metadata, duplicate identities, and unpaired sides remain
on the structural compatibility path and are reported rather than flattened
into an inexact editable move.
...
separators. Arbitrary source/destination edits, duplicate or mark-mismatched
candidates, rich runs, section-boundary ranges, tables, and over-limit text
continue to produce deterministic insertion/deletion or diagnostic results.
Use a native paired move or an explicit browser move mark when broader exact
move-review semantics are required.
...
revisions or explicit diagnostics. The selected author is attached to every generated
...
(`\\h` and `\\* MERGEFORMAT` where applicable); nested, malformed, unsupported,
...
top/center/bottom vertical alignment, and five explicit shape presets:
rectangle, rounded rectangle, ellipse, diamond, and triangle. Every accepted
...
The bounded connector slice now owns isolated straight connectors as editable
content. The contextual Connector ribbon exposes solid, dash, dot, and
dash-dot line styles alongside color, width, endpoint arrows, layout, offsets,
and endpoint percentages. One typed line-style value drives the live SVG,
Undo/Redo, compact-ribbon controls, and native DrawingML `a:prstDash` export.

The WPS boundary remains explicit. The installed WPS 12.0 COM probe writes
`Shapes.AddConnector` as a legacy VML `v:shape` with `o:spt="32"`/`#_x0000_t32`
and reports native `DashStyle=4` for a dashed line; it is not a text-bearing
`wps:wsp`. VML `dashstyle` and DrawingML `a:prstDash` values map into the same
bounded model. Unsupported routed connectors, long-tail dash styles, mixed
paragraphs, arbitrary or malformed drawings, and endpoint/routing semantics
outside the straight model remain fail-closed compatibility boundaries rather
than being presented as exact edits. The deterministic fixture,
`word-connector-editor.acl`, and `word-wps-connector-boundary.acl` cover the
authoring/import path with screenshots, accessibility, console, and page-error
evidence; the `docx.connectors` diagnostic keeps unsupported source branches
visible to hosts.
...
intent together: choose plain text or rich text, give it a display name and
program tag, allow multiline text where appropriate, choose a border, tag, or
hidden appearance, and optionally set a theme-independent color. Selecting an
existing control opens the same dialog for its metadata, while the control's
accessible textbox name follows its alias or tag.
