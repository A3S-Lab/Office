...
# Create/delete use presentation-create/delete-element with the same container.
...
`presentation-update-element`, and `presentation-delete-element`. All target a
...
`id` and `type` cannot change. Delete requires the exact complete current
`expectedElement`, removes it from visible order, and writes a durable
tombstone that permanently reserves the ID. Element JSON is bounded and
prototype-sensitive keys are rejected before mutation.
...
scene-element lifecycles. Richer Document review/structure, Spreadsheet and
Presentation structural/rich-text mutations, PDF signatures, and native
presence projection remain Phase 6 milestones.
