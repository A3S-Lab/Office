...
update, or tombstone one scene element in a stable slide, master, or layout.
Canonical creation claims prevent conflicting same-ID reuse; top-level
expected/current/next field comparison merges unrelated concurrent changes;
exact deletion removes visible order and permanently reserves the element ID.
Neither format mutation replaces a worksheet, workbook, container, or deck.
...
Presentation structural/rich-text mutations, PDF signatures, durable
