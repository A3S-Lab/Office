...
| --- | --- | --- | --- |
| Document | ProseMirror 结构、页面选项、评论与参考文献 | 文本光标与选区 | 文本替换、段落增删、页面颜色与修订模式 |
...
| 会话模式 | `edit` 可以修改五种格式；`view`、`comment` 与 `suggest` 当前只能接收远端内容，直到各格式具备持久审核模型。 |
...
- A3S Office 提供的是传输中立的编辑器绑定，不附带账号系统、协作后端、房间服务或
  数据库存储。
- `comment` 与 `suggest` 模式的本地持久操作尚未开放；它们可以接收并观察已经授权的
  远端更新。
