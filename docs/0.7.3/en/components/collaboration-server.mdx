...
room broadcast, creates a durable Document selection comment, and proves that
review-only or view-only clients cannot forge canonical content.
...
| Authorization | `edit` publishes content/review updates; Document `comment` publishes semantically validated review-only updates; `view` and `suggest` receive only |
...
session actor name and the `author` of every new comment or reply. The adapter
verifies artifact ID/kind, namespace, actor ID/name/kind, mode, and server Yjs
client identity in `collaboration.ready`; a mismatch closes the connection
instead of attaching the wrong room identity.
...
| `view` / `suggest` | Answer synchronization and relay presence, but reject document publishing. |
| Non-Document `comment` | Receive-only until that format has a durable review model. |
...
Origin, payload size, and ticket expiration. Client-supplied origin metadata is
replaced with attribution derived from the signed ticket. Durable receipts
retain that trusted host authorization metadata for audit and reconnect.
