...
| --- | --- | --- | --- |
| Document | ProseMirror structure, options, comments, and sources | Text caret and selection | Text, paragraphs, page color, track changes, and selection comments |
...
receivable by every mode so receive-only peers still converge. `suggest` and
comment mode on non-Document formats do not expose local mutations.
...
| Modes | `edit` can change canonical content and review data. Document `comment` can create selection comments, reply, resolve/reopen, and delete only its actor's own review records while canonical content stays read-only. `view`, `suggest`, and `comment` on other formats are receive-only. |
