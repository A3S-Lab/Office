...
| --- | --- | --- | --- |
| Document | ProseMirror 结构、页面选项、评论与参考文献 | 文本光标与选区 | 文本替换、段落增删、页面颜色、修订模式与选区评论 |
...
结构片段包含分节布局、评论锚点和修订标记。评论、回复与参考文献按稳定 ID 保存，
独立记录可以合并，不需要替换文档大小的 JSON。浏览器暂不支持的 OOXML 包部件仍由
宿主导入导出流程管理，不应作为完整 ZIP 替换写入 Yjs。
...
| 会话模式 | `edit` 可以修改规范内容和审核数据。Document `comment` 可以创建选区评论、回复、解决/重开，并且只能删除当前 Actor 自己的审核记录，正文保持只读。`view`、`suggest` 以及其他格式的 `comment` 只能接收。 |
...
- A3S Office 提供的是传输中立的编辑器绑定，不附带账号系统、协作后端、房间服务或
  数据库存储。
- Document 已开放 `comment` 模式的持久审核操作；`suggest` 与其他格式的
  `comment` 仍只能接收并观察经过授权的远端更新。
