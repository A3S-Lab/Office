...
# structure, option fields, plain text, projection-v2 comments/replies/anchors,
# and the state vector used as a guard. Projection v2 does not yet list
# tracked suggestions or final suggestion-decision records.
...
# selection comment after reading exact projection-v2 paragraph/text identity,
...
`document-comment-set-resolved`, and `document-comment-delete`. Exact text
...
retained and projection v2 reports `detached: true`. Each projected anchor
...
fail. Raw `apply` and received y-sync updates remain usable in every replica
mode because receive-only peers must still integrate remote changes authorized
by the host. Authenticated browser-originated Document `suggest` updates can be
semantically authorized by the A3S Boot/Yrs server and then synchronized into a
native replica. However, `document-suggestion-*` and
`document-change-decision-*` are not variants of
`NativeOfficeCollaborationMutation`: `collab mutate`,
`office_collaboration_mutate`, and projection v2 cannot yet originate or list
those workflows. Agents must not hand-author private ProseMirror/Yjs marks to
bypass the closed mutation schema. Non-Document `suggest` and `comment` modes
do not expose local mutations.
