...
| --- | --- | --- | --- |
| Document | ProseMirror structure, options, comments, attributed suggestions, final decisions, and sources | Text caret and selection | Text, paragraphs, page color, track changes, and selection comments; typed suggestion/decision mutations are not exposed yet |
| Markdown | Markdown source | Source or visual caret and selection | UTF-16-safe replace and splice |
| Spreadsheet | Sheets, names, and conflict-local cells | Sheet, ranges, and active cell | Cell create, update, and delete |
...
receivable by every mode so receive-only peers still converge. Authenticated
Document `suggest` updates are authorized by the A3S Boot/Yrs transport path,
but `collab mutate`, `office_collaboration_mutate`, and projection schema v2 do
not yet provide typed operations for creating a suggestion or deciding one.
Do not construct private ProseMirror/Yjs marks in an agent merely to bypass
that closed mutation boundary. `suggest` on non-Document formats and comment
...
Native agents use the same records through the projection-v2 read/mutate
...
`office_collaboration_read` now return projection schema version `2`, including
comments, replies, resolution and detached state, and every anchor's
`paragraphId`, `textId`, `startUtf16`, `endUtf16`, and current `text`.
...
This release does not add `document-suggestion-*` or
`document-change-decision-*` variants to the native mutation enum. CLI/MCP
replicas synchronize browser-produced suggestions and decision roots as normal
Yjs state, but projection v2 does not list those records and the typed mutation
surface cannot originate them yet.
...
### Mutate one Spreadsheet cell from Rust, CLI, or MCP

Native clients should use the closed `spreadsheet-set-cell` and
`spreadsheet-delete-cell` mutations instead of constructing Yjs fields. Rows
and columns are zero-based, and `sheetId` is the stable sheet identity rather
...
| Modes | `edit` can change canonical content, decide tracked proposals, and update review data. Document `comment` can create selection comments, reply, resolve/reopen, and delete only its actor's own review records while canonical content stays read-only. Authenticated Document `suggest` can create attributed insertion/deletion/replacement proposals and withdraw only its own proposals; it cannot make final decisions or mutate canonical content, structure, formatting, options, comments, or another actor's proposal. `view`, non-Document `suggest`, and non-Document `comment` are receive-only. |
