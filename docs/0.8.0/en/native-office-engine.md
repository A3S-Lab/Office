...
must remain equivalent. This path does not add native typed suggestion-create,
suggestion-decision, or projection-v2 records; CLI/MCP replicas only synchronize
the browser-produced Yjs state for those workflows today.
...
list, and section operations, typed suggestion creation/projection/final
decisions, Spreadsheet structural and remaining Presentation
structural/rich-text mutations, PDF signatures, and native presence projection
