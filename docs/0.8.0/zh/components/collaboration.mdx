...
| --- | --- | --- | --- |
| Document | ProseMirror 结构、页面选项、评论、署名建议、最终决定与参考文献 | 文本光标与选区 | 文本替换、段落增删、页面颜色、修订模式与选区评论；暂未开放建议/决定的原生类型化 Mutation |
...
原生智能体通过同一份投影 v2 和类型化变更管理评论。决策前先读取最新投影，确保
...
`office_collaboration_read` 的投影版本现在是 `2`，会返回评论、回复、解决状态、
脱离锚点状态，以及每个锚点的 `paragraphId`、`textId`、`startUtf16`、`endUtf16`
和当前 `text`。
...
本版本尚未向原生 Mutation 枚举加入 `document-suggestion-*` 或
`document-change-decision-*` 操作。CLI/MCP 副本会把浏览器产生的建议和决定 Root 当作
普通 Yjs 状态同步，但投影 v2 不会列出这些记录，类型化 Mutation 也暂时不能主动创建
或决定建议。
...
a3s-office collab session .a3s/report.replica --poll-ms 100 --json
```
...
- `collab mutate`、`office_collaboration_mutate` 与原生投影 v2 暂未提供建议创建或决定的
  类型化接口；原生副本只能同步浏览器已经产生的对应 Yjs 状态。
