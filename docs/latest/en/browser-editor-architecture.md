...
1. The Office shell owns the ribbon, status bar, dialogs, keyboard routing,
   ...
   Data, and owns F9 workbook recalculation in the root-scoped keyboard
    ...
    values or workbook structure. The catalog is the source of truth for
    labels, locations, and shortcut metadata; typed command ports remain the
    source of truth for execution and availability.
   The
