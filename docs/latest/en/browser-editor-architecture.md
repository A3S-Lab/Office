...
`shape-outside`, and top-and-bottom images clear surrounding text. Paragraphs
...
paginator reserves and observes the image block height, then sends the measured
text fragments to the same Worker/Rust-WASM page-layout protocol. Square,
tight, through, and top-and-bottom metadata round-trips through DOCX
...
inline and floating pictures. Z-order and drawing-layer relationships are not
represented yet.
