...
direct-section paragraph identity. `document-delete-paragraph` additionally
requires the current `textId` and complete text, and refuses last-section-block,
inline-atom, comment-mark, and tracked-change cases. Word paragraph identities
are uppercase eight-digit positive 31-bit hexadecimal values.
...
fields fail-closed. Use `--mutation-input <file>` for larger or multiline JSON.
