...
Review exposes **设置校对语言** beside Spelling Check. Its accessible dialog
accepts bounded BCP 47 tags, reports mixed values independently, leaves every
untouched slot unchanged, and offers Follow style for each language and for the
proofing state. Apply restores the captured selection and focus and commits all
touched fields through one TipTap transaction and one Undo record. Header and
footer editing invokes the same model.
...
To inspect paired native move semantics without importing a file, choose
**体验移动修订**, then open **审阅**. The demo presents the source and
destination as one **移动** card; accepting keeps the destination text,
rejecting keeps the source, and the same review surface becomes modal at phone
width.
...
The floating Writer selection toolbar uses the same compact, theme-aware button
tokens as its surrounding actions. Underline and strikethrough split controls
explicitly reset browser-native button appearance, keep a single-pixel divider
between the primary action and its disclosure, and retain the same hover,
pressed, focus, and keyboard semantics as the Home ribbon. This prevents a
browser's default outset border from changing the toolbar's density or making
...
Accept keeps the current list properties and removes only the review metadata.
Reject restores the complete original numbering snapshot without changing list
text. The original command and each decision remain independent one-step Undo
boundaries. The review panel presents one **编号格式** card, so a native record
per list item is not exposed as a misleading stack of paragraph changes.
...
[Synchronize ordered-list numbering revisions](./collaboration.mdx#synchronize-ordered-list-numbering-revisions).
...
The Changes pane shows one card rather than a misleading body change plus an
empty structural revision. Accepting an inserted paragraph or rejecting a
deleted paragraph preserves the complete block; rejecting an insertion or
accepting a deletion removes it. Each decision is one transaction and one Undo
boundary. Export writes the body revision and a separate native paragraph-mark
revision, enables tracked changes, removes private markers, and reopening the
DOCX restores the same atomic semantics. Import and export admit at most 65,536
records.
...
Compare can infer a move inside a simple paragraph or heading, and can pair a
range across aligned simple text blocks when both blocks belong to the same
document section. In either case the unique lexical range must appear once in
each aligned delete/insert chunk, with matching marks and safely carried
separators. Those admitted inferred pairs export the same companion range
bookmarks as native moves. Arbitrary source/destination edits, duplicate or
mark-mismatched candidates, rich runs, section-boundary ranges, tables, and
over-limit text continue to produce deterministic insertion/deletion or
diagnostic results. Use a native paired move or an explicit browser move mark
when broader exact move-review semantics are required.

The Playground's **体验移动修订** button provides a repeatable text-only
fixture for checking this atomic accept/reject interaction without a file
import.
...
paragraph-formatting revisions. When one simple text paragraph or heading
becomes two (or two become one) with exact text concatenation, Compare emits a
reviewable `paragraph-break` split or merge with the same accept/reject join
semantics as native DOCX import. Soft breaks (`<br>` / `hardBreak`) are admitted
inside simple paragraphs and headings for both inline diffs and whole-paragraph
structural insert/delete (native paragraph-mark export). When a lexical range
...
Combine is a different, fail-closed operation. The imported reviewed copy must
already contain revisions, the current document must contain none, and rejecting
every imported revision on an immutable snapshot must reproduce the current
baseline exactly. The current bounded path admits inline insertion/deletion,
character-formatting, and paragraph-formatting revisions while retaining the
current paragraph identities. A missing revision set, malformed prior-format
snapshot, baseline mismatch, or structural block revision leaves the current
document untouched and returns a localized diagnostic.

Comparison accepts at most 1,024 blocks in each version and 1,000,000 combined
text characters. Block alignment and each inline diff allocate at most about
1.1 million matrix cells. The two documents must have the same section count
and page-layout attributes. Unchanged complex blocks may pass through, but a
changed table, list, image, equation, content control, empty structural block,
or other complex tree is never flattened into a misleading text diff. Existing
revisions in either comparison input, unsupported review marks, and layout
changes also fail closed. Same-shape tables whose cells hold only simple text
paragraphs or headings are an admitted exception: cell text diffs become
reviewable inline insertions and deletions.
...
above, and reopen preserves the atomic block decision. Soft-break paragraphs
follow the same mark path. Richer mixed-content paragraph-mark picture shapes
and other structural changes still fail closed and are reported rather than
presented as complete structural comparison fidelity.
...
Writer represents a Table of Contents as a selectable, non-editable typed block
instead of flattening generated rows into ordinary text. Insert and Customize
in the References ribbon read the existing shared outline, accept semantic
headings and native outline-level paragraphs across levels 1 through 9, and
preserve hyperlinks, page-number visibility, right alignment, and dot, dash,
underline, or no leader. Each insert or option change is one TipTap transaction
and one Undo record. No shortcut is advertised because the conventional Writer
surface does not define a separate direct shortcut for this command.

Every entry stores a bounded title, level, page number, and stable target. An
included outline item without a native paragraph identity receives one in the
same insert/update transaction, so inserting the TOC cannot invalidate its own
links and Undo restores both changes together. Page values come
from the same live Worker/WASM pagination resolver used by PAGE fields. The
explicit **Update Table of Contents** action recomputes every block from those
two sources in one transaction, so changing a heading or pagination never
creates a second heading or page-number model. A document stores at most 512
cached entries per block; additional matches are reported as truncated rather
than producing an unbounded node attribute.
...
Writer keeps every marked index term as a selectable inline atom and keeps the
generated index as a separate selectable, non-editable block. In References,
**Mark Index Entry** captures selected text and can author a primary term,
secondary term, cross-reference, or bold/italic page-number intent. **Insert or
Customize Index** controls one through four columns, indented or run-in layout,
page-number alignment, and dot, dash, underline, or no leader. The conventional
Writer surface does not define separate direct shortcuts for these commands, so
the command catalog does not invent any.

The generated block sorts normalized terms, combines duplicate terms on the
same page, and retains every stable marker target behind the merged page link.
Clicking a page selects its source marker. Page values come from the same live
Worker/WASM field resolver used by PAGE fields; a page break or reflow therefore
does not create a second pagination model. Marking, editing, inserting,
customizing, and explicitly refreshing every index each create one TipTap
transaction and one Undo record. A document reads at most 2,048 markers and
caches at most 512 generated rows; exceeding either limit produces an explicit
truncated state instead of an unbounded node attribute.
...
`NUMPAGES`, `SECTION`, `SECTIONPAGES`, `DATE`, `TIME`, `NUMWORDS`,
`NUMCHARS`, `FILENAME`, `AUTHOR`, `TITLE`, `SUBJECT`, `KEYWORDS`,
`LASTSAVEDBY`, `COMMENTS`, `CREATEDATE`, `SAVEDATE`, `PRINTDATE`, and
`MERGEFIELD`. Numeric page and section fields resolve from the
measured physical
...
line-break separators. `FILENAME`, `AUTHOR`, `TITLE`, `SUBJECT`, `KEYWORDS`,
`LASTSAVEDBY`, `COMMENTS`, `CREATEDATE`, `SAVEDATE`, and `PRINTDATE` refresh from host-provided
context (or keep imported/cached display when none is supplied); the `FILENAME`
path switch (`\p`) and formatting switches on property fields stay fail-closed.
`CREATEDATE`, `SAVEDATE`, and `PRINTDATE` use the same bounded `\@` date formats as `DATE`.
`MERGEFIELD` uses a named identifier and refreshes from host-provided
`mergeRecord` values, showing `«Name»` when no record is supplied. Hosts can
also pass a typed `mailMergeSource` (`DocumentMailMergeSource`) with ordered
records and an active index; `DocumentEditor` layers the active row onto the
field-context resolver for preview, and helpers expose MERGEFIELD name
discovery plus next/previous stepping. Hosts can also call
`generateMailMergeDocuments` for a bounded batch of DOCX buffers (at most 256
records per call, each row refreshing MERGEFIELD values from that record).
Optional AND `filter` rules on `mailMergeSource` narrow preview and batch to
matching recipients; when hosts also pass `onMailMergeSourceChange`,
`DocumentEditor` opens a recipient-filter dialog from the Insert ribbon.
Unsupported mail-merge switches stay fail-closed.
Refresh and F9
update every eligible field in one controlled transaction, so a reflow never
leaves a stale mixture of values.
...
Selecting a body image opens the contextual Picture ribbon. Fast wrapping,
alignment, 90-degree rotation, and horizontal/vertical reflection commands
remain available, while **Picture Properties** edits width and height in
centimeters, a per-image aspect-ratio lock, wrapping, alignment, text distance,
rotation, reflection, and alternative text in one responsive dialog.
...
The Insert ribbon adds a bounded **Text Box** block. Selecting it opens a
contextual Text Box ribbon with the same predictable controls on desktop and
compact Web: inline or floating layout, width and height in millimeters,
page/column/margin or paragraph-relative offsets, fill and outline, padding,
top/center/bottom vertical alignment, and seven explicit shape presets:
rectangle, rounded rectangle, ellipse, diamond, triangle, parallelogram, and
hexagon. Every accepted
change is one typed document update and one Undo step. The selected shape is
visible in the control label and on the page, remains discoverable after a
compact-ribbon scroll, and survives Undo/Redo. A text box keeps its rich inline
text and line breaks, remains selectable as an isolated block, and is kept
together by the pagination resolver so a box is not split across physical
pages.
...
The bounded connector slice now owns isolated straight, elbow, and curved
connectors as editable content. The contextual Connector ribbon exposes one
typed connector-kind control alongside solid, dash, dot, and dash-dot line
styles, color, width, typed none/triangle/stealth/diamond/oval/open endpoint
arrows, layout, offsets, and endpoint percentages. The kind drives a live SVG
line, routed polyline, or quadratic path, while the same model drives compact
controls, Undo/Redo, WPS VML import, and native DrawingML export/reopen.
...
document templates and review forms. The responsive dialog keeps the control's
intent together: choose plain text, rich text, checkbox, drop-down list, combo
box, or date, give it a display name and program tag, allow multiline text
where appropriate (text controls), toggle the default checked state for
checkboxes, author up to 32 list options and the selected or suggested value
for drop-downs and combo boxes, pick a calendar date and display format for
date controls, choose a border, tag, or hidden appearance, and optionally set a
theme-independent color. Selecting an existing control opens the same dialog
for its metadata, while the control's accessible name follows its alias or tag
(checkboxes use `role="checkbox"`; drop-downs use `role="listbox"`; combo boxes
use `role="combobox"`).

Content and shell locks are enforced by the editor transaction boundary rather
than by styling alone. A content lock rejects typing, paste, and generic
metadata writes; a shell lock prevents replacement or deletion while still
allowing the declared editable content path. Typed commands must explicitly
request an unlock-sensitive deletion or property change. Insertion, property
changes, and deletion each remain one document transaction and one Undo step.
Checkbox controls insert ballot-box glyphs (`☐` / `☑`) and sync the glyph when
the checked state changes. Drop-down and combo-box controls sync displayed text
when a selected or suggested value (or the option list) changes; combo boxes
also keep free-text bodies that do not match a list item. Date controls sync
displayed text when `fullDate` or `dateFormat` changes.
...
rich inline runs, bounded `w14:checkbox` state, bounded `w:dropDownList` and
`w:comboBox` options, bounded `w:date` values (fullDate, dateFormat, lid,
gregorian calendar, storeMappedDataAs), aliases, tags, native lock values,
multiline text, collision-free IDs, Word 2012 appearance and color, and bounded
passive custom-XML `w:dataBinding` metadata (`storeItemID`, `xpath`, optional
`prefixMappings`). Empty text/rich-text controls are preserved as empty editable
spans. Bounded body-level repeating sections (`0.355.0`) admit one
plain-paragraph item with native `w15:repeatingSection` export and fixture
import. Bounded body-level block content controls (`0.357.0`–`0.372.0`) admit one
or more plain paragraphs typed as `text` or `richText`, optional passive
`w:dataBinding` metadata, and up to two nesting levels of leaf block controls.
Live custom-XML part sync, placeholders, add/remove repeating items,
multi-paragraph repeating-section items, nested repeating sections, depth ≥ 3
block nesting, picture and other form controls beyond the checkbox, drop-down,
combo-box, date, repeating, data-binding, multi-paragraph, and two-level nested
block slices, and relationship-bound semantics are not represented by this
model. They remain safe editable text (or a fail-closed import) and receive
