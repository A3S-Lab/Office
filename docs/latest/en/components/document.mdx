...
exact 0.5–1,584 point magnitude. Position and spacing track mixed selections
independently; changing both and choosing Apply still creates one transaction
and one Undo record.
...
highlight, character spacing, baseline position, all-caps and small-caps
effects, and Word grid state. Formatting applied to text that is still a
