...
The seven built-ins are immutable. Up to 32 user sequences are reusable while
the current editor instance remains mounted. To retain them across workbook
changes, full-page reloads, and editor remounts, pass the explicit typed
`sortCustomListStore` property. The editor never writes these preferences into
controlled workbook content. Every accepted sort request carries its normalized
sequence, so execution never depends on mutable dialog state.
...
read, and ignores corrupt or unsupported data. If a write fails, the new list
remains available for the current session and the editor reports the failure.
...
before mutation. One accepted request uses one native range write and creates
one Undo record. Closing or applying either dialog restores the exact ribbon
invoker, after which the grid can continue keyboard navigation.
...
reconciliation, sidecar migration, full custom-list preference management, and
...
invalid input, saved-list reuse after a full-page reload, formula-safe
custom-list row movement, and Undo.
...
to contains, begins-with, or ends-with criteria. Source-level 1904 date-system
retention, large aggregate/rank Worker/WASM offload, and
table/AutoFilter-integrated custom sorting remain explicit boundaries.
