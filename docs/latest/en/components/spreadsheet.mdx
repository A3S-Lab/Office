...
columns. Top-to-bottom mode detects a probable header row, keeps that choice
...
sorts ascending or descending. Finite numbers use numeric order, text uses
natural Simplified Chinese collation, equal records retain their source order,
and blank cells stay at the end in both directions.
...
the current editor instance remains mounted. They are deliberately not written
to the controlled workbook or hidden browser storage; durable cross-editor
preferences require a future explicit host-settings API. Every accepted sort
request carries its normalized sequence, so execution never depends on mutable
dialog state.
...
reconciliation, sidecar migration, durable host-managed custom-list settings,
and large-sort Worker/WASM execution remain explicit follow-up work.
...
invalid input, session reuse, formula-safe custom-list row movement, and Undo.
