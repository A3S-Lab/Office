...
bounded to font colors, solid-fill foreground colors, and visible border-line
colors. It records theme, indexed, or automatic identity, the source palette
color, the rendered color, and a valid tint. That nested metadata survives the
same flat JSON/Yjs cell transport as the rest of the cell.
...
from an unsupported pattern/gradient/differential style, export uses explicit
RGB rather than emitting a false semantic reference.
