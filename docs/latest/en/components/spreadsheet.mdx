...
per-cell run limit. Direct editing accepts one contiguous text replacement per
commit. Formatted rich-text paste, simultaneous disjoint edits, and non-font
run objects remain outside the public Spreadsheet surface.
...
retention. Desktop and compact Playwright exercise the controlled Playground
fixture. `tests/e2e/spreadsheet-rich-text.acl` runs only through the local A3S
Test 1.0.0 gate and captures accessibility plus clean console and page-error
evidence; GitHub Actions and Pages do not install or invoke A3S Test.
