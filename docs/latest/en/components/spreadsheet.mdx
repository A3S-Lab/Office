...
totals authoring, table conversion, or table-part export. It also does not
...
to cell colors. The structured-reference calculation boundary is described
below. Calculated-column fill on row insertion, complete totals-row authoring,
slicers, external/query tables, and advanced table integrations remain
explicit boundaries; their presence must not be interpreted as complete Excel
or Traditional Office table parity.
...
calculated column one cell at a time. A qualified form such as
