...
filter hide. Imported Top/Bottom, percentage, and dynamic criteria remain typed
and are written back, but their visible-row calculation is not authored or
recomputed yet; their existing hidden rows are conservatively retained as
manual. General two-item native `<customFilters and="1|0">` groups preserve
