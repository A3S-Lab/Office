...
Imported dynamic criteria remain typed and are written back, but their
visible-row calculation is not authored or recomputed yet; their existing
hidden rows are conservatively retained as manual. General two-item native
...
to contains, begins-with, or ends-with criteria. Dynamic evaluation, large-rank
Worker/WASM offload, and table/AutoFilter-integrated custom sorting remain
explicit boundaries.

Focused model, command, dialog, Hook, import, and XLSX Rstest cover the bounded
contract. Desktop and compact Playwright exercise the real vendor header menu,
owned dialog, selection retention, apply, and Undo. The local-only
