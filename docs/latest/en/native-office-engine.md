...
Document, Spreadsheet, Presentation, and PDF operations, checkpoint, replay,
leave, and consume a resumable durable event cursor. The CLI provides both a JSONL watch
...
insert a plain paragraph next to a stable identity in a bounded section,
nested list-item, table-cell/header, or blockquote container. Deletion requires
the target's current `textId` and complete text, preserves required container
blocks and a list item's leading paragraph, and rejects inline atoms and review
marks. Table-contained text and structural edits rotate every identified
ancestor row's `rowTextId` atomically. Page color and track-changes mutate
independent `document.options` fields. Raw remote updates remain mode-neutral
for convergence. Authenticated browser actor/operation
origins are retained separately from host delivery identity in durable events
and are re-emitted across the live boundary.

Typed Spreadsheet operations create or recursively patch one zero-based cell
after matching the caller's observed JSON, or delete only after an exact
complete-cell match. They preserve browser-compatible field addressing and
dense/sparse projections. Typed Presentation operations create, optimistically
update, move, or tombstone one scene element in a stable slide, master, or
layout. Canonical creation claims prevent conflicting same-ID reuse; top-level
expected/current/next field comparison merges unrelated concurrent changes;
stable observed and requested predecessor IDs move only that element's order
entry without array indexes; exact deletion removes visible order and
permanently reserves the element ID. A move at its requested destination is
idempotent, while a stale source position or unavailable anchor fails before a
durable update. Neither format mutation replaces a worksheet, workbook,
scene-object record, container, or deck.

Typed PDF operations write the browser-compatible conflict-local overlay
roots without opening or synchronizing source bytes. Form values address a
stable fully-qualified field name. Annotation create accepts portable
EmbedPDF FreeText, Highlight, Underline, StrikeOut, or Ink objects; recursive
optimistic updates merge unrelated JSON leaves; immutable identity cannot
drift; and delete writes a durable tombstone. Append-only redaction and
rotate/delete/reorder page-operation proposals derive actor attribution from
the replica and accept exactly one final review decision. Canonical creation
claims, stable operation receipts, bounded source-page validation, and
fail-closed conflicts apply across Rust, CLI, MCP, and A3S Code.

Document projection schema version 2 also exposes durable selection comments,
replies, resolution/detached state, and exact paragraph-local UTF-16 anchors.
The native `document-comment-create`, `document-comment-reply`,
`document-comment-set-resolved`, and `document-comment-delete` mutations use
the same browser-compatible records, immutable claims, and selection marks.
They accept `edit` or Document `comment`; comment-mode deletion is restricted
to records owned by the replica actor.

The Yrs transport authorization path also validates authenticated Document
`suggest` updates as candidate states before persistence: attributed text
proposals may change, while canonical content and every non-suggestion root
must remain equivalent. Projection v3 exposes each live suggestion's exact
placement and identity plus immutable final decisions. Native
`document-suggestion-create` matches one stable plain paragraph, its current
text identity, exact UTF-16 range, and selected text from that projection; the
authenticated replica actor owns the resulting insertion, deletion, or atomic
replacement proposal. Native `document-suggestion-decide` lets an `edit`
replica accept or reject one or more complete projected identities atomically,
apply the tracked change, and append browser-compatible actor-attributed audit
records. CLI, MCP, and A3S Code use the same closed mutations.

Inspection hashes the sorted state vector and canonical logical root values.
This keeps `documentStateSha256` stable across restart and delivery order even
when Yrs re-encodes JSON objects inside arrays in a different `HashMap` order.
Internal commit detection still hashes the complete raw Yjs state, including
causally pending structs, so an out-of-order update is never discarded as a
logical no-op. Complete checkpoint-plus-log replay canonically integrates
dependencies once they arrive.

Neither native surface opens a network provider, chooses a room, authenticates
a participant, persists Awareness, or polls a whole Office file. The optional
live-session Presence controller uses a separate in-memory Yrs Awareness peer,
emits typed activity and format locations, and accepts remote participant
updates without touching replica checkpoints, logs, or receipts. Nested/list/
table paragraph edits are bounded to existing containers; complete table,
list, and section operations, Spreadsheet structural and remaining
Presentation structural/rich-text mutations, and PDF signatures remain later
milestones.
...
and truncated on UTF-8 boundaries. The explicit `a3s-office` CLI
...
are read-only. The closed default registry implements `ABS`, `AND`,
`AVERAGE`, `AVERAGEIF`, `COLUMN`, `CONCAT`, `CONCATENATE`, `COUNT`, `COUNTA`,
`COUNTIF`, `IF`, `IFERROR`, `MAX`, `MIN`, `MOD`, `NA`, `NOT`, `OR`, `PI`,
`POWER`, `ROUND`, `ROW`, `SEQUENCE`, `SQRT`, `SUBTOTAL`, `SUM`, `SUMIF`, and
`TRANSPOSE`, together with ordinary operators,
...
manual-break, and alignment-point properties, fractions, scripts, left-side
pre-sub/superscripts with empty script slots, radicals with optional degrees,
functions, supported n-ary operators, combining accents, overbars and underbars,
...
property containers preserve one optional ordered `m:ctrlPr` control through
the bounded Word run-property model. It may contain a direct `w:rPr` or bounded
tracked provenance rooted at `w:ins`, `w:del`, `w:moveFrom`, or `w:moveTo`.
Every revision retains a non-negative 32-bit ID, a bounded author, an optional
validated core date, and optional Microsoft 365 `w16du:dateUtc` with a UTC
`Z` suffix. Word's legal
`moveFrom/moveTo -> ins/del` and
`ins -> del` chains retain an optional deepest `w:rPr`. Every supported `deg`,
`den`, `e`, `fName`, `lim`, `num`, `sub`, and `sup` argument slot preserves the
same direct or revision-wrapped control format after its expressions. Empty
direct controls canonicalize away; empty revisions remain native provenance.
Safe object-control values project only onto separable MathML control/operator
nodes. Argument formatting, revision provenance, and document-level move-range
pairing remain native metadata because professional MathML does not represent
...
Delimiters require optional `dPr` before one through 32 `e` arguments and
preserve empty argument slots. Their properties follow
...
Browser-injected screenshots, the explicit `a3s-office mcp`
