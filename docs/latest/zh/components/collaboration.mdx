...
| --- | --- | --- | --- |
| Document | ProseMirror 结构、页面选项、评论、文字、字符格式、段落格式、有序列表编号与纯文字移动修订、最终决定和参考文献 | 文本光标与选区 | 文本替换、段落增删、页面颜色、修订模式、选区评论、署名文字建议与原子文字建议决定；Yrs 还会保护格式、编号和移动修订并读取其审计记录 |
| Markdown | 共享 Markdown 源码 | 源码或可视化编辑器中的光标与选区 | UTF-16 安全的替换与 splice |
| Spreadsheet | 工作表、命名区域、原生表格记录（含已校验的计算列和汇总行规则）与冲突局部化单元格 | 工作表、区域与活动单元格 | 单元格创建、更新与删除；原生文件支持封闭的表格添加、设置和删除，浏览器协同编辑中的表格修改由浏览器绑定执行 |
...
# PDF：先用 collab find / office_collaboration_find 定位表单值或 FreeText 内容；
# 命中返回 fieldId 或 annotationId/pageIndex/annotationType，再用
# pdf-set-form-value / pdf-update-annotation（没有 PDF 正文 replace-text）。
...
| 会话模式 | `edit` 可以修改规范内容、创建或决定文字、字符格式、段落格式、编号与纯文字移动修订并更新审核数据。Document `comment` 可以创建选区评论、回复、解决/重开，并且只能删除当前 Actor 自己的审核记录，正文保持只读。经过认证的 Document `suggest` 可以创建署名插入/删除/替换建议，并且只能撤回自己的建议；不能作出最终决定，也不能修改规范正文、结构、格式、编号或移动修订、选项、评论或他人建议。`view`、非 Document `suggest` 与非 Document `comment` 只能接收。 |
