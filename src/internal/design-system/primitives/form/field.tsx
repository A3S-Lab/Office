...
export function Field({
  ...
  required?: boolean;
  children:
    | ReactElement<FieldControlElementProps>
    | ((props: FieldControlProps) => ReactNode);
  className?: string;
}) {
  ...
  const describedBy =
    [inheritedDescription, descriptionId, errorId].filter(Boolean).join(' ') ||
    undefined;
  ...
  return (
    <div
      className={`ds-field${error ? ' invalid' : ''}${className ? ` ${className}` : ''}`}
    >
