...
import {
  errorBarSourceValues,
  spreadsheetErrorBarAmounts,
} from '../work-spreadsheet-error-bars';
...
export function presentationChartAnalysisBounds(
  ...
  for (const series of chart.series) {
    ...
    for (const source of series.errorBars ?? []) {
      ...
      const amounts = spreadsheetErrorBarAmounts(
        analysisSeries,
        errorBars,
        chart.type,
      );
...
export function drawPresentationChartSeriesAnalysis({
  ...
  for (const source of series.errorBars ?? []) {
    ...
    for (const [pointIndex, value] of values.entries()) {
      ...
      const start = errorBarPosition(
        errorBars.direction,
        pointIndex,
        value - amount.minus,
      );
      const end = errorBarPosition(
        errorBars.direction,
        pointIndex,
        value + amount.plus,
      );
  ...
  for (const [trendlineIndex, source] of (series.trendlines ?? []).entries()) {
    ...
    const points = fit.points.map((point) =>
      trendlinePosition(point.x, point.y),
    );
    ...
    const labels = [
      ...
      trendline.displayRSquared && fit.rSquared !== undefined
        ? `R² = ${roundChartNumber(fit.rSquared)}`
        : undefined,
    ...
    for (const [labelIndex, label] of labels.entries()) {
      context.fillText(
        label,
        labelPoint[0] - 3,
        labelPoint[1] - 6 - (labels.length - labelIndex - 1) * 10,
      );
    }
  }
}
...
function drawErrorBarCap(
  ...
  const start: CanvasPoint = horizontal
    ? [point[0], point[1] - 4]
    : [point[0] - 4, point[1]];
  const end: CanvasPoint = horizontal
    ? [point[0], point[1] + 4]
    : [point[0] + 4, point[1]];
  drawLine(context, start, end, color, 1.3);
}
