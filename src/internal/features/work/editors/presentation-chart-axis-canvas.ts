...
export function drawPresentationCartesianAxes(
  ...
  const valueAxis =
    normalizeWorkSpreadsheetChartGrouping(chart.grouping, chart.type) ===
      'percentStacked' && !sourceValueAxis?.numberFormat
  ...
  const scale = spreadsheetChartAxisScale(values, valueAxis, {
    includeZero: true,
  });
  const valuePosition = horizontalBars
    ? (value: number) =>
        plot.x +
        spreadsheetChartAxisValueRatio(value, scale, valueAxis) * plot.width
    : (value: number) =>
        plot.y +
        (1 - spreadsheetChartAxisValueRatio(value, scale, valueAxis)) *
          plot.height;
  ...
  const categoryPosition = (index: number) => {
    ...
    if (horizontalBars)
      return plot.y + ((visualIndex + 0.5) / count) * plot.height;
    if (pointCategories) {
      return count === 1
        ? plot.x + plot.width / 2
        : plot.x + (visualIndex / (count - 1)) * plot.width;
    }
...
export function drawPresentationXyAxes(
  ...
  const xScale = spreadsheetChartAxisScale(xValues, xAxis, {
    paddingRatio: 0.06,
  });
  const yScale = spreadsheetChartAxisScale(yValues, yAxis, {
    paddingRatio: 0.06,
  });
  const xPosition = (value: number) =>
    plot.x + spreadsheetChartAxisValueRatio(value, xScale, xAxis) * plot.width;
  const yPosition = (value: number) =>
    plot.y +
    (1 - spreadsheetChartAxisValueRatio(value, yScale, yAxis)) * plot.height;
...
export function drawPresentationRadarAxes(
  ...
  const valueRatio = (value: number) =>
    spreadsheetChartAxisValueRatio(value, scale, valueAxis);
...
function presentationAxisPlot(
  ...
  const plot = {
    ...
    width: Math.max(
      18,
      rect.width - Math.min(leftSpace + rightSpace, rect.width * 0.42),
    ),
    height: Math.max(
      18,
      rect.height - Math.min(topSpace + bottomSpace, rect.height * 0.42),
    ),
  };
