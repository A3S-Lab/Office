...
export function SlideChart({
  ...
  const legendPosition = normalizePresentationChartLegendPosition(
  ...
  return (
    <canvas
      ...
      data-presentation-chart-legend-position={
        presentationChartShowsLegend(chart) ? legendPosition : 'none'
      }
      data-presentation-chart-data-labels={String(
        presentationChartHasDataLabels(chart),
      )}
      data-presentation-chart-axes={String(
        Boolean(presentationChartAxes(chart)),
      )}
      data-presentation-chart-trendlines={presentationChartTrendlineCount(
        chart,
      )}
      ...
      data-presentation-chart-legend-overlay={
        presentationChartShowsLegend(chart)
          ? String(
              normalizeWorkSpreadsheetChartLegendOverlay(chart.legendOverlay),
            )
          : undefined
      }
      ...
      data-presentation-chart-custom-series-styles={
        chart.series.filter((series) => series.style).length
      }
      ...
      data-presentation-chart-bubble-scale={
        chart.type === 'bubble'
          ? normalizePresentationBubbleScale(chart.bubbleScale)
          : undefined
      }
...
function drawChart(canvas: HTMLCanvasElement, chart: WorkSlideChart) {
  ...
  const width = Math.max(
    80,
    Math.round(bounds.width || canvas.clientWidth || 320),
  );
  const height = Math.max(
    50,
    Math.round(bounds.height || canvas.clientHeight || 180),
  );
  ...
  const layout = presentationChartCanvasLayout(
    chart,
    content,
...
function drawXyChart(
  ...
  const sizeRepresents = normalizePresentationBubbleSizeRepresents(
    chart.bubbleSizeRepresents,
  );
  const visibleBubbleSizes = chart.series.flatMap((series) =>
    presentationChartBubbleSizes(series).filter(
      (value) => value > 0 || chart.showNegativeBubbles,
    ),
  );
  const maximumBubbleSize = Math.max(
    1,
    ...visibleBubbleSizes.map((value) => Math.abs(value)),
  );
  const maximumBubbleRadius = Math.max(
    4,
    Math.min(plot.width, plot.height) * 0.1 * bubbleScale,
  );
    ...
    if (chart.type === 'bubble') {
      ...
      for (const point of points) {
        ...
        const radius = Math.max(
          2,
          maximumBubbleRadius *
            (sizeRepresents === 'width' ? ratio : Math.sqrt(ratio)),
        );
        fillPresentationChartShape(
          context,
          { ...fill, opacity: fill.opacity * (size < 0 ? 0.32 : 0.58) },
          () => {
            context.beginPath();
    ...
    drawPresentationChartSeriesAnalysis({
      ...
      errorBarPosition: (direction, pointIndex, value) =>
        direction === 'x'
          ...
          : [
              xPosition(xValues[pointIndex] ?? pointIndex + 1),
              yPosition(value),
            ],
    });
    for (const point of points) {
      ...
      drawPresentationChartDataLabel(
        context,
        chart,
        seriesIndex,
        point.sourceIndex,
...
function drawPieChart(
  ...
  const innerRadius =
    chart.type === 'doughnut'
      ? radius * (normalizeDoughnutHoleSize(chart.doughnutHoleSize) / 100)
      : 0;
...
function drawRadarChart(
  ...
  const { point, valueRatio } = drawPresentationRadarAxes(
    context,
    chart,
    sourceRect,
    categoryCount,
