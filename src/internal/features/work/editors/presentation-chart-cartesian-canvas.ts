...
export function drawPresentationCartesianChart(
  ...
  const categoryValues = Array.from(
    { length: layout.categoryCount },
    (_, index) => index + 1,
  );
  const values = layout.stacked
    ? layout.scaleValues
    : presentationChartAnalysisBounds(chart, categoryValues).y;
  const axes = drawPresentationCartesianAxes(
    context,
    chart,
    sourceRect,
    values,
...
function drawLineChart(
  ...
  for (const [seriesIndex, series] of chart.series.entries()) {
    ...
    if (chart.type === 'area') {
      ...
      for (const point of [...points].reverse())
        context.lineTo(point.x, point.startY);
      context.closePath();
      fillPresentationChartShape(
        context,
        presentationChartSeriesFill(series, seriesIndex, 0.2, defaultColor),
        () => context.fill(),
      );
    }
    context.beginPath();
    ...
    strokePresentationChartPath(
      context,
      presentationChartSeriesLine(series, seriesIndex, 2, defaultColor),
    );
  ...
  for (const [seriesIndex, series] of chart.series.entries()) {
    ...
    drawPresentationChartSeriesAnalysis({
      ...
      errorBarPosition: (_direction, pointIndex, value) => [
        categoryPosition(pointIndex),
        valuePosition(value),
      ],
    });
  }
...
function drawBarChart(
  ...
  for (const [seriesIndex, series] of chart.series.entries()) {
    const seriesOffset =
      -groupSize / 2 + geometry.offset(seriesIndex) + geometry.renderedSize / 2;
    ...
    drawPresentationChartSeriesAnalysis({
      ...
      color: presentationChartSeriesLine(
        ...
        PRESENTATION_CHART_COLORS[
          seriesIndex % PRESENTATION_CHART_COLORS.length
        ],
      ).color,
      trendlinePosition: position,
      errorBarPosition: (_direction, pointIndex, value) =>
        position(pointIndex + 1, value),
    });
  }
}
...
function drawPresentationBar(
  ...
  fillPresentationChartShape(context, fill, () =>
