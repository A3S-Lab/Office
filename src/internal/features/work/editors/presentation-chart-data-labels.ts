...
export function drawPresentationChartDataLabel(
  ...
  if (!text) return;
  const position = normalizePresentationChartDataLabelPosition(
...
function placeDataLabel(
  ...
  if (position === 'left')
    return { x: geometry.x - 5, y: geometry.y, align: 'right' };
  if (position === 'right')
    return { x: geometry.x + 5, y: geometry.y, align: 'left' };
  if (position === 'below')
    return { x: geometry.x, y: geometry.y + 9, align: 'center' };
  if (
    position === 'center' ||
    position === 'insideBase' ||
    position === 'insideEnd'
...
function placeVerticalBarLabel(
  geometry: Extract<
    PresentationChartDataLabelGeometry,
    { kind: 'verticalBar' }
  >,
  position: WorkSlideChartDataLabelPosition,
): { x: number; y: number; align: CanvasTextAlign } {
  ...
  if (position === 'center') {
    return {
      x: geometry.x,
      y: (geometry.valueY + geometry.baselineY) / 2,
      align: 'center',
    };
  }
  if (position === 'insideBase') {
    return {
      x: geometry.x,
      y: geometry.baselineY + (positive ? -7 : 7),
      align: 'center',
    };
  }
  if (position === 'insideEnd') {
    return {
      x: geometry.x,
      y: geometry.valueY + (positive ? 7 : -7),
      align: 'center',
    };
  }
  return {
