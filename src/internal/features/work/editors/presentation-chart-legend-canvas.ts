...
export const PRESENTATION_CHART_COLORS = [
  '#4472c4',
  '#ed7d31',
  '#a5a5a5',
  '#ffc000',
...
export function presentationChartCanvasLayout(
  ...
  if (!presentationChartShowsLegend(chart) || !hasLegendItems)
    return { plot: content, legendPosition };
  const gap = 6;
      ...
      return {
        ...
        legend: {
          ...
          y:
            legendPosition === 'top'
              ? content.y
              : content.y + content.height - legendHeight,
          height: legendHeight,
        },
        legendPosition,
      };
    ...
    return {
      ...
      legend: {
        ...
        x:
          legendPosition === 'left'
            ? content.x
            : content.x + content.width - legendWidth,
        width: legendWidth,
      },
      legendPosition,
    };
...
export function presentationChartLegendItems(
  chart: WorkSlideChart,
): PresentationChartLegendItem[] {
  if (chart.type === 'pie' || chart.type === 'doughnut') {
    const itemCount = Math.max(
      chart.categories.length,
      chart.series[0]?.values.length ?? 0,
    );
    const seriesColor = normalizeWorkSpreadsheetChartSeriesStyle(
...
export function drawPresentationChartLegend(
  ...
  if (position === 'top' || position === 'bottom') {
    ...
    for (const [index, item] of items.entries()) {
      ...
      drawLegendItem(
        ...
        x,
        y,
        Math.max(5, Math.min(9, rect.height * 0.3)),
      );
    }
    return;
  }
  const itemHeight = Math.max(
    13,
    Math.min(20, rect.height / Math.max(1, items.length)),
  );
  const startY =
    position === 'topRight'
  ...
  for (const [index, item] of items.entries()) {
    drawLegendItem(
      ...
      Math.max(5, Math.min(9, itemHeight * 0.46)),
    );
  }
}
