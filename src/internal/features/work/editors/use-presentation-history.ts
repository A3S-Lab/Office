...
export function usePresentationHistory({
  ...
  const selectionByContentRef = useRef(
    new WeakMap<WorkPresentationContent, string>(),
  );
  ...
  const applyHistory = useCallback((nextContent: WorkPresentationContent) => {
    ...
    const nextSlideId = resolveHistorySlideSelection(
      currentContent,
      nextContent,
      currentSlideId,
      rememberedSlideId,
...
function resolveHistorySlideSelection(
  ...
  if (
    rememberedSlideId &&
    nextContent.slides.some((slide) => slide.id === rememberedSlideId)
  ) {
    return rememberedSlideId;
  }
  if (nextContent.slides.some((slide) => slide.id === currentSlideId))
  ...
  return (
    nextContent.slides[
      Math.min(currentIndex, Math.max(0, nextContent.slides.length - 1))
    ]?.id ?? ''
  );
}
