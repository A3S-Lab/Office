...
export function workAgentProposalInstruction(
  instruction: string,
...
export function workAgentProposalStatus(
  ...
  if (responses.some((response) => response.pending))
    return { state: 'waiting', phase: 'response' };
  if (!responses.length) return { state: 'waiting', phase: 'response' };
  return {
    state: 'invalid',
    message:
      'AI 助手已返回说明，但没有生成可验证的 A3S Work 差异。你可以调整草稿后重试。',
  };
}
...
function parseProposalResponse(
  ...
  const targetById = new Map(
    request.targets.map((target) => [target.id, target]),
  );
  for (const candidate of proposalJsonCandidates(content)) {
    ...
    if (
      value.protocol !== WORK_AGENT_PROPOSAL_PROTOCOL ||
    ...
    for (const rawChange of value.changes.slice(0, maximumProposalTargets)) {
      ...
      const targetId =
        typeof rawChange.targetId === 'string' ? rawChange.targetId.trim() : '';
      ...
      changes.push({
        ...
        reason:
          typeof rawChange.reason === 'string'
            ? rawChange.reason.trim().slice(0, 2_000)
            : '',
      });
    }
