...
export function insertDocumentCaption(
  editor: Editor,
  ...
  const caption = captionType.create(
    { id, kind, number },
    title.trim() ? editor.schema.text(title.trim()) : undefined,
  );
...
export function insertDocumentCrossReference(
  editor: Editor,
...
function nextCaptionNumber(
  ...
  return (
    editorDocumentCaptionTargets(editor).filter(
...
function selectionInsideCaption(editor: Editor): boolean {
  for (let depth = editor.state.selection.$from.depth; depth > 0; depth -= 1) {
    if (
      editor.state.selection.$from.node(depth).type.name === 'documentCaption'
    )
      return true;
  }
  return false;
