...
export function collectDocumentCaptionTargets(
  source: string,
): WorkDocumentCaptionTarget[] {
  const document = new DOMParser().parseFromString(
    normalizeDocumentCaptionsHtml(source),
    'text/html',
  );
  return Array.from(
    document.body.querySelectorAll<HTMLElement>(CAPTION_SELECTOR),
  ).flatMap((element) => {
...
export function documentCaptionKind(
...
export function documentCaptionDisplay(
  kind: WorkDocumentCaptionKind,
  number: number,
): string {
...
function normalizeReferences(
  document: Document,
  targets: Map<string, WorkDocumentCaptionTarget>,
): void {
...
function documentCaptionTarget(
  element: HTMLElement,
): WorkDocumentCaptionTarget | null {
