...
export function insertDocumentBibliography(
  editor: Editor,
  bibliography: WorkDocumentBibliography,
): boolean {
  const existing = documentBibliographyNodes(editor.state.doc)[0];
  if (existing) {
    refreshDocumentCitations(editor, {
      type: 'document',
      pageSize: 'a4',
      html: editor.getHTML(),
      bibliography,
  ...
  if (
    paragraphType &&
    transaction.selection.$to.parent.type === replacement.type
  ) {
...
export function refreshDocumentCitations(
  editor: Editor,
...
export function renameDocumentCitationTag(
  editor: Editor,
  previousTag: string,
  nextTag: string,
): boolean {
