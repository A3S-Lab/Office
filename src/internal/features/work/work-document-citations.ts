...
const STYLE_DETAILS: Record<
  WorkDocumentCitationStyle,
  { name: string; selectedStyle: string }
> = {
...
export function createDocumentBibliography(
  style: WorkDocumentCitationStyle = 'apa',
): WorkDocumentBibliography {
  return {
    style,
...
export function documentCitationStyle(
...
export function documentCitationStyleDetails(
...
export function documentCitationTagsFromInstruction(
  instruction: string,
): string[] {
  ...
  for (const match of instruction.matchAll(additional))
...
export function documentCitationInstruction(tags: readonly string[]): string {
  ...
  return [
    ...
    ...normalized
      .slice(1)
      .flatMap((tag) => ['\\m', citationTagInstructionValue(tag)]),
    '\\l',
    '2052',
  ].join(' ');
}
  ...
  return instruction.replace(
    ...
    (match, prefix: string, source: string) => {
      ...
      return tag === previousTag
        ? `${prefix}${citationTagInstructionValue(nextTag)}`
        : match;
    },
  );
}
...
export function resolveDocumentCitation(
  ...
  if (!normalized.length)
    return { text: cachedValue || '缺失引文', orphaned: true };
  const sources = new Map(
    (bibliography?.sources ?? []).map(
      (source) => [source.tag, source] as const,
    ),
  );
...
export function normalizeDocumentCitationsHtml(
  ...
  for (const [index, element] of Array.from(
    document.body.querySelectorAll<HTMLElement>(CITATION_SELECTOR),
  ).entries()) {
    ...
    const cached =
      element.dataset.citationDisplay?.trim() ||
    ...
    element.dataset.citationId = uniqueCitationId(
      element.dataset.citationId,
      index + 1,
      usedIds,
    );
    element.dataset.citationTags = normalizedTags.join(' ');
    element.dataset.citationInstruction =
      instruction || documentCitationInstruction(normalizedTags);
...
export function documentBibliographyEntry(
  ...
  const authors =
    ...
    people?.people
      ?.map((person) => bibliographyPersonName(person, bibliography.style))
      .join(', ') ||
    '未知作者';
...
export function primaryCitationContributor(
  ...
  return (
    source.contributors?.Author ?? Object.values(source.contributors ?? {})[0]
  );
}
...
function createBibliographyElement(
  ...
  bibliography.sources.forEach((source, index) => {
    ...
    paragraph.textContent = documentBibliographyEntry(
      source,
      bibliography,
      index,
    );
...
function ieeeCitation(
  sources: WorkDocumentCitationSource[],
  allSources: WorkDocumentCitationSource[],
): string {
    ...
    .map(
      (source) =>
        allSources.findIndex(
          (candidate) =>
            candidate.id === source.id || candidate.tag === source.tag,
        ) + 1,
    )
...
function bibliographyPersonName(
  person: WorkDocumentCitationPerson,
  style: WorkDocumentCitationStyle,
): string {
