...
export function normalizeDocumentColumns(
  columns?: Partial<WorkDocumentColumns>,
): WorkDocumentColumns {
  const customCount = Array.isArray(columns?.custom)
    ? columns.custom.length
    : 0;
  const count = clampInteger(
    columns?.count,
    customCount || DEFAULT_DOCUMENT_COLUMNS.count,
    1,
    MAX_COLUMNS,
  );
  const spacing = clampNumber(
    columns?.spacing,
    DEFAULT_DOCUMENT_COLUMNS.spacing,
    0,
    30,
  );
  ...
  const widths = normalizedPercentages(
    Array.from({ length: count }, (_, index) =>
      finiteNumber(source[index]?.widthPercent, 100 / count),
    ),
    100,
    MIN_COLUMN_PERCENT,
  );
  normalized.custom = Array.from({ length: count }, (_, index) => ({
    ...
    spacing:
      index === count - 1
        ? 0
        : roundOne(
            clampNumber(source[index]?.spacing, normalized.spacing, 0, 30),
          ),
  }));
  return normalized;
}
...
export function parseDocumentColumns(
  ...
  if (source?.trim()) {
    try {
      return normalizeDocumentColumns(
        JSON.parse(source) as Partial<WorkDocumentColumns>,
      );
    } catch {
  ...
  if (
    legacy.count !== undefined ||
    legacy.spacing !== undefined ||
...
export function updateDocumentColumnWidth(
  ...
  const target = Math.min(
    ...
    Math.max(
      MIN_COLUMN_PERCENT,
      finiteNumber(widthPercent, 100 / normalized.count),
    ),
  );
  const otherIndexes = normalized.custom
    .map((_, itemIndex) => itemIndex)
    .filter((itemIndex) => itemIndex !== index);
  const otherWidths = normalizedPercentages(
    otherIndexes.map(
      (itemIndex) => normalized.custom?.[itemIndex].widthPercent ?? 1,
    ),
    100 - target,
    MIN_COLUMN_PERCENT,
  );
  const custom = normalized.custom.map((column, itemIndex) => {
    ...
    return {
      ...column,
      widthPercent:
        itemIndex === index ? roundOne(target) : otherWidths[otherIndex],
    };
  });
  return { ...normalized, custom };
}

export function documentColumnGridTemplate(
  columns: WorkDocumentColumns,
): string {
  ...
  return custom
    .flatMap((column, index) => [
      ...
      ...(index < custom.length - 1
        ? [`${formatNumber(column.spacing)}mm`]
        : []),
    ])
    .join(' ');
}

export function documentUnequalColumnGroups(
  html: string,
  columns: WorkDocumentColumns,
): string[] {
  const custom = normalizeDocumentColumns(columns).custom;
  ...
  for (const [nodeIndex, node] of nodes.entries()) {
    if (
      columnIndex < custom.length - 1 &&
      groups[columnIndex].childNodes.length > 0
    ) {
      const targetPercent = custom
        .slice(0, columnIndex + 1)
        .reduce((total, column) => total + column.widthPercent, 0);
      const targetCost = (totalCost * targetPercent) / 100;
      const currentDistance = Math.abs(targetCost - assignedCost);
      const nextDistance = Math.abs(
        targetCost - assignedCost - costs[nodeIndex],
      );
      const remainingNodes = nodes.length - nodeIndex;
      const remainingColumns = custom.length - columnIndex - 1;
      if (currentDistance <= nextDistance || remainingNodes <= remainingColumns)
        columnIndex += 1;
    }
...
function normalizedPercentages(
  ...
  while (remaining.size) {
    ...
    const belowMinimum = Array.from(remaining).filter(
      (index) =>
        (Math.max(0.001, finiteNumber(values[index], 1)) / sourceTotal) *
          remainingTotal <
        minimum,
    );
...
function blockCost(node: Node): number {
  if (!(node instanceof HTMLElement))
    return Math.max(1, (node.textContent?.trim().length ?? 0) / 80);
  const textCost = Math.max(1, (node.textContent?.trim().length ?? 0) / 100);
  const tableCost =
    node.tagName === 'TABLE'
      ? Math.max(4, node.querySelectorAll('tr').length * 2)
      : 0;
