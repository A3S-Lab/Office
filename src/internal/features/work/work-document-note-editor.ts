...
export function insertDocumentNote(
  editor: Editor,
  ...
  transaction.setSelection(
    TextSelection.near(transaction.doc.resolve(insertPosition + 2)),
  );
...
function nextNoteNumber(editor: Editor, kind: WorkDocumentNoteKind): number {
  ...
  editor.state.doc.descendants((node) => {
    if (node.type.name !== 'documentNoteReference' || node.attrs.kind !== kind)
      return;
...
function selectionInsideNote(editor: Editor): boolean {
  for (let depth = editor.state.selection.$from.depth; depth > 0; depth -= 1) {
    if (editor.state.selection.$from.node(depth).type.name === 'documentNote')
      return true;
  }
  return false;
}
