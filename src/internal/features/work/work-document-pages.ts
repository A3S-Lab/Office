...
export function documentPageDescriptors(
  ...
  sections.forEach((section, sectionIndex) => {
    ...
    const previousBreak =
      sectionIndex > 0 ? sections[sectionIndex - 1].layout.breakAfter : null;
    ...
    if (!continuesCurrentPage) {
      ...
      if (restartsNumbering)
        nextPageNumber = section.layout.pageNumberStart ?? nextPageNumber;
      sectionPage = 1;
  ...
  const result = pages.length
    ...
    : [
        createPage(
          documentSections(content)[0]?.layout,
          content.pageNumberStart ?? 1,
          1,
          1,
          false,
        ),
...
function resolveDocumentPageFields(
  ...
  const sectionNumbers = new Map(
    sections.map((section, index) => [section.id, index + 1] as const),
  );
  const sectionPages = new Map<string, number>();
  for (const page of pages) {
    for (const sectionId of new Set(
      page.segments.map((segment) => segment.sectionId),
    )) {
...
function samePhysicalPageLayout(
  current: WorkDocumentSectionLayout,
...
function attachDocumentNotes(
  ...
  const byKey = new Map(
    safeNotes.map(
      (note) => [documentNoteKey(note.kind, note.id), note] as const,
    ),
  );
  for (const page of pages) {
    const keys = new Set(
      page.segments.flatMap((segment) =>
        documentNoteReferenceKeys(segment.html),
      ),
    );
  ...
  if (lastContentPage)
    lastContentPage.endnotes = safeNotes.filter(
