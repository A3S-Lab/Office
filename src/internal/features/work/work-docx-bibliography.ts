...
const BIBLIOGRAPHY_NAMESPACE =
  'http://schemas.openxmlformats.org/officeDocument/2006/bibliography';
const CUSTOM_XML_NAMESPACE =
...
export async function readDocxBibliography(
  archive: OoxmlPackage,
): Promise<DocxBibliographyReadResult> {
  const paths = archive
    .paths('customXml/')
    .filter((path) => /^customXml\/item(?!Props)[^/]*\.xml$/i.test(path));
  ...
  for (const path of paths) {
    try {
      ...
      sourcePartCount += 1;
      selectedStyle ||=
        attribute(document.documentElement, 'SelectedStyle')?.trim() ?? '';
      styleName ||=
        attribute(document.documentElement, 'StyleName')?.trim() ?? '';
      for (const element of directChildren(
        ...
        if (!COMMON_SOURCE_TYPES.has(source.sourceType))
          uncommonSourceTypes.add(source.sourceType);
        sources.push(source);
      }
    } catch {
  ...
  return {
    bibliography: {
      ...
      selectedStyle:
        selectedStyle || documentCitationStyleDetails(style).selectedStyle,
    ...
    uncommonStyle: styleResult.recognized
      ? undefined
...
export async function patchDocxBibliography(
  ...
  const itemId = stableGuid(
    `bibliography:${bibliography.sources.map((source) => source.tag).join('|')}`,
  );
  archive.file(
    'customXml/item1.xml',
    serializeBibliographySources(bibliography),
  );
  archive.file(
    'customXml/itemProps1.xml',
    serializeCustomXmlProperties(itemId),
  );
  archive.file(
    'customXml/_rels/item1.xml.rels',
    serializeCustomXmlRelationships(),
  );
...
function parseCitationSource(
  ...
  const simple = new Map(
    directChildren(element)
      ...
      .map(
        (child) => [child.localName, child.textContent?.trim() ?? ''] as const,
      ),
  );
  ...
  return {
    ...
    additionalFields: Object.keys(additionalFields).length
      ? additionalFields
      : undefined,
  };
}
...
function parseCitationContributors(
  ...
  const roles =
    directChild(wrapper, 'NameList') || directChild(wrapper, 'Corporate')
      ? [wrapper]
  ...
  for (const role of roles) {
    ...
    const people = nameList
      ? directChildren(nameList, 'Person')
          .map(parseCitationPerson)
          .filter(validCitationPerson)
      : [];
...
function serializeBibliographySources(
  bibliography: WorkDocumentBibliography,
): string {
  ...
  root.setAttribute(
    'SelectedStyle',
    bibliography.selectedStyle || styleDetails.selectedStyle,
  );
  ...
  for (const source of bibliography.sources) {
    ...
    appendTextElement(
      ...
      element,
      'SourceType',
      source.sourceType || 'Misc',
    );
    appendTextElement(
      document,
      element,
      'Guid',
      normalizeGuid(source.guid) ||
        stableGuid(`source:${source.id}:${source.tag}`),
    );
...
function appendContributors(
  ...
  const entries = Object.entries(contributors ?? {}).filter(
    ([, contributor]) =>
      contributor.people?.length || contributor.corporate?.trim(),
  );
...
function serializeCustomXmlProperties(itemId: string): string {
  ...
  const references = document.createElementNS(
...
async function upsertRelationship(
  ...
  const root = document.documentElement;
  const existing = directChildren(root, 'Relationship').find(
    (item) => attribute(item, 'Type') === type,
  );
  if (existing) {
...
function nextRelationshipId(root: Element): string {
  const used = new Set(
    directChildren(root, 'Relationship').map(
