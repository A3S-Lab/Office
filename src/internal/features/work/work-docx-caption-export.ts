...
export function docxCaptionParagraph(
  ...
  const bookmarkId = docxCaptionBookmarkId(
  ...
  return new docx.Paragraph({
    ...
    children: [
      ...
      new docx.Bookmark({
        id: bookmarkId,
        children: [
          new docx.SequentialIdentifier(kind === 'table' ? 'Table' : 'Figure'),
        ],
      }),
      new docx.TextRun('　'),
      ...titleChildren,
...
export function docxCrossReferenceRuns(
  element: HTMLElement,
  docx: typeof import('docx'),
): ParagraphChild[] {
  ...
  return [
    new docx.TextRun(`${documentCaptionLabel(kind)} `),
    new docx.NumberedItemReference(
      docxCaptionBookmarkId(targetId),
      String(number),
      {
        hyperlink: true,
