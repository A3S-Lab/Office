...
export function docxCaptionSequenceKind(
  instruction: string,
): WorkDocumentCaptionKind | null {
  const identifier = /^\s*SEQ\s+([^\s\\]+)/i
    .exec(instruction)?.[1]
    ?.toLowerCase();
  if (!identifier) return null;
  if (identifier === 'table' || identifier === '表') return 'table';
  if (identifier === 'figure' || identifier === 'fig' || identifier === '图')
    return 'figure';
  return null;
}
...
export function docxCaptionBookmark(
  paragraph: Element,
  ...
  const enclosing = bookmarks.filter((bookmark) =>
    bookmarkContains(paragraph, bookmark, field),
  );
  if (enclosing.length) return enclosing.at(-1);
  return (
    bookmarks.filter((bookmark) => precedes(bookmark, field.start)).at(-1) ??
    bookmarks[0]
  );
}

function bookmarkContains(
  paragraph: Element,
  start: Element,
  field: DocxFieldOccurrence,
): boolean {
  ...
  if (!id) return false;
  const end = descendants(paragraph, 'bookmarkEnd').find(
    (bookmark) => attribute(bookmark, 'id') === id,
  );
  return Boolean(
    end && precedes(start, field.start) && precedes(field.end, end),
  );
}
