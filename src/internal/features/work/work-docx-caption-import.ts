...
export function markDocxCaptionFields(
  document: Document,
): ImportedDocxCaptionMarkers {
  ...
  for (const paragraph of paragraphs) {
    ...
    insertParagraphBoundaryMarkers(
      document,
      paragraph,
      marker.start,
      marker.end,
  ...
  for (const paragraph of paragraphs) {
    for (const field of docxFieldOccurrences(paragraph)) {
      ...
      const targetId = bookmarkName
        ? bookmarkTargets.get(bookmarkName)
        : undefined;
...
export function applyImportedDocxCaptionMarkers(
  document: Document,
...
export function hasImportedDocxCaptionMarkers(
  markers: ImportedDocxCaptionMarkers,
): boolean {
...
function uniqueCaptionId(
  ...
  const base = bookmark
    ? `docx-caption-${bookmark}`
    : `docx-${kind}-caption-${index}`;
...
function insertParagraphBoundaryMarkers(
  document: Document,
  paragraph: Element,
  start: string,
  end: string,
...
function markerBlock(
  ...
  while (current && current !== root) {
    if (current.contains(endNode) && /^(?:P|DIV|LI)$/.test(current.tagName))
      return current;
...
function removeCaptionPrefix(
  ...
  if (
    !match ||
    (!source.trimStart().startsWith(label) &&
      !/^(?:Figure|Fig|Table)/i.test(source.trimStart()))
  )
    return;
