...
export async function diagnoseDocxCitations(
  ...
  const bibliographyFields = fields.filter((field) =>
    /^\s*BIBLIOGRAPHY\b/i.test(field.instruction),
  );
  const result = await readDocxBibliography(archive);
  const sourceTags = new Set(
    result.bibliography?.sources.map((source) => source.tag) ?? [],
  );
  const citedTags = Array.from(
    new Set(citationFields.flatMap((field) => field.tags)),
  );
