...
export function docxCitationRun(
  element: HTMLElement,
  docx: typeof import('docx'),
): ParagraphChild {
  const tags = documentCitationTags(element.dataset.citationTags);
  const instruction =
    element.dataset.citationInstruction?.trim() ||
    documentCitationInstruction(tags);
  const display =
...
export function docxBibliographyParagraph(
  ...
  const cached = Array.from(
    element.querySelectorAll<HTMLElement>('[data-bibliography-entry]'),
  )
