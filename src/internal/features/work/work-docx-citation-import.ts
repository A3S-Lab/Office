...
export function markDocxCitationFields(
  document: Document,
): ImportedDocxCitationMarkers {
...
export function applyImportedDocxCitationMarkers(
  document: Document,
  markers: ImportedDocxCitationMarkers,
): void {
...
export function hasImportedDocxCitationMarkers(
  markers: ImportedDocxCitationMarkers,
): boolean {
...
function replaceBibliographyMarkerRange(
  root: HTMLElement,
  marker: ImportedDocxBibliographyMarker,
): boolean {
  const nodes = textNodes(root);
  ...
  section.dataset.bibliographyDisplay =
    marker.display.trim() || convertedDisplay;
  ...
  paragraph.textContent =
    marker.display.trim() || convertedDisplay || '尚无文献源';
