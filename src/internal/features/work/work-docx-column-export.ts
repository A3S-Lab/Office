...
export function docxSectionColumns(
  ...
  const requestedSpaces = columns.custom
    .slice(0, -1)
    .map((column) => millimetersToTwips(column.spacing));
  const maximumSpaceWidth = Math.max(
    0,
    contentWidth - columns.count * MIN_COLUMN_WIDTH_TWIPS,
  );
  const requestedSpaceWidth = requestedSpaces.reduce(
    (total, value) => total + value,
    0,
  );
  const spaceScale =
    requestedSpaceWidth > maximumSpaceWidth
      ? maximumSpaceWidth / requestedSpaceWidth
      : 1;
  const spaces = requestedSpaces.map((space) => Math.round(space * spaceScale));
  const availableWidth =
    contentWidth - spaces.reduce((total, value) => total + value, 0);
  const widths = columns.custom.map((column) =>
    Math.round((availableWidth * column.widthPercent) / 100),
  );
  widths[widths.length - 1] +=
    availableWidth - widths.reduce((total, value) => total + value, 0);
