...
export function importDocxColumns(
  element: Element,
  fallback: WorkDocumentColumns,
): WorkDocumentColumns {
  const columnElements = directChildren(element, 'col');
  const sourceCount =
    numberAttribute(element, 'num') || columnElements.length || fallback.count;
  const count = Math.min(6, Math.max(1, sourceCount));
  const spacingAttribute = attribute(element, 'space');
  const spacing =
    spacingAttribute === null
      ? fallback.spacing
      : twipsToMillimeters(Number(spacingAttribute), fallback.spacing);
  const base = {
    count,
    spacing,
...
function twipsToMillimeters(value: number, fallback: number): number {
  return Number.isFinite(value)
    ? roundOne(Math.max(0, value) / TWIPS_PER_MILLIMETER)
    : fallback;
}
