...
export async function markDocxComments(
  document: Document,
  archive: OoxmlPackage,
): Promise<ImportedDocxCommentMarkers> {
  if (!archive.has('word/comments.xml')) return { comments: [], ranges: [] };
  const definitions = await readCommentDefinitions(archive);
  const bySourceId = new Map(
    definitions.map((definition) => [definition.sourceId, definition] as const),
  );
  ...
  for (const start of starts) {
    ...
    const end = descendants(document, 'commentRangeEnd').find(
      (candidate) =>
        ...
        Boolean(
          start.compareDocumentPosition(candidate) &
    ...
    const workId =
      workIds.get(sourceId) ??
      uniqueWorkCommentId(`docx-comment-${sourceId}`, comments);
    if (!workIds.has(sourceId)) {
      workIds.set(sourceId, workId);
      comments.push(
        toWorkComment(workId, bySourceId.get(sourceId), definitions),
      );
    }
...
export function applyImportedDocxCommentMarkers(
  document: Document,
  markers: ImportedDocxCommentMarkers,
): void {
...
export function hasImportedDocxCommentMarkers(
  markers: ImportedDocxCommentMarkers,
): boolean {
  return markers.ranges.length > 0;
}

async function readCommentDefinitions(
  archive: OoxmlPackage,
): Promise<DocxCommentDefinition[]> {
  ...
  return descendants(document, 'comment').map((comment) => {
    ...
    const paraId =
      attribute(paragraphs.at(-1) ?? comment, 'paraId')?.trim() ?? '';
...
async function readCommentExtended(
  archive: OoxmlPackage,
): Promise<Map<string, DocxCommentExtended>> {
      ...
      .map((comment) => {
        ...
        return [
          paraId,
          {
            ...
            resolved:
              attribute(comment, 'done') === '1' ||
              attribute(comment, 'done') === 'true',
          },
        ] as const;
      })
...
function toWorkComment(
  ...
  const replies = definitions
    .filter(
      (definition) =>
        definition !== root && commentRoot(definition, definitions) === root,
    )
    .map(
...
function commentRoot(
  definition: DocxCommentDefinition,
  definitions: DocxCommentDefinition[],
): DocxCommentDefinition {
  const byParaId = new Map(
    definitions.map((item) => [item.paraId, item] as const),
  );
...
function wrapTextBetweenMarkers(
  ...
  if (
    ...
    (startNode === endNode && endIndex < startIndex + startMarker.length)
  ) {
    return false;
  }
  ...
  if (startNode === endNode) {
    ...
    startNode.data =
      startNode.data.slice(0, startIndex) +
      startNode.data.slice(startIndex + startMarker.length);
    endNode.data =
      endNode.data.slice(0, endIndex) +
      endNode.data.slice(endIndex + endMarker.length);
  }
