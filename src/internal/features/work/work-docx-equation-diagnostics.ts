  ...
  if (total > MAX_EDITABLE_EQUATIONS) {
    issues.push(
      equationIssue(
        ...
        `Only the first ${MAX_EDITABLE_EQUATIONS} valid equations enter the editable structured model; later equations are preserved as atomic native markup when eligible, otherwise flattened to bounded text.`,
      ),
    );
  }
  return issues;
