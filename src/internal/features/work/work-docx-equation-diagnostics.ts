...
export async function diagnoseDocxEquations(
  ...
  if (supported) {
    issues.push(
      equationIssue(
        'docx.equations',
        `${supported} native OMML equation(s) use the editable structured subset. Inline equations and display equations with left, right, center, or center-group paragraph justification, Unicode math runs with literal/normal-text semantics, six math scripts, four styles, manual breaks, alignment points, ordered core Word run formatting for fonts/theme references, emphasis, strike, proofing/grid flags, colors, half-point sizes, underlines, script direction, and languages, and the same bounded Word formatting in ordered object-level control properties and native-only argument-slot control properties, ordered fractions with four native types and a bar default, ordered right-side scripts with aligned sub/superscripts, left-side pre-sub/superscripts with empty script slots, radicals with optional degrees and canonical square-root slots, functions with required, optionally empty name/argument slots, canonical empty arguments across every supported construct, ordered argument/control properties, default-zero argument sizes, and strictly aligned matrix-cell, equation-array-row, and delimiter-argument metadata, ordered n-ary operators with default integrals and canonical empty limit slots, combining accents, overbars and underbars, group characters with explicit character position and baseline justification, phantoms with visible or hidden bases, zeroed width/ascent/descent, and transparent spacing, border boxes with independently visible edges and four strike directions, semantic boxes with operator-emulation, no-break, differential-spacing, manual-break, and alignment properties, bounded rectangular matrices, equation arrays with bounded rows, vertical base alignment, distribution, row-spacing, and alignment/spacer markers, lower and upper limit objects, ordered delimiters with distinct omitted/empty characters and bounded empty argument slots, and their accessible MathML previews round-trip through body text, headers, footers, footnotes, and endnotes.`,
        'info',
      ),
    );
  }
  if (unsupported || spoofed) {
    issues.push(
      equationIssue(
        ...
        `${unsupported + spoofed} equation-like object(s) use unsupported, malformed, misplaced, namespace-spoofed, relationship-bound, or ambiguous markup. They are flattened to bounded text instead of being treated as trusted structured OMML. Invalid or non-combining accent characters, malformed math-run, script, function, argument-property, or math-paragraph structures, unknown, duplicated, reordered, spoofed, or invalid Word math-run, object-control, or argument-control formatting, revision-wrapped control properties in object or argument slots, nondefault argument sizes, invalid or contradictory fraction, radical, n-ary, delimiter, bar, group-character, phantom, border-box, box, or equation-array properties, malformed pre-script or lower/upper limit structures, matrix spacing and gap rules, ragged or over-limit matrices, over-limit equation arrays, and nested Office Math remain outside the structured subset.`,
      ),
    );
  }
