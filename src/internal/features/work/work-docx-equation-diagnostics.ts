...
export async function diagnoseDocxEquations(
  ...
  if (supported) {
    issues.push(
      equationIssue(
        'docx.equations',
        `${supported} native OMML equation(s) use the editable structured subset. Inline equations and display equations with left, right, center, or center-group paragraph justification, Unicode math runs with literal/normal-text semantics, six math scripts, four styles, manual breaks, and alignment points, fractions, ordered right-side scripts with aligned sub/superscripts, left-side pre-sub/superscripts with empty script slots, radicals, functions, supported n-ary operators, combining accents, overbars and underbars, group characters with explicit character position and baseline justification, phantoms with visible or hidden bases, zeroed width/ascent/descent, and transparent spacing, border boxes with independently visible edges and four strike directions, semantic boxes with operator-emulation, no-break, differential-spacing, manual-break, and alignment properties, bounded rectangular matrices, equation arrays with bounded rows, vertical base alignment, distribution, row-spacing, and alignment/spacer markers, lower and upper limit objects, custom delimiters, and their accessible MathML previews round-trip through body text, headers, footers, footnotes, and endnotes.`,
        'info',
      ),
    );
  }
