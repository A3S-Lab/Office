...
export function extractMammothDocumentNotes(
  ...
  for (const reference of references) {
    ...
    if (
      wrapper?.tagName.toLowerCase() === 'sup' &&
...
export function placeMammothDocumentNotes(
  document: Document,
  ...
  for (const note of notes) {
    const reference = Array.from(
      ...
      (element) =>
        element.dataset.noteKind === note.kind &&
        element.dataset.noteId === note.id,
    );
    const target =
      note.kind === 'footnote'
        ? reference?.closest<HTMLElement>('section[data-document-section]')
        : sections.at(-1);
...
function extractDefinitions(document: Document): Map<string, string> {
  ...
  for (const element of elements) {
    ...
    for (const backlink of Array.from(
      clone.querySelectorAll<HTMLAnchorElement>(
