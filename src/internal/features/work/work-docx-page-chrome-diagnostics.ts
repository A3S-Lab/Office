...
export async function diagnoseDocxPageChrome(
  ...
  const instructions = documents.flatMap(({ document }) =>
    docxFieldInstructions(document),
  );
  ...
  if (
    documents.some(
      ({ path, document }) =>
        ...
        docxFieldInstructions(document).some((instruction) =>
          PAGE_FIELD.test(instruction),
        ),
    )
  ) {
    issues.push(
  ...
  if (
    documents.some(({ document }) => descendants(document, 'sdt').length > 0)
  ) {
    issues.push(
      pageChromeIssue(
  ...
  if (
    documents.some(({ document }) =>
      Array.from(document.querySelectorAll('*')).some((element) =>
        ADVANCED_POSITIONING.has(element.localName),
      ),
    )
  ) {
    issues.push(
