      ...
      if (supportedTableFormattingRevisionCount) {
        issues.push(
          issue(
            ...
            `${supportedTableFormattingRevisionCount} relationship-free w:tblPrChange record(s) with a prior alignment, preferred-width, indent, default cell-margin, layout, bidiVisual, solid fill, tblLook, tblOverlap, relationship-free tblStyle, dxa tblCellSpacing, relationship-free tblStyleColBandSize, relationship-free tblStyleRowBandSize, direct-color tblBorders, relationship-free tblCaption, and/or relationship-free tblDescription snapshot remain reviewable in Work as table-formatting changes and round-trip as native w:tblPrChange records.`,
            'info',
          ),
        );
      }
      ...
      if (isolatedParagraphBreakMarkRevisionCount) {
        issues.push(
          issue(
            ...
            `${isolatedParagraphBreakMarkRevisionCount} isolated paragraph-mark revision(s) look like paragraph-break merge or split candidates. Eligible text-only neighbor pairs (including soft breaks, tabs, non-breaking and soft hyphens, relationship-free internal hyperlinks, relationship-free bookmarks, and empty/rPr-only runs) become reviewable paragraph-break changes; others stay fail-closed diagnostics instead of whole-paragraph guesses.`,
          ),
        );
      }
