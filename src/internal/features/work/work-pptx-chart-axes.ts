...
export async function patchPptxChartAxes(
  buffer: ArrayBuffer,
  ...
  for (const [index, chart] of charts.entries()) {
    ...
    if (!plotArea)
      throw new Error(`PPTX chart part is missing its plot area: ${path}`);
    const bottom = findChartAxis(plotArea, 'bottom');
    const left = findChartAxis(plotArea, 'left');
    if (!bottom || !left)
      throw new Error(
        `PPTX chart part is missing an editable primary axis: ${path}`,
      );
...
export function writePptxChartAxis(
  ...
  writeScaling(
    ...
    axisNode,
    axis,
    workSpreadsheetChartAxisIsValueAxis(chart.type, position),
  );
  ...
  replaceAxisValueElement(
    ...
    axisNode,
    'majorTickMark',
    nativeTickMark(axis?.majorTickMark),
    ['minorTickMark', 'tickLblPos', 'spPr', 'txPr', 'crossAx', 'extLst'],
  );
  replaceAxisValueElement(
    document,
    axisNode,
...
function findChartAxis(
  ...
  return directChildren(plotArea).find(
    (child) =>
      (child.localName === 'catAx' ||
        child.localName === 'valAx' ||
        child.localName === 'dateAx') &&
...
function writeScaling(
  ...
  const scaling =
    directChild(axisNode, 'scaling') ??
    createChartElement(document, axisNode, 'scaling');
  if (!scaling.parentElement) {
    const anchor =
      directChildren(axisNode).find((child) => child.localName !== 'axId') ??
      null;
  ...
  appendChartValue(
    ...
    scaling,
    'orientation',
    axis?.reverseOrder ? 'maxMin' : 'minMax',
  );
...
function replaceAxisNumberFormat(
  document: Document,
  axisNode: Element,
  axis: WorkSlideChartAxis | undefined,
): void {
...
function replaceAxisTitle(
  ...
  if (reference) {
    ...
    const endProperties = appendDrawingElement(
      document,
      paragraph,
      'endParaRPr',
    );
...
function insertBeforeNames(
  parent: Element,
  element: Element,
  names: readonly string[],
): void {
  const anchor =
    directChildren(parent).find((child) => names.includes(child.localName)) ??
    null;
