...
export function readPptxChartDataLabels(
  ...
  if (chartLabels && seriesLabels.length) {
    diagnostics.push({
      ...
      message:
        'Per-series data-label overrides were normalized to the chart-level editable settings.',
    });
  } else if (
    !chartLabels &&
    (seriesLabels.length !== series.length ||
      new Set(seriesLabels.map(dataLabelSignature)).size > 1)
  ) {
    diagnostics.push({
      code: 'pptx.chart.data-label-overrides',
      message:
        'Different per-series data-label settings were normalized to one editable chart-level setting.',
    });
  }
  const allLabelNodes = [
  ...
  if (
    allLabelNodes.some((labels) => directChildren(labels, 'dLbl').length > 0)
  ) {
    diagnostics.push({
      code: 'pptx.chart.data-label-overrides',
      message:
        'Per-point data-label overrides were normalized to the chart-level editable settings.',
    });
  }

  const rawPosition = attribute(
    directChild(source, 'dLblPos') ?? source,
    'val',
  );
  const parsedPosition = pptxDataLabelPosition(rawPosition);
  const normalizedPosition = normalizePresentationChartDataLabelPosition(
    parsedPosition,
    chartType,
  );
  if (
    rawPosition &&
    (!parsedPosition || normalizedPosition !== parsedPosition)
  ) {
    diagnostics.push({
  ...
  if (separator && separator.length > 64) {
    diagnostics.push({
      ...
      message:
        'The data-label separator exceeded 64 characters and was truncated.',
    });
  }
  ...
  if (showPercentage && chartType !== 'pie' && chartType !== 'doughnut') {
    diagnostics.push({
      ...
      message:
        'Percentage data labels are supported only for editable pie and doughnut charts.',
    });
  }
  if (showBubbleSize && chartType !== 'bubble') {
    diagnostics.push({
      code: 'pptx.chart.data-label-content',
      message:
        'Bubble-size data labels are supported only for editable bubble charts.',
    });
  }
  if (
    pptxBooleanElement(source, 'showLegendKey') ||
    pptxBooleanElement(source, 'showLeaderLines')
  ) {
    diagnostics.push({
      code: 'pptx.chart.data-label-content',
  ...
  if (
    ['numFmt', 'spPr', 'txPr', 'extLst'].some(
      (name) => directChildren(source, name).length > 0,
    )
  ) {
    diagnostics.push({
      code: 'pptx.chart.data-label-format',
      message:
        'Advanced data-label number and text formatting is normalized by the editable chart model.',
    });
  }
  return {
...
export function writePptxChartDataLabels(
  document: Document,
  chartNode: Element,
  ...
  appendChartElement(document, container, prefix, 'dLblPos', {
    val: pptxDataLabelPositionValue(
      normalizePresentationChartDataLabelPosition(labels.position, chart.type),
    ),
  });
  ...
  if (labels.separator !== undefined) {
    const separator = appendChartElement(
      document,
      container,
      prefix,
      'separator',
  ...
  const anchor =
    directChildren(chartNode).find((child) =>
      DATA_LABEL_ANCHORS.has(child.localName),
    ) ?? null;
...
function pptxDataLabelPosition(
  value: string | null,
): WorkSlideChartDataLabelPosition | undefined {
...
function pptxDataLabelPositionValue(
