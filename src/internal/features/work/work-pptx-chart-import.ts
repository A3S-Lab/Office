...
export function readPptxChart(document: Document): PptxChartImportResult {
  ...
  const chartNode = chartTypes
    .map((name) => firstDescendant(document, name))
    .find(Boolean);
  if (!chartNode) {
    diagnostics.push({
      ...
      feature: 'Charts',
      message:
        'This chart type is shown as a basic column chart and will be normalized on export.',
    });
  }
  const localName = chartNode?.localName;
  const barDirection = attribute(
    firstDescendant(chartNode, 'barDir') ?? document.documentElement,
    'val',
  );
  ...
  const seriesNodes = chartNode
    ? directChildren(chartNode, 'ser')
    : descendants(document, 'ser');
  const formatting = readPptxChartLayoutAndSeriesStyles(
    document,
    chartNode,
    type,
    seriesNodes,
  );
  const xValues = seriesNodes.map((node) =>
    cachedValues(directChild(node, 'xVal')).map(finiteNumber),
  );
  const seriesAnalysis = seriesNodes.map((node) =>
    readPptxChartSeriesAnalysis(node, type),
  );
  const series = seriesNodes.map((node, index) => ({
    name: cachedValues(directChild(node, 'tx'))[0] ?? `Series ${index + 1}`,
    values: cachedValues(directChild(node, numericXAxis ? 'yVal' : 'val')).map(
      finiteNumber,
    ),
    ...(type === 'bubble'
      ? {
          bubbleSizes: cachedValues(directChild(node, 'bubbleSize')).map(
            finiteNumber,
          ),
        }
      : {}),
  ...
  const categories = numericXAxis
    ...
    : cachedValues(
        directChild(seriesNodes[0] ?? document.documentElement, 'cat'),
      );
  if (
    numericXAxis &&
    new Set(xValues.map((values) => values.join('|'))).size > 1
  ) {
    diagnostics.push({
      code: 'pptx.chart.xy-values',
      feature: 'XY chart data',
      message:
        'Different per-series X values were normalized to the first series shared X-value list.',
    });
  }
  const chartRoot = firstDescendant(document, 'chart');
  const legend = directChild(chartRoot ?? document.documentElement, 'legend');
  const title = chartText(
    directChild(chartRoot ?? document.documentElement, 'title'),
  );
  ...
  const chart: WorkSlideChart = {
    ...
    ...(formatting.legendOverlay !== undefined
  ...
  if (type === 'doughnut')
    readDoughnutSettings(chartNode, document, chart, diagnostics);
  if (type === 'radar')
    readRadarSettings(chartNode, document, chart, diagnostics);
...
function readDoughnutSettings(
  ...
  const holeSize = firstDescendant(
    chartNode ?? document.documentElement,
    'holeSize',
  );
  const holeSizeValue = attribute(holeSize ?? document.documentElement, 'val');
  const rawHoleSize =
    holeSizeValue === null ? undefined : Number(holeSizeValue);
  chart.doughnutHoleSize = normalizeDoughnutHoleSize(rawHoleSize);
  if (
    holeSize &&
    (rawHoleSize === undefined ||
      !Number.isFinite(rawHoleSize) ||
      rawHoleSize < 10 ||
      rawHoleSize > 90)
  ) {
    diagnostics.push({
      ...
      feature: 'Chart formatting',
      message:
        'The doughnut hole size was outside the editable 10–90% range and was normalized.',
    });
  }
}
...
function readRadarSettings(
  ...
  const radarStyle = attribute(
    firstDescendant(chartNode ?? document.documentElement, 'radarStyle') ??
      chartNode ??
...
function readScatterSettings(
  ...
  if (!chartNode) return;
  const rawStyle = attribute(
    directChild(chartNode, 'scatterStyle') ?? chartNode,
    'val',
  );
...
function readBubbleSettings(
  ...
  const rawScale = scaleElement
    ? Number(attribute(scaleElement, 'val'))
    : undefined;
  ...
  if (
    ...
    diagnostics.push({
      ...
      feature: 'Chart formatting',
      message:
        'The bubble scale was outside the editable 5–300% range and was normalized.',
    });
  }
  chart.showNegativeBubbles = pptxBooleanElement(source, 'showNegBubbles');
  const rawSizeRepresents = attribute(
    directChild(source, 'sizeRepresents') ?? source,
    'val',
  );
  ...
  if (
    descendants(source, 'bubble3D').some((element) => pptxBooleanValue(element))
  ) {
    diagnostics.push({
...
function cachedValues(parent: Element | undefined): string[] {
  ...
  if (multiLevelCache) {
    const levels = directChildren(multiLevelCache, 'lvl').map((level) =>
      cachedPoints(level),
    );
...
function cachedPoints(cache: Element): string[] {
  return descendants(cache, 'pt')
    .sort(
      (left, right) =>
        numberAttribute(left, 'idx', 0) - numberAttribute(right, 'idx', 0),
    )
...
function pptxBooleanValue(element: Element): boolean {
  ...
  return (
    value === null || value === '1' || value.toLocaleLowerCase() === 'true'
  );
}
