...
export function readPptxChartLayoutAndSeriesStyles(
  ...
  if (legend && !isSupportedXlsxChartLegend(document)) {
    diagnostics.push({
      ...
      message:
        'Manual legend layout, entry overrides, or advanced legend formatting was normalized.',
    });
  }

  const hasLayout = Boolean(
    chartNode && chartHasEditableLayoutElements(chartNode),
  );
  const layout =
    chartNode && hasLayout
      ? parseXlsxChartPlotLayout(chartNode, chartType)
      : {};
  if (chartNode && hasLayout && !isSupportedXlsxChartPlotLayout(chartNode)) {
    diagnostics.push({
      code: 'pptx.chart.format.layout',
      message:
        'Invalid grouping, spacing, overlap, or mixed smoothing settings were normalized.',
    });
  }

  const seriesStyles = seriesNodes.map((series) =>
    parseXlsxChartSeriesStyle(series),
  );
  if (
    chartNode &&
    seriesNodes.some(
      (series) =>
        !isSupportedXlsxChartSeriesFormatting(series, chartNode.localName) ||
        directChildren(series, 'dPt').length > 0,
    )
  ) {
    diagnostics.push({
  ...
  if (
    ...
    seriesNodes.some(
      (series) =>
        directChild(series, 'trendline') || directChild(series, 'errBars'),
    )
  ) {
    diagnostics.push({
      code: 'pptx.chart.format.series-analysis',
      message:
        'Trendlines and error bars were removed because stacked chart layouts do not support series analysis.',
    });
  }
  ...
  return {
    ...(legend
      ? {
          legendOverlay: normalizeWorkSpreadsheetChartLegendOverlay(
            parsedLegend.legendOverlay,
          ),
        }
      : {}),
    layout,
...
export function writePptxChartLayoutAndSeriesStyles(
  ...
  for (const [seriesIndex, seriesNode] of seriesNodes.entries()) {
    writeSeriesStyle(
      document,
      seriesNode,
      chart.series[seriesIndex].style,
      chart.type,
    );
  }
}
...
function writeLegendOverlay(document: Document, chart: WorkSlideChart): void {
  ...
  const anchor =
    directChildren(legend).find((child) =>
      ['spPr', 'txPr', 'extLst'].includes(child.localName),
    ) ?? null;
  insertChartSetting(
    document,
    legend,
...
function writePlotLayout(
  ...
  if (workSpreadsheetChartSupportsGrouping(chart.type)) {
    replaceChartSetting(
      ...
      directChildren(chartNode).find((child) =>
        ['varyColors', 'ser'].includes(child.localName),
      ) ?? null,
    );
  }
  if (workSpreadsheetChartSupportsBarSpacing(chart.type)) {
    ...
    const anchor =
      directChildren(chartNode).find((child) =>
        ['axId', 'extLst'].includes(child.localName),
      ) ?? null;
    insertChartSetting(
      document,
      chartNode,
...
function writeSeriesStyle(
  ...
  if (shapeXml) {
    ...
    seriesNode.insertBefore(
      chartFragmentElement(document, seriesNode, shapeXml),
      anchor ?? null,
    );
  }
  ...
  seriesNode.insertBefore(
    chartFragmentElement(document, seriesNode, markerXml),
    anchor ?? null,
  );
}

function chartFragmentElement(
  document: Document,
  parent: Element,
  xml: string,
): Element {
  ...
  if (!element)
    throw new Error(
      'PPTX chart formatting fragment did not contain an element.',
    );
...
function chartHasEditableLayoutElements(chartNode: Element): boolean {
  return (
    ['grouping', 'gapWidth', 'overlap'].some(
      (name) => directChildren(chartNode, name).length > 0,
    ) ||
    (chartNode.localName === 'lineChart' &&
      (directChildren(chartNode, 'smooth').length > 0 ||
        directChildren(chartNode, 'ser').some(
          (series) => directChildren(series, 'smooth').length > 0,
        )))
  );
}
