...
export function readPptxChartSeriesAnalysis(
  ...
  if (
    (!workSpreadsheetChartSupportsTrendlines(chartType) &&
      trendlineNodes.length) ||
    ...
    ? trendlineNodes.flatMap((node) => {
        ...
        if (!trendline) {
          diagnostics.push({
            ...
            message:
              'An unsupported or incomplete trendline was omitted from the editable chart.',
          });
          return [];
        }
    ...
    ? errorBarNodes.flatMap((node) => {
        ...
        if (!errorBar) {
          diagnostics.push({
            ...
            message:
              'Unsupported or incomplete error-bar settings were omitted from the editable chart.',
          });
          return [];
        }
...
export async function patchPptxChartSeriesAnalysis(
  ...
  const charts = slides.flatMap((slide) =>
    slide.elements.flatMap((element) =>
      element.type === 'chart' && element.chart ? [element.chart] : [],
    ),
  );
  ...
  for (const [index, chart] of charts.entries()) {
    ...
    if (!entry)
      throw new Error(`PPTX chart export is missing chart part: ${path}`);
    const document = parseXml(await entry.async('text'), path);
    const chartNode = CHART_TYPES.map((name) =>
      firstDescendant(document, name),
    ).find(Boolean);
    if (!chartNode)
      throw new Error(
        `PPTX chart part is missing a supported chart node: ${path}`,
      );
...
export function writePptxChartSeriesAnalysis(
  document: Document,
  chartNode: Element,
  ...
  for (const [seriesIndex, seriesNode] of seriesNodes.entries()) {
    ...
    if (
      ...
      for (const source of series.trendlines ?? []) {
        seriesNode.insertBefore(
          createTrendlineElement(document, seriesNode, source),
          anchor,
        );
      }
    }
    if (
      ...
      for (const source of series.errorBars ?? []) {
        ...
        seriesNode.insertBefore(
          createErrorBarsElement(document, seriesNode, errorBars),
          anchor,
        );
      }
    }
...
function readTrendlineDiagnostics(
  node: Element,
  diagnostics: PptxChartSeriesAnalysisDiagnostic[],
): void {
  ...
  if (
    ['spPr', 'extLst'].some((name) => directChildren(node, name).length > 0)
  ) {
    diagnostics.push({
      code: 'pptx.chart.trendline-format',
      message:
        'Advanced trendline styling and extensions are normalized by the editable chart model.',
    });
  }
  const type = attribute(directChild(node, 'trendlineType') ?? node, 'val');
  const parameter =
    type === 'poly'
      ? numericSetting(node, 'order')
      : type === 'movingAvg'
        ? numericSetting(node, 'period')
        : undefined;
  ...
  const forecastInvalid = ['forward', 'backward'].some((name) => {
    ...
    return (
      directChild(node, name) !== undefined &&
  ...
  if (parameterInvalid || forecastInvalid) {
    diagnostics.push({
      ...
      message:
        'Trendline order, period, or forecast values were normalized to the editable range.',
    });
  }
}
...
function readErrorBarDiagnostics(
  ...
  if (
    ...
    diagnostics.push({
      ...
      message:
        'Advanced error-bar styling and extensions are normalized by the editable chart model.',
    });
  }
  ...
  if (incompatibleCustomSource || invalidValue) {
    diagnostics.push({
      ...
      message:
        'Error-bar values or custom sources were normalized to the editable direction and non-negative range.',
    });
  }
}

function createTrendlineElement(
  document: Document,
  parent: Element,
  ...
  if (trendline.name)
    appendTextElement(document, container, 'name', trendline.name);
  appendSetting(
    document,
    container,
    'trendlineType',
    trendlineTypeValue(trendline.type),
  );
  if (trendline.type === 'polynomial')
    appendSetting(document, container, 'order', trendline.order ?? 2);
  if (trendline.type === 'movingAverage')
    appendSetting(document, container, 'period', trendline.period ?? 2);
  if (trendline.forward)
    appendSetting(document, container, 'forward', trendline.forward);
  if (trendline.backward)
    appendSetting(document, container, 'backward', trendline.backward);
  if (trendline.intercept !== undefined)
    appendSetting(document, container, 'intercept', trendline.intercept);
  if (trendline.displayRSquared)
    appendSetting(document, container, 'dispRSq', 1);
  if (trendline.displayEquation)
    appendSetting(document, container, 'dispEq', 1);
  return container;
}

function createErrorBarsElement(
  document: Document,
  parent: Element,
  errorBars: WorkSpreadsheetErrorBars,
): Element {
  ...
  appendSetting(
    ...
    container,
    'errValType',
    errorBarValueType(errorBars.valueType),
  );
  if (errorBars.showEndCaps === false)
    appendSetting(document, container, 'noEndCap', 1);
  if (errorBars.valueType === 'custom') {
    if (errorBars.barType !== 'minus') {
      appendCustomErrorSource(
        document,
        container,
        'plus',
    ...
    appendSetting(
      ...
      container,
      'val',
      errorBars.value ?? (errorBars.valueType === 'percentage' ? 5 : 1),
    );
  }
  return container;
}
...
function appendCustomErrorSource(
  ...
  if (reference?.trim()) {
    ...
    if (values?.length)
      appendNumberCollection(document, numberReference, 'numCache', values);
    return;
  }
...
function errorBarValueType(
  type: WorkSpreadsheetErrorBars['valueType'],
): string {
...
function chartHasSeriesAnalysis(chart: WorkSlideChart): boolean {
  return (
    workSpreadsheetChartSupportsSeriesAnalysis(chart) &&
    chart.series.some(
      (series) => series.trendlines?.length || series.errorBars?.length,
    )
  );
}
