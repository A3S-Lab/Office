...
export async function patchPptxChartXySettings(
  ...
  if (
    !charts.some((chart) => chart.type === 'scatter' || chart.type === 'bubble')
  )
    return buffer;
  ...
  for (const [index, chart] of charts.entries()) {
    ...
    const chartNode = firstDescendant(
      document,
      chart.type === 'scatter' ? 'scatterChart' : 'bubbleChart',
    );
    if (!chartNode)
      throw new Error(
        `PPTX chart part does not contain the expected ${chart.type} chart: ${path}`,
      );
...
export function writePptxChartXySettings(
  document: Document,
  chartNode: Element,
  ...
  if (chart.type === 'scatter') {
    replaceChartSetting(
      ...
      directChildren(chartNode).find(
        (child) => child.localName !== 'scatterStyle',
      ) ?? null,
    );
    return;
  }
  ...
  const anchor =
    directChildren(chartNode).find((child) => child.localName === 'axId') ??
    null;
  for (const name of [
    'bubble3D',
    'bubbleScale',
  ...
  insertChartSetting(
    ...
    chartNode,
    'sizeRepresents',
    normalizePresentationBubbleSizeRepresents(chart.bubbleSizeRepresents) ===
      'width'
      ? 'w'
      : 'area',
    anchor,
