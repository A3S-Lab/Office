...
export async function loadPptxCommentAuthors(
  ...
  const relationship = Array.from(presentationRelationships.values()).find(
    (item) => item.type.endsWith('/commentAuthors'),
  );
...
export async function readPptxSlideComments(
  ...
  const commentRelationships = Array.from(relationships.values()).filter(
    (item) => item.type.endsWith('/comments'),
  );
  for (const relationship of commentRelationships) {
    ...
    if (
      root.namespaceURI !== PRESENTATION_NAMESPACE ||
      root.localName !== 'cmLst'
    ) {
    ...
    for (const [index, element] of directChildren(root, 'cm').entries()) {
      ...
      const sourceIndex =
        attribute(element, 'idx')?.trim() || String(index + 1);
      ...
      result.comments.push({
        ...
        x: coordinatePercent(
          attribute(position ?? element, 'x'),
          slideWidthEmu,
        ),
        y: coordinatePercent(
          attribute(position ?? element, 'y'),
          slideHeightEmu,
        ),
      });
    }
...
function collectExportAuthors(
  slides: readonly WorkSlide[],
): Map<string, ExportCommentAuthor> {
...
function serializeSlideComments(
  ...
  for (const comment of comments) {
    ...
    position.setAttribute(
      'x',
      String(percentCoordinate(comment.x, slideWidthEmu)),
    );
    position.setAttribute(
      'y',
      String(percentCoordinate(comment.y, slideHeightEmu)),
    );
...
function serializeCommentAuthors(
  authors: Map<string, ExportCommentAuthor>,
): string {
...
async function upsertContentTypes(
  ...
  for (const part of commentParts)
    upsertContentType(document, `/${part}`, COMMENTS_CONTENT_TYPE);
  archive.file(path, new XMLSerializer().serializeToString(document));
}

function upsertContentType(
  document: Document,
  partName: string,
  contentType: string,
): void {
...
function percentCoordinate(value: number, size: number): number {
  return Math.round(
    (clamp(Number.isFinite(value) ? value : 50, 0, 100) / 100) * size,
  );
}
