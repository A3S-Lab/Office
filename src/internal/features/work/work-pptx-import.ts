...
export async function importPptxPresentation(
  ...
  const presentationRelationships = await archive.relationships(
    'ppt/presentation.xml',
  );
  const size = firstDescendant(presentation, 'sldSz');
  const slideWidthEmu =
    positiveNumber(attribute(size ?? presentation.documentElement, 'cx')) ??
    12_192_000;
  const slideHeightEmu =
    positiveNumber(attribute(size ?? presentation.documentElement, 'cy')) ??
    6_858_000;
  const theme = await loadPptxTheme(archive, presentationRelationships);
  const commentAuthors = await loadPptxCommentAuthors(
    archive,
  ...
  if (!slideParts.length)
    throw new Error('The PPTX presentation does not contain any slides.');
...
async function parseSlide(
  ...
  const inheritance = await loadPptxSlideInheritance(
    archive,
    relationships,
    theme,
  );
  ...
  const resolvedElements = inheritPptxPlaceholderStyles(
    elements,
    design.master,
    design.layout,
  );
  ...
  for (const relationship of relationships.values()) {
    if (
      ...
      relationship.type.endsWith('/oleObject') ||
      relationship.type.endsWith('/package')
    ) {
      addIssue(
        context,
        'pptx.ole',
  ...
  return {
    ...
    background:
      slideBackground ??
      inheritance.layout?.background ??
      inheritance.master?.background ??
      '#ffffff',
...
async function loadPptxPresentationDesigns(
  ...
  for (const relationship of relationships.values()) {
    ...
    const part = await loadPptxDesignPart(
      archive,
      relationship.target,
      theme,
      'Slide master',
...
async function ensurePptxDesignDefinitions(
  ...
  const layout =
    layoutPart && master
      ? await ensurePptxLayout(layoutPart, master.id, context, registry)
      : undefined;
...
async function ensurePptxMaster(
  ...
  for (const relationship of part.relationships.values()) {
    ...
    const layoutPart = await loadPptxDesignPart(
      archive,
      relationship.target,
      context.theme,
      'Slide layout',
...
function ensureFallbackPptxMaster(
  registry: PptxDesignRegistry,
): WorkPresentationMaster {
...
async function parsePptxDesignElements(
  part: PptxDesignPart,
  context: PptxImportContext,
...
function inheritPptxPlaceholderStyles(
  ...
  const addDefinitions = (candidates: readonly WorkSlideElement[]) => {
    for (const candidate of candidates) {
      ...
      const inheritedDefinition = placeholder.inheritsStyle
        ? (definitions.get(placeholder.key) ??
          definitions.get(`type:${placeholder.type}`))
        : undefined;
      const definition = inheritedDefinition
        ? inheritPptxPlaceholderStyle(candidate, inheritedDefinition)
        : candidate;
  ...
  if (layout?.showMasterElements !== false)
    addDefinitions(master?.elements ?? []);
  ...
  return elements.map((element) => {
    ...
    const definition =
      ...
      definitions.get(`type:${placeholder.type}`);
    return definition
      ? inheritPptxPlaceholderStyle(element, definition)
      : element;
  });
}

function inheritPptxPlaceholderStyle(
...
function parseShape(
  ...
  const fill = readPptxFill(
    properties,
    context.theme,
    text.text ? 'transparent' : '#dce6fb',
  );
  const geometry =
    attribute(firstDescendant(properties, 'prstGeom') ?? node, 'prst') ??
    'rect';
  ...
  const border = readPptxFill(
    line,
    context.theme,
    fill.color === 'transparent' ? '#657087' : fill.color,
  );
  const isTextBox =
    attribute(firstDescendant(node, 'cNvSpPr') ?? node, 'txBox') === '1';
  const isTextPlaceholder =
    placeholder &&
    !['pic', 'chart', 'tbl', 'media', 'obj'].includes(placeholder.type);
...
async function parsePicture(
  ...
  const relationshipId =
    attribute(properties ?? node, 'r:embed') ??
    attribute(properties ?? node, 'embed');
  ...
  if (
    ...
    !context.archive.has(relationship.target)
  ) {
    addIssue(
      context,
      'pptx.image.linked',
      'Linked pictures',
      `The linked picture “${name}” could not be embedded.`,
    );
...
async function parseGraphicFrame(
  ...
  if (table) {
    ...
    if (
      descendants(table, 'tc').some((cell) =>
        ['gridSpan', 'rowSpan', 'hMerge', 'vMerge'].some((name) =>
          attribute(cell, name),
        ),
      )
    ) {
  ...
  const relationshipId =
    attribute(chartReference ?? node, 'r:id') ??
    attribute(chartReference ?? node, 'id');
  const relationship = relationshipId
    ? context.relationships.get(relationshipId)
    : undefined;
  if (
    ...
    const result = readPptxChart(
      await context.archive.xml(relationship.target),
    );
  ...
  const transform: GroupTransform = {
    map: (box) => {
      const mapped = {
        ...
        x:
          group.x +
          ((box.x - viewport.x) * group.width) / Math.max(1, viewport.width),
        y:
          group.y +
          ((box.y - viewport.y) * group.height) / Math.max(1, viewport.height),
...
function parseText(
  ...
  for (const [paragraphIndex, paragraph] of paragraphs.entries()) {
    ...
    if (
      paragraphProperties &&
      (paragraphProperties.attributes.length ||
        paragraphProperties.children.length)
    ) {
      explicitStyle = true;
    }
    if (paragraphIndex === 0)
      align = pptxTextAlignment(
        attribute(paragraphProperties ?? paragraph, 'algn'),
      );
    const bullet = directChild(paragraphProperties ?? paragraph, 'buChar');
    const autoNumber = directChild(
    ...
    for (const child of directChildren(paragraph)) {
      ...
      const runProperties =
        ...
        directChild(paragraphProperties ?? paragraph, 'defRPr');
      if (runProperties) explicitStyle = true;
      const color = runProperties
        ? readPptxFill(runProperties, context.theme, '#172033').color
        : undefined;
      const hyperlink = firstDescendant(runProperties, 'hlinkClick');
      const relationshipId =
        attribute(hyperlink ?? child, 'r:id') ??
        attribute(hyperlink ?? child, 'id');
      const relationship = relationshipId
        ? context.relationships.get(relationshipId)
        : undefined;
      runs.push({
        ...
        italic: runProperties
          ? booleanAttribute(runProperties, 'i')
          : undefined,
        ...
        fontFamily: runProperties
          ? (attribute(
              directChild(runProperties, 'latin') ?? runProperties,
              'typeface',
            ) ??
            attribute(
              directChild(runProperties, 'ea') ?? runProperties,
              'typeface',
            ) ??
            undefined)
          : undefined,
  ...
  return {
    ...
    runs,
    align,
    verticalAlign: pptxVerticalAlignment(
      bodyProperties ? attribute(bodyProperties, 'anchor') : null,
    ),
...
async function readSpeakerNotes(
  ...
  const relationship = Array.from(relationships.values()).find((item) =>
  ...
  const bodies = descendants(document, 'sp')
    .filter(
      (shape) =>
        attribute(
          firstDescendant(directChild(shape, 'nvSpPr'), 'ph') ?? shape,
          'type',
        ) === 'body',
    )
    .map((shape) =>
      parseText(firstDescendant(shape, 'txBody'), context).text.trim(),
    )
    .filter(Boolean);
  return bodies.join('\n\n') || undefined;
}

function orderedSlideParts(
...
function readAlternativeText(node: Element): string | undefined {
  ...
  return (
    attribute(properties ?? node, 'descr')?.trim() ||
    attribute(properties ?? node, 'title')?.trim() ||
    undefined
  );
}
