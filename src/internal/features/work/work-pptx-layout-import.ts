...
export async function loadPptxSlideInheritance(
  ...
  const layoutRelationship = Array.from(slideRelationships.values()).find(
    (item) => item.type.endsWith('/slideLayout'),
  );
  const layout = layoutRelationship
    ? await loadPptxDesignPart(
        archive,
  ...
  const masterRelationship = layout
    ? Array.from(layout.relationships.values()).find((item) =>
...
export function pptxPlaceholderBox(
  node: Element,
  placeholders: Map<string, PptxRawBox>,
): PptxRawBox | undefined {
  const placeholder = pptxPlaceholder(node);
  if (!placeholder) return undefined;
  return (
    placeholders.get(placeholder.key) ??
    placeholders.get(`type:${placeholder.type}`)
  );
}

export function pptxPlaceholder(
  node: Element,
): WorkSlidePlaceholder | undefined {
...
export async function loadPptxDesignPart(
  ...
  return {
    ...
    name:
      attribute(commonSlideData ?? document.documentElement, 'name')?.trim() ||
      `${fallbackName} ${partNumber(path)}`,
    background: readPptxBackground(document, theme),
    showMasterElements: pptxShowsMasterElements(document),
    sourceType:
      attribute(document.documentElement, 'type')?.trim() || undefined,
  };
}
