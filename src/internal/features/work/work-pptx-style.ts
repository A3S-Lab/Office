...
export async function loadPptxTheme(
  ...
  const masterRelationship = Array.from(
    presentationRelationships.values(),
  ).find((item) => item.type.endsWith('/slideMaster'));
  if (!masterRelationship || !archive.has(masterRelationship.target))
    return defaults;
  const masterRelationships = await archive.relationships(
    masterRelationship.target,
  );
  const themeRelationship = Array.from(masterRelationships.values()).find(
    (item) => item.type.endsWith('/theme'),
  );
  if (!themeRelationship || !archive.has(themeRelationship.target))
    return defaults;
...
export function readPptxBackground(
  document: Document,
...
export function supportedPptxShapeType(
  value: string,
): WorkSlideShapeType | undefined {
...
export function pptxVerticalAlignment(
  value: string | null,
): WorkSlideVerticalAlign {
...
function drawingColor(
  ...
  theme: PptxThemeColors,
): PptxFillValue | undefined {
  const colorNames = ['srgbClr', 'scrgbClr', 'sysClr', 'schemeClr', 'prstClr'];
  const colorNode =
    directChildren(container).find((node) =>
      colorNames.includes(node.localName),
    ) ??
  ...
  if (colorNode.localName === 'srgbClr')
    color = normalizeColor(attribute(colorNode, 'val'));
  else if (colorNode.localName === 'sysClr')
    color = normalizeColor(attribute(colorNode, 'lastClr'));
  else if (colorNode.localName === 'schemeClr')
    color = theme[attribute(colorNode, 'val') ?? ''];
  else if (colorNode.localName === 'prstClr')
    color = presetColor(attribute(colorNode, 'val'));
  else {
  ...
  const tint =
    numberAttribute(firstDescendant(colorNode, 'tint'), 'val', 0) / 100_000;
  const shadeNode = firstDescendant(colorNode, 'shade');
  const shade = shadeNode
    ? numberAttribute(shadeNode, 'val', 100_000) / 100_000
    : 1;
  ...
  return {
    color,
    opacity: alphaNode
      ? numberAttribute(alphaNode, 'val', 100_000) / 100_000
      : undefined,
  };
}
