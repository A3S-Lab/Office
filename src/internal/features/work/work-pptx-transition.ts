...
export function readPptxTransition(
  document: Document,
): PptxTransitionReadResult {
  ...
  const effect = directChildren(element).find(
  ...
  const base = createWorkSlideTransition(
    effect.localName as WorkSlideTransition['type'],
  );
  ...
  if (transition.type === 'push' || transition.type === 'wipe') {
    transition.direction =
      CARDINAL_DIRECTIONS[attribute(effect, 'dir') ?? ''] ?? 'left';
  }
  if (transition.type === 'split') {
    transition.direction = attribute(effect, 'dir') === 'in' ? 'in' : 'out';
    transition.orientation =
      attribute(effect, 'orient') === 'vert' ? 'vertical' : 'horizontal';
  }
  if (readBoolean(attribute(effect, 'thruBlk'), false)) {
    diagnostics.push({
      code: 'pptx.transition.through-black',
      message:
        'The through-black transition option is normalized to the basic effect.',
    });
  }
  return { present: true, transition, diagnostics };
}

export async function patchPptxTransitions(
  buffer: ArrayBuffer,
  slides: WorkSlide[],
): Promise<ArrayBuffer> {
...
export function writePptxTransition(
  document: Document,
  ...
  element.setAttribute(
    'spd',
    transition.speed === 'medium' ? 'med' : transition.speed,
  );
  element.setAttribute('advClick', transition.advanceOnClick ? '1' : '0');
  if (transition.advanceAfterMs !== undefined) {
    element.setAttribute(
      'advTm',
      String(Math.max(0, Math.trunc(transition.advanceAfterMs))),
    );
  }
  const effect = document.createElementNS(
    namespace,
  ...
  if (transition.type === 'push' || transition.type === 'wipe') {
    effect.setAttribute(
      'dir',
      OOXML_DIRECTIONS[transition.direction ?? 'left'] ?? 'l',
    );
  }
  if (transition.type === 'split') {
    effect.setAttribute(
      'orient',
      transition.orientation === 'vertical' ? 'vert' : 'horz',
    );
    effect.setAttribute('dir', transition.direction === 'in' ? 'in' : 'out');
  }
  element.append(effect);
  const anchor =
    directChildren(root).find((child) =>
