...
export function presentationAgentProposalTargets(
  ...
  return candidates.flatMap((candidate, elementIndex) => {
    const label = element
      ? slideElementLabel(candidate)
      : `元素 ${elementIndex + 1} · ${slideElementLabel(candidate)}`;
    if (candidate.table) {
...
export function presentationNotesProposalTarget(
  slide: WorkSlide,
): WorkAgentProposalTarget {
  return {
    id: 'notes',
...
function slideElementDetails(element: WorkSlideElement): string[] {
  ...
  if (element.table?.rows.length) {
    lines.push(
      '表格：',
      ...element.table.rows.map((row) => row.map(normalizeValue).join('\t')),
    );
  }
  if (element.chart) {
    lines.push(
  ...
  if (element.altText?.trim())
    lines.push(`替代文本：${element.altText.trim()}`);
...
function presentationChartAxisDescription(
  ...
  const parts = [
    ...
    !axis.title?.trim() && axis.titleReference?.trim()
      ? `标题引用“${axis.titleReference.trim()}”`
      : '',
