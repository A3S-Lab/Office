...
function semanticPresentationChartAxes(chart: WorkSlideChart): {
  ...
  const normalized = normalizeWorkSpreadsheetChartAxes(
    chart.axes,
    sourceType,
    false,
  );
  ...
  const categoryOrX = mergeLegacyTitle(
    normalized?.[categoryPosition],
    legacyCategoryTitle,
  );
  const valueOrY = mergeLegacyTitle(
    normalized?.[valuePosition],
    legacyValueTitle,
  );
...
function mergeLegacyTitle(
  ...
  if (!legacyTitle || axis?.title?.trim() || axis?.titleReference?.trim())
    return axis;
 ...
 */
export function presentationChartAxes(
...
export function withPresentationChartAxes(
  chart: WorkSlideChart,
  axes: WorkSlideChartAxes | undefined,
): WorkSlideChart {
  const {
    axes: _axes,
    categoryAxisTitle: _categoryAxisTitle,
    valueAxisTitle: _valueAxisTitle,
    ...base
  } = chart;
  const normalized = normalizeWorkSpreadsheetChartAxes(
    axes,
    chartTypeWithAxes(chart.type),
    false,
  );
