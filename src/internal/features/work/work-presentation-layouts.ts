...
export function withPresentationDesign(
  ...
  const layouts = content.layouts?.length
    ? content.layouts.map((layout) => ({
        ...layout,
        masterId: masterIds.has(layout.masterId)
          ? layout.masterId
          : firstMasterId,
      }))
    : [
...
export function presentationSlideView(
  ...
  const placeholderElements = mergedPlaceholders(
    masterElements,
    layoutElements,
  );
  return {
    background:
      slide.useLayoutBackground === true
        ...
        : slide.background,
    inheritedElements: [...masterElements, ...layoutElements].filter(
...
export function applyPresentationLayout(
  ...
  const normalized = withPresentationDesign(content);
  const layout = normalized.layouts?.find(
  ...
  const definitions = layout.elements.filter(
    (
      element,
    ): element is WorkSlideElement & {
      placeholder: NonNullable<WorkSlideElement['placeholder']>;
    ...
    slides: normalized.slides.map((slide) => {
      ...
      const elements = slide.elements.map((element) => {
        ...
        const definition = definitions.find(
          (candidate) => candidate.placeholder.key === key,
        );
...
function applyPlaceholderDefinition(
