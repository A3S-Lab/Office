...
const CARDINAL_DIRECTIONS = new Set<WorkSlideTransitionDirection>([
  'left',
  'right',
  'up',
  'down',
...
export function createWorkSlideTransition(
  ...
  if (type === 'push' || type === 'wipe') {
    return {
      ...common,
      direction:
        previous?.direction && CARDINAL_DIRECTIONS.has(previous.direction)
          ? previous.direction
          : 'left',
    };
  }
  if (type === 'split') {
    return {
      ...common,
      direction:
        previous?.direction && SPLIT_DIRECTIONS.has(previous.direction)
          ? previous.direction
          : 'out',
...
export function slideTransitionDurationMilliseconds(
