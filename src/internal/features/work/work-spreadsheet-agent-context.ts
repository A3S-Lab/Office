...
import {
  chartSeriesErrorBarContext,
  chartSeriesTrendlineContext,
} from './work-chart-series-analysis-context';
import {
...
export function spreadsheetAgentSelection(
  ...
  const sheet =
    content.sheets.find((candidate) => candidate.id === sheetId) ??
    content.sheets[0];
  if (!sheet || selection.row.length < 2 || selection.column.length < 2)
    return null;
  ...
  for (let row = rowStart; row <= rowEnd && included < limit; row += 1) {
    ...
    for (
      ...
      column <= columnEnd && included < limit;
      column += 1
    ) {
      ...
      if (cell?.f) {
        const sourceFormula =
          sheet.formulaMetadata?.sourceFormulas?.[cellReference] ?? cell.f;
  ...
  const formulaContext =
    formulas.length || formulaRanges.length
      ? [
          ...
          ...formulaRanges.map(
            (range) =>
              `${range.reference}：${formulaRangeTypeLabel(range.type)}（锚点 ${range.anchor}）`,
          ),
        ]
      : [];
...
function relatedPivotContext(
  ...
  const related = new Map<
    ...
    {
      pivot: NonNullable<
        WorkSpreadsheetContent['sheets'][number]['pivotTables']
      >[number];
  ...
  return Array.from(related.values()).map(({ pivot, owner }) => {
    ...
    const filters =
      (pivot.reportFilters ?? [])
        .map((filter) => {
          const field =
            fields[filter.fieldIndex]?.name ?? `字段 ${filter.fieldIndex + 1}`;
          const selected =
            filter.selectedItem === undefined
              ? '（全部）'
              : spreadsheetPivotFilterDisplay(filter.selectedItem);
          return `${field}=${selected}`;
        })
        .join('、') || '无';
    const sourceSheet = content.sheets.find(
      (candidate) => candidate.id === pivot.sourceSheetId,
    );
...
function spreadsheetPivotFilterDisplay(
        ...
        .map((series) => {
          if (resolved.type === 'scatter' || resolved.type === 'bubble') {
            const xValues = series.xValues?.length
              ? series.xValues
              : series.values.map((_, index) => index + 1);
            return `${series.name}：X=${xValues.slice(0, 12).join('、')}；Y=${series.values.slice(0, 12).join('、')}${
              resolved.type === 'bubble'
                ? `；大小=${series.bubbleSizes?.slice(0, 12).join('、') || '无'}`
                : ''
          ...
          if (resolved.type === 'combination') {
            const seriesType = normalizeWorkSpreadsheetCombinationSeriesType(
              series.chartType,
            );
            const axisGroup = normalizeWorkSpreadsheetChartAxisGroup(
              series.axisGroup,
            );
...
function chartLayoutContext(chart: WorkSpreadsheetChart): string {
  ...
  if (chart.showLegend) {
    const position = normalizeWorkSpreadsheetChartLegendPosition(
    ...
    const overlay = normalizeWorkSpreadsheetChartLegendOverlay(
      ...
      : '';
    descriptions.push(
      `图例：${workSpreadsheetChartLegendPositionLabel(position)}${overlay}`,
    );
  }
  if (workSpreadsheetChartSupportsGrouping(chart.type)) {
    ...
    if (workSpreadsheetChartSupportsBarSpacing(chart.type)) {
      settings.push(
        `分类间距 ${normalizeWorkSpreadsheetChartGapWidth(chart.gapWidth)}%`,
      );
      settings.push(
        `系列重叠 ${normalizeWorkSpreadsheetChartOverlap(chart.overlap, grouping)}%`,
      );
    }
    if (
...
function chartAxisTitleContext(chart: WorkSpreadsheetChart): string {
  ...
  if (!axes) return '';
  const descriptions = (['bottom', 'left', 'top', 'right'] as const).flatMap(
    (position) => {
      ...
      const settings = [
        ...
        axis.numberFormat
          ...
          : axis.numberFormatSourceLinked
            ? '数字格式链接源'
            : '',
        axis.showMajorGridlines === undefined
          ? ''
        ...
        axis.labelPosition
          ? `标签置于${workSpreadsheetChartAxisLabelPositionLabel(axis.labelPosition)}`
          : '',
        axis.majorTickMark
          ? `主要刻度线${workSpreadsheetChartAxisTickMarkLabel(axis.majorTickMark)}`
          : '',
...
function seriesDataLabelContext(
  ...
  const description = [
    content.join('、') || '无内容字段',
    labels.position
      ? workSpreadsheetDataLabelPositionLabel(labels.position)
      : '',
    labels.separator !== undefined
      ? `分隔符“${labels.separator.replace(/\r?\n/g, '↵')}”`
      : '',
  ].filter(Boolean);
...
function calculationModeLabel(
  mode: ReturnType<typeof effectiveSpreadsheetCalculationSettings>['mode'],
): string {
