...
export function workSpreadsheetChartAxisIsValueAxis(
  ...
  if (!workSpreadsheetChartSupportsAxes(chartType) || position === 'top')
    return false;
  if (workSpreadsheetChartUsesNumericXAxis(chartType))
...
export function workSpreadsheetChartAxisIsCategoryAxis(
  ...
  if (
    !workSpreadsheetChartSupportsAxes(chartType) ||
...
export function workSpreadsheetChartAxisPositionLabel(
  position: WorkSpreadsheetChartAxisPosition,
): string {
...
export function normalizeWorkSpreadsheetChartAxes(
  ...
  const bottom = normalizeWorkSpreadsheetChartAxis(
    source.bottom,
    chartType,
    'bottom',
  );
  const left = normalizeWorkSpreadsheetChartAxis(
    source.left,
    chartType,
    'left',
  );
...
function normalizeWorkSpreadsheetChartAxis(
  ...
  const minimum =
    valueAxis && Number.isFinite(source.minimum)
      ? Number(source.minimum)
      : undefined;
  const maximum =
    valueAxis && Number.isFinite(source.maximum)
  ...
  const majorUnit =
    ...
    Number(source.majorUnit) > 0
      ? Number(source.majorUnit)
      : undefined;
  const showMajorGridlines =
    valueAxis && typeof source.showMajorGridlines === 'boolean'
      ? source.showMajorGridlines
      : undefined;
  const numberFormat = valueAxis
    ? source.numberFormat?.trim().slice(0, 255)
    : undefined;
  const numberFormatSourceLinked =
    valueAxis && typeof source.numberFormatSourceLinked === 'boolean'
      ? source.numberFormatSourceLinked
      : undefined;
  ...
  const labelInterval = categoryAxis
    ? normalizedLabelInterval(source.labelInterval)
    : undefined;
  const normalized: WorkSpreadsheetChartAxis = {
    ...
    ...(numberFormatSourceLinked !== undefined
...
function normalizedAxisLabelPosition(
  value: unknown,
...
function normalizedAxisTickMark(
  value: unknown,
...
function normalizedLabelInterval(value: unknown): number | undefined {
  ...
  return Number.isInteger(interval) && interval >= 1 && interval <= 31_999
    ? interval
    : undefined;
}
...
export function spreadsheetChartAxisValueRatio(
  ...
  const ratio = Math.min(
    1,
    Math.max(0, (finiteChartNumber(value) - scale.minimum) / scale.span),
  );
...
export function spreadsheetChartCategoryLabelVisible(
  ...
  return (
    index % interval === 0 ||
    (explicitInterval === undefined && index === count - 1)
  );
}
...
export function spreadsheetChartAxisScale(
  ...
  const padding =
    sourceSpan > 0
      ? sourceSpan * paddingRatio
      : Math.max(1, Math.abs(automaticMinimum) * Math.max(paddingRatio, 0.05));
  automaticMinimum -= padding;
  automaticMaximum += padding;

  let minimum = Number.isFinite(axis?.minimum)
    ? Number(axis?.minimum)
    : automaticMinimum;
  let maximum = Number.isFinite(axis?.maximum)
    ? Number(axis?.maximum)
    : automaticMaximum;
  const fallbackSpan = Math.max(
    Number.isFinite(axis?.majorUnit) && Number(axis?.majorUnit) > 0
      ? Number(axis?.majorUnit)
      : 0,
  ...
  if (!(minimum < maximum)) {
    if (axis?.minimum !== undefined && axis.maximum === undefined)
      maximum = minimum + fallbackSpan;
    else if (axis?.maximum !== undefined && axis.minimum === undefined)
      minimum = maximum - fallbackSpan;
    else {
...
export function spreadsheetChartAxisScaleAttributes(
  ...
  return [
    ...
    scale.majorUnit === undefined
      ? ''
      : `data-axis-major-unit="${roundChartNumber(scale.majorUnit)}"`,
    `data-axis-gridlines="${spreadsheetChartAxisGridlinesVisible(axis, chartType, position)}"`,
    axis?.numberFormat
      ? `data-axis-number-format="${escapeChartXml(axis.numberFormat)}"`
      : '',
...
export function formatSpreadsheetChartAxisNumber(
  value: number,
  ...
  const section =
    code.split(';')[value < 0 ? 1 : 0] || code.split(';')[0] || code;
...
function axisTicks(
  ...
  if (majorUnit === undefined) {
    return Array.from({ length: DEFAULT_TICK_COUNT }, (_, index) =>
      normalizedTick(
        minimum + (index / (DEFAULT_TICK_COUNT - 1)) * (maximum - minimum),
      ),
    );
  }
  ...
  return Array.from({ length: count }, (_, index) =>
    normalizedTick(first + index * majorUnit),
  );
}
