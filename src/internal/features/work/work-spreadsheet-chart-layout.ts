...
export type WorkSpreadsheetChartLegendPosition =
  | 'right'
  | 'left'
  | 'top'
  | 'bottom'
...
export function normalizeWorkSpreadsheetChartLegendPosition(
  value: unknown,
...
export function normalizeWorkSpreadsheetChartLegendOverlay(
  value: unknown,
): boolean {
  return value === true;
}

export function workSpreadsheetChartSupportsGrouping(
  type: WorkSpreadsheetChartType,
): boolean {
  return (
    type === 'column' || type === 'bar' || type === 'line' || type === 'area'
  );
}

export function workSpreadsheetChartSupportsBarSpacing(
  type: WorkSpreadsheetChartType,
): boolean {
  return type === 'column' || type === 'bar';
}

export function workSpreadsheetChartSupportsSmoothLines(
  type: WorkSpreadsheetChartType,
): boolean {
  return type === 'line';
}

export function workSpreadsheetChartDefaultGrouping(
  type: WorkSpreadsheetChartType,
): WorkSpreadsheetChartGrouping {
  return workSpreadsheetChartSupportsBarSpacing(type)
    ? 'clustered'
    : 'standard';
}
...
export function workSpreadsheetChartGroupingIsStacked(
  grouping: WorkSpreadsheetChartGrouping,
): boolean {
...
export function normalizeWorkSpreadsheetChartOverlap(
  value: unknown,
  grouping: WorkSpreadsheetChartGrouping,
): number {
  return normalizedInteger(
    ...
    -100,
    100,
    workSpreadsheetChartGroupingIsStacked(grouping) ? 100 : 0,
  );
}

export function normalizeWorkSpreadsheetChartSmoothLines(
  value: unknown,
): boolean {
  return value === true;
}
...
export function normalizeWorkSpreadsheetChartLayout(
  ...
  return {
    legendPosition: normalizeWorkSpreadsheetChartLegendPosition(
      source.legendPosition,
    ),
    legendOverlay: normalizeWorkSpreadsheetChartLegendOverlay(
      source.legendOverlay,
    ),
    grouping: workSpreadsheetChartSupportsGrouping(source.type)
      ? grouping
      : undefined,
...
export function workSpreadsheetChartSupportsSeriesAnalysis(
  source: WorkSpreadsheetChartLayoutSource,
): boolean {
  if (!workSpreadsheetChartSupportsGrouping(source.type)) return true;
  return !workSpreadsheetChartGroupingIsStacked(
    normalizeWorkSpreadsheetChartGrouping(source.grouping, source.type),
  );
}

export function workSpreadsheetChartLegendPositionLabel(
...
export function workSpreadsheetChartGroupingLabel(
  grouping: WorkSpreadsheetChartGrouping,
): string {
