...
function legendBounds(
  ...
  if (position === 'right' || position === 'topRight') {
    return {
      x: overlay
        ? source.x + source.width - VERTICAL_RESERVE + 12
        : plot.x + plot.width + 12,
      y:
        position === 'topRight'
          ? frame.titleHeight + 4
          : frame.titleHeight + 18,
      width: VERTICAL_RESERVE - 18,
      height: frame.height - frame.titleHeight - 32,
    };
  }
...
function spreadsheetChartLegendItems(
  chart: WorkSpreadsheetChart,
): SpreadsheetChartLegendItem[] {
  if (chart.type === 'pie' || chart.type === 'doughnut') {
    ...
    return chart.categories.map((category, index) => ({
      color:
        seriesColor ??
        WORK_SPREADSHEET_CHART_COLORS[
          index % WORK_SPREADSHEET_CHART_COLORS.length
        ],
...
function verticalLegendEntries(
  items: SpreadsheetChartLegendItem[],
  bounds: SpreadsheetChartPlotBounds,
): string {
  return items
    .map((item, index) =>
      legendEntrySvg(item, index, bounds.x, bounds.y + index * 21, 16),
    )
    .join('');
}

function horizontalLegendEntries(
  items: SpreadsheetChartLegendItem[],
  bounds: SpreadsheetChartPlotBounds,
): string {
  ...
  const columns = Math.max(
    1,
    Math.min(items.length, Math.floor(bounds.width / 96)),
  );
