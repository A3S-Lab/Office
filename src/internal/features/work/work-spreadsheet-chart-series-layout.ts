...
type SpreadsheetChartSeriesLayoutSource = Pick<
  WorkSpreadsheetChart,
  'categories' | 'grouping' | 'series' | 'type'
>;
...
export function spreadsheetChartSeriesLayout(
  chart: SpreadsheetChartSeriesLayoutSource,
  ...
  const result = chart.series.map(
    () => [] as SpreadsheetChartSeriesLayoutPoint[],
  );
  ...
  for (
    ...
    const rawValues = chart.series.map((series) =>
      finiteChartNumber(series.values[categoryIndex] ?? 0),
    );
    const positiveTotal = rawValues.reduce(
      (sum, value) => sum + Math.max(0, value),
      0,
    );
    const negativeTotal = rawValues.reduce(
  ...
  const scaleValues = result.flatMap((series) =>
    series.flatMap((point) => [point.start, point.end]),
  );
...
export function spreadsheetChartBarGeometry(
  ...
  chart: Pick<
    WorkSpreadsheetChart,
    'gapWidth' | 'overlap' | 'grouping' | 'type'
  >,
