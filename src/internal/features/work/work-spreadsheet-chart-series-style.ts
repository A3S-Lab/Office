...
const LINE_DASHES = new Set<WorkSpreadsheetChartLineDash>([
  'solid',
  'dash',
  'dot',
  'dashDot',
...
export function normalizeWorkSpreadsheetChartSeriesStyle(
  ...
  const fillTransparency = optionalBoundedNumber(
    source.fillTransparency,
    0,
    100,
    0,
  );
  ...
  const lineDash = LINE_DASHES.has(
...
export function normalizeWorkSpreadsheetChartColor(
  value: unknown,
): string | undefined {
...
export function defaultWorkSpreadsheetChartSeriesStyle(
  seriesIndex: number,
): WorkSpreadsheetChartSeriesStyle {
...
export function workSpreadsheetChartSeriesDefaultColor(
  seriesIndex: number,
): string {
  return WORK_SPREADSHEET_CHART_COLORS[
    seriesIndex % WORK_SPREADSHEET_CHART_COLORS.length
  ].toUpperCase();
}
...
export function spreadsheetChartSeriesFillStyle(
  ...
  const opacity =
    style?.fillTransparency === undefined
      ? boundedNumber(defaultOpacity, 0, 1, 1)
...
export function spreadsheetChartSeriesLineStyle(
  ...
  const style = normalizeWorkSpreadsheetChartSeriesStyle(series.style);
  const hasCustomLine = Boolean(
    style?.lineColor || style?.lineWidth !== undefined || style?.lineDash,
  );
  const color = style?.lineColor ?? defaultColor;
  const width =
    style?.lineWidth ?? (hasCustomLine && defaultWidth <= 0 ? 1 : defaultWidth);
...
export function spreadsheetChartSeriesLegendColor(
  series: WorkSpreadsheetChartSeries,
  seriesIndex: number,
): string {
  const style = normalizeWorkSpreadsheetChartSeriesStyle(series.style);
  return (
    style?.fillColor ??
    style?.lineColor ??
    workSpreadsheetChartSeriesDefaultColor(seriesIndex)
  );
}
...
export function spreadsheetChartSeriesMarkerSvg(
  ...
  if (!visible) return '';
  const symbol =
    marker?.symbol && marker.symbol !== 'none' ? marker.symbol : 'circle';
...
export function spreadsheetChartSeriesStyleContext(
  ...
  if (normalized.fillColor || normalized.fillTransparency !== undefined) {
    descriptions.push(
      `填充 ${normalized.fillColor ?? '默认颜色'}${
        normalized.fillTransparency === undefined
          ? ''
          : `（透明度 ${compactDecimal(normalized.fillTransparency)}%）`
      }`,
    );
  }
  if (
    normalized.lineColor ||
    ...
    descriptions.push(
      [
        ...
        normalized.lineWidth === undefined
          ? ''
          : `${compactDecimal(normalized.lineWidth)} 磅`,
        normalized.lineDash
          ? workSpreadsheetChartLineDashLabel(normalized.lineDash)
          : '',
      ]
        .filter(Boolean)
        .join('、'),
...
export function workSpreadsheetChartLineDashLabel(
  dash: WorkSpreadsheetChartLineDash,
): string {
...
export function workSpreadsheetChartMarkerSymbolLabel(
...
function normalizeWorkSpreadsheetChartMarkerStyle(
  value: unknown,
): WorkSpreadsheetChartMarkerStyle | undefined {
  if (!value || typeof value !== 'object') return undefined;
  const source = value as Record<string, unknown>;
  const symbol = MARKER_SYMBOLS.has(
...
function spreadsheetChartLineDashArray(
  dash: WorkSpreadsheetChartLineDash,
): string {
