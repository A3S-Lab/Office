...
import {
  ...
  type SpreadsheetChartPlotBounds,
} from './work-spreadsheet-chart-legend-svg';
import {
...
import {
  spreadsheetSeriesTrendlineFits,
  spreadsheetSeriesTrendlinesSvg,
} from './work-spreadsheet-trendline-svg';
import {
...
export function spreadsheetChartSvgDataUrl(
...
export function spreadsheetChartSvg(chart: WorkSpreadsheetChart): string {
  ...
  const axes = normalizeWorkSpreadsheetChartAxes(
    chart.axes,
    chart.type,
    hasSecondaryAxis,
  );
  ...
  const renderedChart: WorkSpreadsheetChart = {
    ...
    ...(workSpreadsheetChartSupportsBarSpacing(chart.type)
      ? {
          ...
          overlap: normalizeWorkSpreadsheetChartOverlap(
            chart.overlap,
            grouping,
          ),
        }
    ...
    ...(workSpreadsheetChartSupportsSmoothLines(chart.type)
      ? {
          smoothLines: normalizeWorkSpreadsheetChartSmoothLines(
            chart.smoothLines,
          ),
        }
      : {}),
  ...
  const secondaryAxisWidth = hasSecondaryAxis
    ? axes?.right?.title
      ? 58
      : 44
    : 0;
  const plotLeft =
    chart.type === 'bar' ? 88 : circular || polar ? 24 : xy ? 64 : 56;
  const basePlot = {
    x: plotLeft,
...
function spreadsheetChartPlotAttributes(
  chart: WorkSpreadsheetChart,
  ...
  if (workSpreadsheetChartSupportsGrouping(chart.type)) {
    attributes.push(
      `data-chart-grouping="${normalizeWorkSpreadsheetChartGrouping(chart.grouping, chart.type)}"`,
    );
  }
  if (workSpreadsheetChartSupportsBarSpacing(chart.type)) {
    ...
    attributes.push(
      `data-chart-gap-width="${normalizeWorkSpreadsheetChartGapWidth(chart.gapWidth)}"`,
    );
    attributes.push(
      `data-chart-overlap="${normalizeWorkSpreadsheetChartOverlap(chart.overlap, grouping)}"`,
    );
  }
  if (workSpreadsheetChartSupportsSmoothLines(chart.type)) {
    attributes.push(
      `data-chart-smooth-lines="${normalizeWorkSpreadsheetChartSmoothLines(chart.smoothLines)}"`,
    );
  }
...
function doughnutChartSvg(
  ...
  const hasValues = chart.series.some((series) =>
    series.values.some((value) => Math.max(0, finiteNumber(value)) > 0),
  );
...
function doughnutRingSvg(
  ...
  if (!total) return '';
  const positiveIndices = usableValues.flatMap((value, index) =>
    value > 0 ? [index] : [],
  );
  if (positiveIndices.length === 1) {
    ...
    )}" data-chart-slice="${seriesIndex}:${index}"/>${spreadsheetDataLabelSvg(
      chart,
      seriesIndex,
      index,
      {
  ...
  return usableValues
    .map((value, index) => {
      ...
      const line = spreadsheetChartSeriesLineStyle(
        chart.series[seriesIndex],
        seriesIndex,
        1,
        '#FFFFFF',
...
function radarChartSvg(
  ...
  const point = (index: number, ratio: number) => {
    const visualIndex = spreadsheetChartCategoryVisualIndex(
      index,
      categoryCount,
      categoryAxis,
    );
  ...
  const polygon = (ratio: number) =>
    Array.from({ length: categoryCount }, (_, index) =>
      point(index, ratio).map(round).join(','),
    ).join(' ');
  const showGridlines = spreadsheetChartAxisGridlinesVisible(
    axis,
    chart.type,
    'left',
  );
  const grid = scale.ticks
    ...
    .map((tick, index) => {
      ...
      return `${
        showGridlines
          ? `<polygon points="${polygon(ratio)}" fill="none" stroke="#dfe4ec" stroke-width="1"/>`
          : ''
  ...
  const labels = Array.from({ length: categoryCount }, (_, index) => {
    ...
    const anchor =
      Math.abs(horizontal) < 8 ? 'middle' : horizontal > 0 ? 'start' : 'end';
    const visible =
  ...
  const series = chart.series
    .map((item, seriesIndex) => {
      ...
      const dataLabels = points
        .map(([x, y], pointIndex) =>
          spreadsheetDataLabelSvg(chart, seriesIndex, pointIndex, {
            kind: 'point',
            x,
            y,
          }),
...
function xyChartSvg(
  ...
  const trendlinePoints = chart.series.flatMap((series) =>
    spreadsheetSeriesTrendlineFits(
      series,
      spreadsheetTrendlineXValues(series),
    ).flatMap((item) => item.fit.points),
  );
  ...
  const xBounds = spreadsheetChartAxisScale(
    [
      ...
      ...chart.series.flatMap((series) =>
        spreadsheetSeriesErrorBarBounds(series, chart.type, 'x'),
      ),
    ],
    chart.axes?.bottom,
    { paddingRatio },
  ...
  const yBounds = spreadsheetChartAxisScale(
    [
      ...
      ...chart.series.flatMap((series) =>
        spreadsheetSeriesErrorBarBounds(series, chart.type, 'y'),
      ),
    ],
    chart.axes?.left,
    { paddingRatio },
  ...
  const pointX = (value: number) =>
    plot.x +
    spreadsheetChartAxisValueRatio(value, xBounds, chart.axes?.bottom) *
      plot.width;
  const pointY = (value: number) =>
    plot.y +
    (1 - spreadsheetChartAxisValueRatio(value, yBounds, chart.axes?.left)) *
      plot.height;
  const axes = spreadsheetXyChartAxesSvg(chart, plot, xBounds, yBounds);
  const errorBars = chart.series
    .map((series, seriesIndex) =>
      spreadsheetSeriesErrorBarsSvg(
        chart,
        seriesIndex,
        (direction, pointIndex, value) => {
          const x = series.xValues?.[pointIndex] ?? pointIndex + 1;
          const y = series.values[pointIndex] ?? 0;
          return direction === 'x'
            ? [pointX(value), pointY(y)]
            : [pointX(x), pointY(value)];
        },
      ),
    )
    .join('');
...
function spreadsheetTrendlineXValues(
...
function scatterSeriesSvg(
  ...
  const series = seriesPoints
    .map((points, seriesIndex) => {
      ...
      const lineStyle = spreadsheetChartSeriesLineStyle(
        sourceSeries,
        seriesIndex,
        2.3,
      );
...
function scatterPath(
  points: Array<SpreadsheetXyPoint & { plotX: number; plotY: number }>,
  smooth: boolean,
): string {
  ...
  if (!smooth || points.length < 3) {
    return points
      .map(
        (point, index) =>
          `${index ? 'L' : 'M'} ${round(point.plotX)} ${round(point.plotY)}`,
      )
      .join(' ');
  }
...
function bubbleSeriesSvg(
  ...
  const sizeRepresents = normalizeWorkSpreadsheetBubbleSizeRepresents(
    chart.bubbleSizeRepresents,
  );
  const maximumSize = Math.max(
    1,
    ...seriesPoints.flatMap((points) =>
      points.map((point) => Math.abs(point.bubbleSize ?? 0)),
    ),
  );
  ...
  const series = seriesPoints
    .map((points, seriesIndex) => {
      const sourceSeries = chart.series[seriesIndex];
      const sourceStyle = normalizeWorkSpreadsheetChartSeriesStyle(
        sourceSeries.style,
      );
      const fill = spreadsheetChartSeriesFillStyle(
        sourceSeries,
      ...
      const bubbles = points
        .map((point) => {
          ...
          const radius =
            maximumRadius *
            (sizeRepresents === 'area' ? Math.sqrt(ratio) : ratio);
          const negative = size < 0;
          const bubbleOpacity =
            sourceStyle?.fillTransparency === undefined
              ? negative
                ? 0.2
                : 0.34
              : fill.opacity;
...
function cartesianChartSvg(
  ...
  const trendlineValues = layout.stacked
    ...
    : chart.series.flatMap((series) =>
        spreadsheetSeriesTrendlineFits(
          series,
          categoryTrendlineXValues(series),
        ).flatMap((item) => item.fit.points.map((point) => point.y)),
      );
  const values = [
    ...layout.scaleValues,
    ...(layout.stacked
  ...
  if (!values.length) return emptyChartSvg(plot, '没有可绘制的数据');
  if (chart.type === 'bar')
    return horizontalBarChartSvg(chart, plot, layout, values);
  const axis = chart.axes?.left;
  const scale = spreadsheetChartAxisScale(values, axis, { includeZero: true });
  const valuePosition = (value: number) =>
    plot.y +
    (1 - spreadsheetChartAxisValueRatio(value, scale, axis)) * plot.height;
  const baseline = valuePosition(0);
  const numberFormat =
    axis?.numberFormat ??
    (layout.grouping === 'percentStacked' ? '0%' : undefined);
  const showGridlines = spreadsheetChartAxisGridlinesVisible(
    axis,
    chart.type,
    'left',
  ...
  const trendlines = layout.stacked
    ? ''
    : cartesianTrendlinesSvg(chart, plot, valuePosition);
  ...
  return chart.series
    .map((series, seriesIndex) =>
      spreadsheetSeriesTrendlinesSvg(
        ...
        seriesIndex,
        categoryTrendlineXValues(series),
        (x, y) => [categoryPosition(x), valuePosition(y)],
      ),
    )
    .join('');
}

function categoryTrendlineXValues(
  series: WorkSpreadsheetChartSeries,
): number[] {
...
function barChartSvg(
  ...
  const categoryAxis = chart.axes?.bottom;
  const geometry = spreadsheetChartBarGeometry(
    groupWidth,
    seriesCount,
    chart,
    44,
...
function horizontalBarChartSvg(
  ...
  const scale = spreadsheetChartAxisScale(values, axis, { includeZero: true });
  const valuePosition = (value: number) =>
    plot.x + spreadsheetChartAxisValueRatio(value, scale, axis) * plot.width;
  ...
  const grid = scale.ticks
    ...
    .join('');
  const labels = Array.from({ length: categoryCount }, (_, index) => {
    const visible = spreadsheetChartCategoryLabelVisible(
      index,
      categoryCount,
      categoryAxis,
    );
    ...
    const labelLayout = spreadsheetChartAxisLabelLayout(
      categoryAxis,
      chart.type,
      'left',
      plot,
