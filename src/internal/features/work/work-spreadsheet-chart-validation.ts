...
export function validateSpreadsheetChartSeriesTrendlines(
  ...
  for (const [trendlineIndex, trendline] of (
    ...
    if (
      ...
      (!Number.isInteger(trendline.period) ||
        (trendline.period ?? 0) < 2 ||
        (trendline.period ?? 0) > 255)
    ) {
      return `${prefix}的移动平均周期必须是 2 到 255 之间的整数。`;
    }
    if (
      trendline.forward !== undefined &&
      (!Number.isFinite(trendline.forward) || trendline.forward < 0)
    ) {
      return `${prefix}的前推值必须是非负数。`;
    }
    if (
      trendline.backward !== undefined &&
      (!Number.isFinite(trendline.backward) || trendline.backward < 0)
    ) {
      return `${prefix}的后推值必须是非负数。`;
    }
    if (
      trendline.intercept !== undefined &&
      !Number.isFinite(trendline.intercept)
    ) {
