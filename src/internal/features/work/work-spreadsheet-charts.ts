...
export function spreadsheetChartCount(content: WorkSpreadsheetContent): number {
  return content.sheets.reduce(
    (count, sheet) => count + (sheet.charts?.length ?? 0),
    0,
  );
}

export function spreadsheetSheetsWithChartPreviews(
...
export function reconcileSpreadsheetChartPreviews(
  ...
  const sourceById = new Map(
    content.sheets.flatMap((sheet) =>
      sheet.id ? [[sheet.id, sheet] as const] : [],
    ),
  );
  const renamedSheets = new Map(
    changedSheets.flatMap((sheet) => {
      ...
      return sourceName && sourceName !== sheet.name
        ? [[sourceName, sheet.name] as const]
        : [];
    }),
  );
  const sheets = changedSheets.map((changedSheet) => {
    const source =
      (changedSheet.id ? sourceById.get(changedSheet.id) : undefined) ??
      changedSheet;
    ...
    const charts = (source.charts ?? []).flatMap((sourceChart) => {
      const chart = renameSpreadsheetChartReferences(
        sourceChart,
        renamedSheets,
      );
    ...
    const images = (changedSheet.images ?? []).filter(
      (image) => !image.id.startsWith(CHART_PREVIEW_IMAGE_PREFIX),
    );
    return {
      ...changedSheet,
...
export function resolveSpreadsheetChart(
  ...
  const series = chart.series.map((item) =>
    resolveSpreadsheetChartSeries(content, ownerSheet, item),
  );
  const axes = resolveSpreadsheetChartAxes(content, ownerSheet, chart.axes);
  return {
    ...chart,
    title:
      titleValue === undefined ? chart.title : displayCellValue(titleValue),
...
export function createSpreadsheetChartFromSelection(
  ...
  if (columnCount > 1) {
    ...
    series = inclusiveNumbers(bounds.startColumn + 1, bounds.endColumn).map(
      (column, index) => ({
        name: hasHeader
          ? displayCellValue(
    ...
    categories = inclusiveNumbers(dataStartRow, bounds.endRow).map((row) =>
      String(row + 1),
    );
    series = [
      {
  ...
  const left =
    axisPosition(bounds.endColumn + 1, (index) =>
      spreadsheetColumnWidth(sheet.config, index),
    ) + 16;
  const top = axisPosition(bounds.startRow, (index) =>
    spreadsheetRowHeight(sheet.config, index),
  );
  return {
...
export function parseSpreadsheetChartReference(
  ...
  const sheet = sheetName
    ? content.sheets.find((candidate) => candidate.name === sheetName)
    : ownerSheet;
  if (!sheet) return null;
...
function renameSpreadsheetChartReferences(
  ...
  return {
    ...chart,
    titleReference: renameSpreadsheetReference(
      chart.titleReference,
      renamedSheets,
    ),
    axes: renameSpreadsheetChartAxesReferences(chart.axes, renamedSheets),
    categoryReference: renameSpreadsheetReference(
      chart.categoryReference,
      renamedSheets,
    ),
    series: chart.series.map((series) => ({
      ...series,
      nameReference: renameSpreadsheetReference(
        series.nameReference,
        renamedSheets,
      ),
      valuesReference: renameSpreadsheetReference(
        series.valuesReference,
        renamedSheets,
      ),
      xValuesReference: renameSpreadsheetReference(
        series.xValuesReference,
        renamedSheets,
      ),
      bubbleSizesReference: renameSpreadsheetReference(
      ...
      ...(series.errorBars
        ? {
            errorBars: series.errorBars.map((errorBars) => ({
              ...errorBars,
              plusReference: renameSpreadsheetReference(
                errorBars.plusReference,
                renamedSheets,
              ),
              minusReference: renameSpreadsheetReference(
...
function renameSpreadsheetChartAxesReferences(
  ...
  const bottom = renameSpreadsheetChartAxisReference(
    axes.bottom,
    renamedSheets,
  );
...
function renameSpreadsheetReference(
  ...
  return renamed && match
    ? `${quoteSpreadsheetSheetName(renamed)}!${match[3]}`
    : formula;
}
  ...
  const errorBars = series.errorBars?.map((source) => ({
    ...
    plusValues: source.plusReference
      ? (spreadsheetReferenceValues(
          content,
          ownerSheet,
          source.plusReference,
        )?.map(numericCellValue) ?? source.plusValues)
      : source.plusValues,
    minusValues: source.minusReference
      ? (spreadsheetReferenceValues(
          content,
          ownerSheet,
          source.minusReference,
        )?.map(numericCellValue) ?? source.minusValues)
      : source.minusValues,
  }));
  return {
    ...series,
...
function resolveSpreadsheetChartAxis(
  ...
  return {
    ...axis,
    title: titleValue === undefined ? axis.title : displayCellValue(titleValue),
  };
}
...
function selectionHasHeader(
  ...
  const seriesStartColumn =
    bounds.endColumn > bounds.startColumn
      ? bounds.startColumn + 1
      : bounds.startColumn;
  const firstRowValues = rangeValues(
    sheet,
    bounds.startRow,
    seriesStartColumn,
    bounds.startRow,
  ...
  return (
    firstRowValues.some(
      (value) => String(value ?? '').trim() && !isNumericValue(value),
    ) && bodyValues.some(isNumericValue)
  );
}

function normalizedSelection(selection: Selection | undefined): {
  startRow: number;
  endRow: number;
  startColumn: number;
  endColumn: number;
  ...
  const endRow = Math.max(
    startRow,
    Math.max(selection.row[0], selection.row[1]),
  );
  const startColumn = Math.max(
    0,
    Math.min(selection.column[0], selection.column[1]),
  );
  const endColumn = Math.max(
    startColumn,
    Math.max(selection.column[0], selection.column[1]),
  );
