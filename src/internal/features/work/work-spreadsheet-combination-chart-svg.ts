...
export function spreadsheetCombinationChartSvg(
  ...
  const primaryValues = chart.series
    .filter(
      (series) =>
        normalizeWorkSpreadsheetChartAxisGroup(series.axisGroup) === 'primary',
    )
  ...
  const secondaryValues = chart.series
    ...
    .flatMap((series) => combinationSeriesValues(series));
  const primaryAxis = combinationAxis(
    primaryValues.length ? primaryValues : values,
    plot,
    chart.axes?.left,
  );
  const secondaryAxis = combinationAxis(
    secondaryValues.length ? secondaryValues : values,
    plot,
    chart.axes?.right,
  );
  ...
  const pointX = (index: number, axis: WorkSpreadsheetChartAxis | undefined) =>
    categoryCount === 1
      ? plot.x + plot.width / 2
      : plot.x +
        ((spreadsheetChartCategoryVisualIndex(index, categoryCount, axis) +
          0.5) /
          categoryCount) *
          plot.width;
  const axisForSeries = (seriesIndex: number) =>
    normalizeWorkSpreadsheetChartAxisGroup(
      chart.series[seriesIndex]?.axisGroup,
    ) === 'secondary'
      ? secondaryAxis
      : primaryAxis;
  ...
  const areas = chart.series
    ...
    .join('');
  const columns = combinationColumnSeriesSvg(
    chart,
    plot,
    categoryCount,
    axisForSeries,
  ...
  const trendlines = chart.series
    .map((series, seriesIndex) =>
      spreadsheetSeriesTrendlinesSvg(
        series,
        seriesIndex,
        combinationTrendlineXValues(series),
        (x, y) => [
...
function combinationTrendlineXValues(
  series: WorkSpreadsheetChart['series'][number],
): number[] {
  return series.values.map((_, index) => index + 1);
}

function combinationTrendlineValues(
  series: WorkSpreadsheetChart['series'][number],
): number[] {
  return spreadsheetSeriesTrendlineFits(
    series,
    combinationTrendlineXValues(series),
  ).flatMap((item) => item.fit.points.map((point) => point.y));
}

function combinationSeriesValues(
  series: WorkSpreadsheetChart['series'][number],
): number[] {
  return [
...
function combinationAxesSvg(
  ...
  const primaryGridlines = spreadsheetChartAxisGridlinesVisible(
    primaryAxis,
    chart.type,
    'left',
  );
  const secondaryGridlines = spreadsheetChartAxisGridlinesVisible(
    secondaryAxis,
    chart.type,
    'right',
  );
  ...
  const secondaryLabels = hasSecondary
    ? `<g data-secondary-axis="true" ${spreadsheetChartAxisScaleAttributes(
        ...
        .join(
          '',
        )}<line x1="${roundChartNumber(plot.x + plot.width)}" y1="${plot.y}" x2="${roundChartNumber(
        plot.x + plot.width,
...
function combinationAreaSeriesSvg(
  ...
  const points = series.values.map(
    (value, index) => [pointX(index), axis.position(value)] as const,
  );
  ...
  const line = points
    .map(
      ([x, y], index) =>
        `${index ? 'L' : 'M'} ${roundChartNumber(x)} ${roundChartNumber(y)}`,
    )
    .join(' ');
  ...
  const dataLabels = points
    ...
    .join('');
  const errorBars = spreadsheetSeriesErrorBarsSvg(
    chart,
    seriesIndex,
...
function combinationColumnSeriesSvg(
  ...
  const columnSeries = chart.series.flatMap((series, seriesIndex) =>
    normalizeWorkSpreadsheetCombinationSeriesType(series.chartType) === 'column'
      ? [{ series, seriesIndex }]
      : [],
  );
  ...
  const barWidth = Math.max(
    2,
    Math.min(36, (groupWidth * 0.72) / columnSeries.length),
  );
  return columnSeries
    .map(({ series, seriesIndex }, columnIndex) => {
      ...
      const seriesX = (pointIndex: number) =>
        pointXForSeries(seriesIndex, pointIndex) -
        (barWidth * columnSeries.length) / 2 +
        columnIndex * barWidth;
      ...
      const bars = series.values
        .map((value, index) => {
          ...
          )}" rx="1.5" ${fill.attributes} ${line.attributes}/>${spreadsheetDataLabelSvg(
            chart,
            seriesIndex,
            index,
            {
...
function combinationCategoryLabelsSvg(
  ...
  const showTopAxis =
    (topAxis?.labelPosition ?? 'none') !== 'none' ||
    (topAxis?.majorTickMark ?? 'none') !== 'none';
