...
export function importXlsxCellComment(
  source: Comments | undefined,
): WorkSpreadsheetCellComment | undefined {
  const comments = (source ?? []).filter((comment) => comment.t.trim());
  if (!comments.length) return undefined;
  const authors = Array.from(
    new Set(
      comments
        .map((comment) => comment.a?.trim())
  ...
  const value =
    comments.length === 1
      ...
      : comments
          .map(
            (comment) =>
              `${comment.a?.trim() ? `${comment.a.trim()}: ` : ''}${comment.t}`,
          )
          .join('\n\n');
  return {
    left: null,
...
export function exportXlsxCellComment(
  source: WorkSpreadsheetCellComment | undefined,
): Comments | undefined {
  ...
  const author =
    typeof (source as WorkSpreadsheetCellComment).author === 'string'
      ...
      : '';
  const comments = [
    { a: author.slice(0, 54) || 'A3S Work', t: text },
  ] as Comments;
...
function commentPlainText(value: string): string {
  ...
  for (const block of Array.from(document.body.querySelectorAll('div, p')))
    block.append('\n');
