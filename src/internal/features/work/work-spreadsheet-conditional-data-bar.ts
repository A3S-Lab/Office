...
export function spreadsheetConditionalDataBar(
  ...
  if (minimum < 0 && maximum > 0) {
    ...
    const ratio =
      value < 0 ? Math.abs(value / minimum) : value > 0 ? value / maximum : 0;
    const width =
      value === 0
        ? 0
        : displayLength(ratio, minLength, maxLength) *
          (value < 0 ? axis / 100 : (100 - axis) / 100);
    return {
      color,
    ...
    const ratio =
      minimum === maximum
        ? value === 0
          ? 0
          : 1
        : Math.max(0, Math.min(1, (maximum - value) / (maximum - minimum)));
