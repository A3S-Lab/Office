...
function applyDefaultRule(
  ...
  if (
    rule.type !== 'default' ||
    !isConditionalFormatStyle(rule.format) ||
    !rule.conditionName
  )
    return;
  ...
  if (rule.conditionName === 'duplicateValue') {
    for (const range of rule.cellrange) {
      const positions = cellPositions(data, [range]).filter(
        ({ row, column }) => cellValue(data, row, column) != null,
      );
      ...
      for (const { row, column } of positions) {
        const count =
          occurrences.get(comparableKey(cellValue(data, row, column))) ?? 0;
...
function applyAverageRule(
  ...
  for (const { row, column } of positions) {
    ...
    if (
      value !== null &&
      (conditionName === 'aboveAverage' ? value > average : value < average)
    ) {
      applyDifferentialStyle(styles, blocked, matched, row, column, style);
    }
  }
...
function applyColorScale(
  ...
  if (!options) return;
  const points = options.thresholds.map((threshold) =>
    spreadsheetConditionalThresholdValue(threshold, values),
  );
...
function applyIconSet(
  ...
  if (!format) return;
  const numericPositions = cellPositions(data, rule.cellrange).flatMap(
    (position) => {
      const value = numericCellValue(data, position.row, position.column);
      return value === null ? [] : [{ ...position, value }];
    },
  ...
  for (const { row, column, value } of numericPositions) {
    ...
    if (icon)
      applyConditionalStyle(styles, blocked, matched, row, column, { icon });
  }
}
...
function matchesCondition(
  ...
  if (name === 'greaterThanOrEqual')
    return compareValues(value, conditions[0]) >= 0;
