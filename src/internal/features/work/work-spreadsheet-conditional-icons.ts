...
export type SpreadsheetConditionalIconThresholdType =
  SpreadsheetConditionalThresholdType;

export interface SpreadsheetConditionalIconThreshold
  extends SpreadsheetConditionalThreshold {
  gte: boolean;
}
...
export function isSpreadsheetConditionalIconSetName(
  value: unknown,
): value is SpreadsheetConditionalIconSetName {
...
export function spreadsheetConditionalIconSetCount(
  ...
  return SPREADSHEET_CONDITIONAL_ICON_SETS.find(
    (item) => item.name === iconSet,
  )!.count;
}
...
export function spreadsheetConditionalIconAppearance(
  ...
  if (icon.iconSet.includes('Arrows')) {
    ...
    return {
      glyph: glyphs[index],
      color: icon.iconSet.includes('Gray')
        ? '#606a78'
        : palette(icon.count)[index],
      label,
    };
  }
  if (icon.iconSet === '3Flags')
    return { glyph: '⚑', color: COLOR_3[index], label };
  if (icon.iconSet.includes('TrafficLights')) {
    return {
      glyph: icon.iconSet === '3TrafficLights2' ? '◉' : '●',
  ...
  if (icon.iconSet === '4RedToBlack') {
    return {
      glyph: '●',
      color: ['#c62828', '#e45b4f', '#697386', '#111827'][index],
      label,
    };
  }
