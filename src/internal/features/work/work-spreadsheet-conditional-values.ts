export type SpreadsheetConditionalThresholdType =
  | 'min'
  | 'max'
  | 'num'
  | 'percent'
...
export function defaultSpreadsheetColorScaleThresholds(
  colorCount: number,
...
export function normalizeSpreadsheetConditionalVisualOptions(
  ...
  if (
    ...
    source.thresholds.length !== expectedThresholdCount
  )
    return null;
  const thresholds = source.thresholds.map(
    normalizeSpreadsheetConditionalThreshold,
  );
  if (thresholds.some((threshold) => !threshold)) return null;
  if (kind === 'colorScale')
    return { thresholds: thresholds as SpreadsheetConditionalThreshold[] };
...
export function spreadsheetConditionalThresholdValue(
  ...
  if (threshold.type === 'percent')
    return minimum + ((maximum - minimum) * percentage) / 100;
  const position =
    (Math.max(0, Math.min(100, percentage)) / 100) * (values.length - 1);
...
export function spreadsheetConditionalThresholdsEqual(
  ...
  return (
    ...
    left.every(
      (threshold, index) =>
        threshold.type === right[index]?.type &&
        threshold.value === right[index]?.value,
    )
  );
}
