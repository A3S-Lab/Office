...
export function spreadsheetDataLabelText(
  chart: WorkSpreadsheetChart,
  seriesIndex: number,
  pointIndex: number,
): string {
  ...
  if (labels.showSeriesName)
    parts.push(series.name || `系列 ${seriesIndex + 1}`);
  ...
  if (labels.showValue)
    parts.push(compactChartNumber(series.values[pointIndex]));
  if (labels.showPercentage) {
    const values = series.values.map((value) =>
      Math.max(0, finiteChartNumber(value)),
    );
    const total = values.reduce((sum, value) => sum + value, 0);
    if (total > 0)
      parts.push(
        `${compactChartNumber(Math.round((values[pointIndex] / total) * 1_000) / 10)}%`,
      );
  }
  if (labels.showBubbleSize)
    parts.push(compactChartNumber(series.bubbleSizes?.[pointIndex] ?? 0));
...
function placeDataLabel(
  ...
  if (geometry.kind === 'verticalBar')
    return placeVerticalBarLabel(geometry, position);
  if (geometry.kind === 'horizontalBar')
    return placeHorizontalBarLabel(geometry, position);
  if (geometry.kind === 'circular')
    return placeCircularLabel(geometry, position);
  if (position === 'left')
    return { x: geometry.x - 6, y: geometry.y + 3, anchor: 'end' };
  if (position === 'right')
    return { x: geometry.x + 6, y: geometry.y + 3, anchor: 'start' };
  if (position === 'below')
    return { x: geometry.x, y: geometry.y + 13, anchor: 'middle' };
  if (
    position === 'center' ||
    position === 'insideBase' ||
    position === 'insideEnd'
...
function placeVerticalBarLabel(
  ...
  if (position === 'left')
    return { x: geometry.x - 5, y: geometry.valueY + 3, anchor: 'end' };
  if (position === 'right')
    return { x: geometry.x + 5, y: geometry.valueY + 3, anchor: 'start' };
  if (position === 'center') {
    return {
      x: geometry.x,
  ...
  if (position === 'below')
    return { x: geometry.x, y: geometry.valueY + 13, anchor: 'middle' };
  return {
    x: geometry.x,
...
function placeHorizontalBarLabel(
  ...
  const positive = geometry.value >= 0;
  if (position === 'above')
    return { x: geometry.valueX, y: geometry.y - 5, anchor: 'middle' };
  if (position === 'below')
    return { x: geometry.valueX, y: geometry.y + 12, anchor: 'middle' };
  if (position === 'center') {
    return {
  ...
  if (position === 'left')
    return { x: geometry.valueX - 6, y: geometry.y + 3, anchor: 'end' };
  if (position === 'right')
    return { x: geometry.valueX + 6, y: geometry.y + 3, anchor: 'start' };
  return {
