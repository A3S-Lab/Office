...
export function spreadsheetErrorBarAmounts(
  ...
  return values.map((value, index) => {
    ...
    const minus =
      errorBars.valueType === 'custom'
        ? finiteNonNegative(errorBars.minusValues?.[index])
        : amount;
    const plus =
      errorBars.valueType === 'custom'
        ? finiteNonNegative(errorBars.plusValues?.[index])
        : amount;
    return {
...
export function spreadsheetSeriesErrorBarBounds(
  ...
  const matching = (series.errorBars ?? []).filter(
    (source) =>
      normalizeWorkSpreadsheetErrorBars(source, chartType).direction ===
      direction,
  );
...
function sharedErrorAmount(
  ...
  if (errorBars.valueType === 'standardError') {
    ...
    return usableCount
      ? populationStandardDeviation(values) / Math.sqrt(usableCount)
      : 0;
  }
  return 0;
}
