...
export function spreadsheetFormulaSummary(
  content: WorkSpreadsheetContent,
): SpreadsheetFormulaSummary {
  return scanSpreadsheetFormulas(content).summary;
}

export function spreadsheetFormulaDiagnostics(
  content: WorkSpreadsheetContent,
): SpreadsheetFormulaDiagnostic[] {
  return spreadsheetFormulaAnalysis(content).diagnostics;
}

export function spreadsheetFormulaAnalysis(
  content: WorkSpreadsheetContent,
): SpreadsheetFormulaAnalysis {
...
function createSpreadsheetFormulaDiagnostics(
  ...
  if (settings.mode === 'manual') {
    diagnostics.push({
      ...
      title: '手动计算',
      message:
        '工作簿将以手动模式保存；Work 只在显式重算时刷新整簿或选区缓存，正在编辑的单个公式仍可能即时计算。',
      locations: [],
    });
...
function scanSpreadsheetFormulas(content: WorkSpreadsheetContent): FormulaScan {
  ...
  for (const sheet of content.sheets) {
    summary.normalizedSharedFormulaGroups +=
    ...
    for (const [row, cells] of (sheet.data ?? []).entries()) {
      for (const [column, cell] of cells.entries()) {
        ...
        const formula =
          sheet.formulaMetadata?.sourceFormulas?.[address] ?? cell.f;
    ...
    for (const range of sheet.formulaMetadata?.ranges ?? []) {
      ...
      if (conflict) {
        ...
        locations.conflicts.push(
          `${sheet.name}!${range.reference}（${conflict}）`,
        );
      }
    }
  }
