...
export function effectiveSpreadsheetCalculationSettings(
  ...
  return {
    ...
    maximumIterations: clampedInteger(
...
export function spreadsheetFormulaForXlsx(
  currentFormula: string,
  sourceFormula?: string,
): string {
  const current = currentFormula.trim().replace(/^=/, '');
  const source = sourceFormula?.trim().replace(/^=/, '');
  if (source && comparableFormula(source) === comparableFormula(current))
    return source;
...
export function spreadsheetFormulaRangeConflict(
  ...
  if (!anchor || !containsCell(bounds, anchor.row, anchor.column))
    return '锚点不在引用范围内';
...
export function formulaHasStructuredReference(formula: string): boolean {
  const withoutExternalReferences = formula
    .replace(/"(?:[^"]|"")*"/g, '""')
    .replace(/\[[^\]]+\][^!]*!/gi, '');
  return (
...
export function volatileSpreadsheetFormulaFunctions(formula: string): string[] {
  return spreadsheetFormulaFunctions(formula).filter((name) =>
    VOLATILE_FUNCTIONS.has(name),
  );
}
...
function parseSpreadsheetFormulaRange(
  reference: string,
): SpreadsheetRangeBounds | null {
  const normalized =
    reference.trim().replaceAll('$', '').split('!').at(-1) ?? '';
...
function parseSpreadsheetCellAddress(
  address: string,
): { row: number; column: number } | null {
  const match = /^([A-Z]{1,3})([1-9]\d*)$/i.exec(
...
function rangesOverlap(
  first: SpreadsheetRangeBounds,
  second: SpreadsheetRangeBounds | null,
): boolean {
...
function futureFunctionPrefixes(
  formula: string,
  sourceFormula?: string,
): string {
  const sourcePrefixes = new Map<string, string>();
  const sourceFunctions = (sourceFormula ?? '').replace(
    /"(?:[^"]|"")*"/g,
    '""',
  );
  for (const match of sourceFunctions.matchAll(
    /(_xlfn\.(?:_xlws\.)?)([A-Z][A-Z0-9_.]*)\s*(?=\()/gi,
  )) {
    if (match[1] && match[2])
      sourcePrefixes.set(match[2].toUpperCase(), match[1]);
  }
  return transformFormulaOutsideStrings(formula, (segment) =>
    segment.replace(
      /(?<![A-Z0-9_.])([A-Z][A-Z0-9_.]*)\s*(?=\()/gi,
      (match, functionName: string) => {
        const normalized = functionName.toUpperCase();
        const prefix =
          sourcePrefixes.get(normalized) ??
          FUTURE_FUNCTION_PREFIXES.get(normalized);
...
function transformFormulaOutsideStrings(
  formula: string,
