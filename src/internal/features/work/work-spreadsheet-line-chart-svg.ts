...
function stackedAreaPath(
  points: Array<
    SpreadsheetChartSeriesLayout['series'][number][number] & {
      x: number;
      startY: number;
      endY: number;
    }
  ...
  const upper = points.map(
    (point, index) =>
      `${index ? 'L' : 'M'} ${roundChartNumber(point.x)} ${roundChartNumber(point.endY)}`,
  );
  const lower = [...points]
    .reverse()
    .map(
      (point) =>
        `L ${roundChartNumber(point.x)} ${roundChartNumber(point.startY)}`,
    );
