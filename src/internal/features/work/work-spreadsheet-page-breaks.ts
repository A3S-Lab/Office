export function parseSpreadsheetRowPageBreaks(
  value: string,
  maximumRow: number,
): number[] | null {
...
export function formatSpreadsheetRowPageBreaks(
...
export function parseSpreadsheetColumnPageBreaks(
  value: string,
...
export function formatSpreadsheetColumnPageBreaks(
  breaks: number[] | undefined,
): string {
  return normalizedBreaks(breaks).map(encodeColumn).join(', ');
}

function parsePageBreakList(
  value: string,
  maximum: number,
