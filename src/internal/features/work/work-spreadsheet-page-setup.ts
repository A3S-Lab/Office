...
export function effectiveSpreadsheetPageSetup(
  ...
  return {
    ...
    orientation:
      pageSetup?.orientation === 'portrait' ? 'portrait' : 'landscape',
    ...
    pageOrder:
      pageSetup?.pageOrder === 'downThenOver' ? 'downThenOver' : 'overThenDown',
    ...
    margins: {
      ...
      header: boundedNumber(
        pageSetup?.margins?.header,
        0,
        100,
        DEFAULT_MARGINS.header,
      ),
      footer: boundedNumber(
        pageSetup?.margins?.footer,
        0,
        100,
        DEFAULT_MARGINS.footer,
      ),
    },
  };
}

export function normalizeSpreadsheetPaperSize(
  value: unknown,
): WorkSpreadsheetPaperSize {
  switch (value) {
    case 'a3':
...
export function spreadsheetPageCapacity(
  ...
  const pageHeight =
    pageSetup.orientation === 'landscape' ? paper.width : paper.height;
  const printableWidth = Math.max(
    10,
    pageWidth - pageSetup.margins.left - pageSetup.margins.right,
  );
  const printableHeight = Math.max(
    10,
    pageHeight - pageSetup.margins.top - pageSetup.margins.bottom,
  );
  const scaleFactor = pageSetup.fitToPage ? 1 : 100 / pageSetup.scale;
  return {
    rows: Math.max(
      1,
      Math.floor(34 * (printableHeight / BASE_PRINTABLE_HEIGHT) * scaleFactor),
    ),
    columns: Math.max(
      1,
      Math.floor(10 * (printableWidth / BASE_PRINTABLE_WIDTH) * scaleFactor),
    ),
  };
}
...
export function fitSpreadsheetAxisCapacity(
  ...
  const titleLength = titleRange
    ? Math.max(0, titleRange[1] - titleRange[0] + 1)
    : 0;
