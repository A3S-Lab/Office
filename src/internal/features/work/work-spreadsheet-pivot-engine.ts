...
export function spreadsheetPivotFilterItemsFromSource(
  ...
  if (
    fieldIndex < 0 ||
    fieldIndex > sourceBounds.endColumn - sourceBounds.startColumn
  )
    return [];
  const items = new Map<string, SpreadsheetPivotFilterItem>();
  for (
    let row = sourceBounds.startRow + 1;
    row <= sourceBounds.endRow;
    row += 1
  ) {
    const value = normalizeSpreadsheetPivotFilterValue(
      spreadsheetPivotCellValue(
        sourceSheet.data?.[row]?.[sourceBounds.startColumn + fieldIndex],
      ),
    );
...
export function buildSpreadsheetPivotOutput(
  ...
  if (!pivot.columnFields.length)
    columnCombinations.set(singleColumn.key, singleColumn);
  ...
  for (
    ...
    const values = fields.map((field) =>
      spreadsheetPivotCellValue(
        sourceSheet.data?.[row]?.[sourceBounds.startColumn + field.index],
      ),
    );
    if (
      values.every(
    ...
    const rowCombination = combination(
      pivot.rowFields.map((fieldIndex) => values[fieldIndex]),
    );
    ...
    for (const [valueIndex, valueField] of pivot.values.entries()) {
      ...
      updateAccumulator(
        mapAccumulator(
          cells,
          cellKey(rowCombination.key, columnCombination.key, valueIndex),
        ),
        value,
      );
      updateAccumulator(
        mapAccumulator(rowTotals, cellKey(rowCombination.key, '', valueIndex)),
        value,
      );
      updateAccumulator(
        mapAccumulator(
          columnTotals,
          cellKey('', columnCombination.key, valueIndex),
        ),
        value,
      );
  ...
  const sortedRows = Array.from(rowCombinations.values()).sort(
    compareCombinations,
  );
  const sortedColumns = Array.from(columnCombinations.values()).sort(
    compareCombinations,
  );
  ...
  const headers = [
    ...
    ...sortedColumns.flatMap((column) =>
      pivot.values.map((value) =>
        columnValueCaption(column, pivot.columnFields, fields, value),
      ),
    ),
  ...
  for (const row of sortedRows) {
    const result: Array<Cell | null> = row.values.map((value) =>
      pivotDimensionCell(value),
    );
    for (const column of sortedColumns) {
      for (const [valueIndex, value] of pivot.values.entries()) {
        result.push(
          pivotValueCell(
            accumulatorValue(
              cells.get(cellKey(row.key, column.key, valueIndex)),
              value.aggregation,
            ),
            false,
          ),
    ...
    if (pivot.rowGrandTotals && pivot.columnFields.length) {
      for (const [valueIndex, value] of pivot.values.entries()) {
        result.push(
          pivotValueCell(
            accumulatorValue(
              rowTotals.get(cellKey(row.key, '', valueIndex)),
              value.aggregation,
            ),
            true,
          ),
  ...
  if (pivot.columnGrandTotals) {
    ...
    for (const column of sortedColumns) {
      for (const [valueIndex, value] of pivot.values.entries()) {
        result.push(
          pivotValueCell(
            accumulatorValue(
              columnTotals.get(cellKey('', column.key, valueIndex)),
              value.aggregation,
            ),
            true,
          ),
    ...
    if (pivot.rowGrandTotals && pivot.columnFields.length) {
      for (const [valueIndex, value] of pivot.values.entries()) {
        result.push(
          pivotValueCell(
            accumulatorValue(totals[valueIndex], value.aggregation),
            true,
          ),
        );
      }
...
function updateAccumulator(state: PivotAccumulator, value: unknown): void {
  ...
  state.minimum =
    state.minimum === undefined ? number : Math.min(state.minimum, number);
  state.maximum =
    state.maximum === undefined ? number : Math.max(state.maximum, number);
...
function accumulatorValue(
  ...
  if (aggregation === 'average')
    return state.numberCount ? state.sum / state.numberCount : null;
  if (aggregation === 'max') return state.maximum ?? null;
  if (aggregation === 'min') return state.minimum ?? null;
  if (aggregation === 'product')
    return state.numberCount ? state.product : null;
  if (aggregation === 'stdDev')
    return state.numberCount > 1
      ? Math.sqrt(state.m2 / (state.numberCount - 1))
      : null;
  if (aggregation === 'stdDevP')
    return state.numberCount ? Math.sqrt(state.m2 / state.numberCount) : null;
  if (aggregation === 'var')
    return state.numberCount > 1 ? state.m2 / (state.numberCount - 1) : null;
  if (aggregation === 'varP')
    return state.numberCount ? state.m2 / state.numberCount : null;
  return state.sum;
}
...
function compareCombinations(
  left: PivotCombination,
  right: PivotCombination,
): number {
  const count = Math.max(left.values.length, right.values.length);
  for (let index = 0; index < count; index += 1) {
    const difference = displaySpreadsheetPivotValue(
      left.values[index],
    ).localeCompare(
...
function columnValueCaption(
  ...
  const dimensions = column.values.map((item, index) => {
    ...
    return columnFields.length > 1
      ? `${fields[columnFields[index]].name}: ${label}`
      : label;
  });
  return [...dimensions, valueCaption(value, fields)]
    .filter(Boolean)
    .join(' · ');
}
...
function matchesPivotReportFilters(
  values: unknown[],
  pivot: WorkSpreadsheetPivotTable,
): boolean {
