...
export function spreadsheetPivotCount(content: WorkSpreadsheetContent): number {
  return content.sheets.reduce(
    (count, sheet) => count + (sheet.pivotTables?.length ?? 0),
    0,
  );
}
...
export function spreadsheetPivotFields(
  ...
  for (let column = range.startColumn; column <= range.endColumn; column += 1) {
    ...
    for (let row = range.startRow + 1; row <= range.endRow; row += 1) {
      const value = spreadsheetPivotCellValue(
        sourceSheet.data?.[row]?.[column],
      );
...
export function spreadsheetPivotFilterItems(
  ...
  if (
    ...
    !range ||
    fieldIndex < 0 ||
    fieldIndex > range.endColumn - range.startColumn
  )
    return [];
  ...
  draftPivot.values = [
    {
      ...
      caption: defaultPivotValueCaption(
        valueField.name,
        valueField.numeric ? 'sum' : 'counta',
      ),
    },
  ];
...
export function refreshSpreadsheetPivotTables(
  content: WorkSpreadsheetContent,
): WorkSpreadsheetContent {
  ...
  for (const ownerSheet of sheets) {
    ...
    for (const pivot of ownerSheet.pivotTables) {
      ...
      ownerSheet.row = Math.max(
        ownerSheet.row ?? 0,
        resolved.outputBounds.endRow + 1,
        data!.length,
      );
      ownerSheet.column = Math.max(
        ownerSheet.column ?? 0,
        resolved.outputBounds.endColumn + 1,
      );
    }
  }
  return next;
...
export function deleteSpreadsheetPivotTable(
  ...
  const sheets = content.sheets.map((sheet) => {
    ...
    const pivot = sheet.pivotTables?.find(
      (candidate) => candidate.id === pivotId,
    );
    if (!pivot) return sheet;
    const data = cloneCellMatrix(sheet.data);
    clearRange(data, singleRange(pivot.outputReference ?? ''));
    const pivotTables = (sheet.pivotTables ?? []).filter(
      (candidate) => candidate.id !== pivotId,
    );
    return {
      ...sheet,
...
export function spreadsheetPivotOutputContains(
...
function resolvePivot(
  ...
  if (
    outputBounds.endRow > MAXIMUM_XLSX_ROW ||
    outputBounds.endColumn > MAXIMUM_XLSX_COLUMN
  )
    return null;
  if (
    ownerSheet.id === sourceSheet.id &&
    rangesOverlap(sourceBounds, outputBounds)
  )
    return null;
  ...
  for (let row = outputBounds.startRow; row <= outputBounds.endRow; row += 1) {
    for (
      let column = outputBounds.startColumn;
      column <= outputBounds.endColumn;
      column += 1
    ) {
...
function pivotFailure(
  ...
  const sourceCellCount =
    (sourceBounds.endRow - sourceBounds.startRow + 1) *
    (sourceBounds.endColumn - sourceBounds.startColumn + 1);
  ...
  if (
    ...
    reportFilterFields.some(
      (index) =>
        pivot.rowFields.includes(index) || pivot.columnFields.includes(index),
    )
  ) {
    return invalid(
...
function nextSheetName(content: WorkSpreadsheetContent, base: string): string {
  const existing = new Set(
    content.sheets.map((sheet) => sheet.name.toLocaleLowerCase()),
  );
...
function nextPivotName(content: WorkSpreadsheetContent): string {
  const existing = new Set(
    content.sheets.flatMap((sheet) =>
      (sheet.pivotTables ?? []).map((pivot) => pivot.name.toLocaleLowerCase()),
    ),
  );
  let index = 1;
