...
const CELL_OR_RANGE = new RegExp(
  `^${CELL_REFERENCE.source}(?::${CELL_REFERENCE.source})?$`,
  'i',
);
...
export function isValidSpreadsheetDefinedName(value: string): boolean {
  ...
  if (
    ...
    return false;
  if (/^[A-Z]{1,3}[1-9]\d*$/i.test(name) || /^R\d+C\d+$/i.test(name))
    return false;
  return true;
}
...
export function normalizeSpreadsheetPrintArea(value: string): string | null {
  ...
  const normalized = parts.map((part) => {
    ...
    if (
      !CELL_OR_RANGE.test(reference) &&
      !COLUMN_RANGE.test(reference) &&
      !ROW_RANGE.test(reference)
    )
      return null;
    return reference.replace(/[A-Z]+/gi, (column) => column.toUpperCase());
  });
  return normalized.every((part): part is string => Boolean(part))
    ? normalized.join(',')
    : null;
}

export function normalizeSpreadsheetPrintTitleRows(
  value: string,
): string | null {
  const match = ROW_RANGE.exec(
    unqualifiedReference(value.trim().replace(/^=/, '')),
  );
...
export function normalizeSpreadsheetPrintTitleColumns(
  value: string,
): string | null {
  const match = COLUMN_RANGE.exec(
    unqualifiedReference(value.trim().replace(/^=/, '')),
  );
  if (!match) return null;
...
export function parseSpreadsheetPrintTitles(
  value: string,
...
export function spreadsheetPrintTitleBounds(
  ...
  const rows = rowReference
    ? normalizeSpreadsheetPrintTitleRows(rowReference)
    : null;
  const columns = columnReference
    ? normalizeSpreadsheetPrintTitleColumns(columnReference)
    : null;
  if (
    (rowReference && !rows) ||
  ...
  return {
    rows: rowMatch
      ? [
          clamp(Number(rowMatch[1]) - 1, maximumRow),
          clamp(Number(rowMatch[2]) - 1, maximumRow),
        ]
      : undefined,
    columns: columnMatch
      ? [
          clamp(decodeColumn(columnMatch[1]), maximumColumn),
          clamp(decodeColumn(columnMatch[2]), maximumColumn),
        ]
      : undefined,
  };
}

export function parseSpreadsheetCellRanges(
  value: string,
  ...
  const ranges = parts.map((part) => {
    ...
    const second =
      cells[3] && cells[4] ? decodeCell(cells[3], cells[4]) : first;
    return {
      row: [
  ...
  return ranges.every((range): range is SpreadsheetCellRange => Boolean(range))
    ? ranges
    : null;
}

export function formatSpreadsheetCellRanges(
...
export function qualifySpreadsheetRange(
  value: string,
  sheetName: string,
): string {
  ...
  return splitRangeList(value)
    .map((part) =>
      part.includes('!') ? part.trim() : `${prefix}${part.trim()}`,
    )
    .join(',');
}

export function stripSpreadsheetSheetQualifier(
  value: string,
  sheetName: string,
): string {
...
export function spreadsheetPrintBounds(
    ...
    if (columns) {
      ranges.push({
        ...
        startColumn: Math.min(
          decodeColumn(columns[1]),
          decodeColumn(columns[2]),
        ),
  ...
  return {
    startRow: clamp(
      Math.min(...ranges.map((range) => range.startRow)),
      maximumRow,
    ),
    endRow: clamp(Math.max(...ranges.map((range) => range.endRow)), maximumRow),
    startColumn: clamp(
      Math.min(...ranges.map((range) => range.startColumn)),
      maximumColumn,
    ),
    endColumn: clamp(
      Math.max(...ranges.map((range) => range.endColumn)),
      maximumColumn,
    ),
  };
}
