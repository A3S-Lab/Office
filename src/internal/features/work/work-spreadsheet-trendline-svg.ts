...
import {
  fitSpreadsheetTrendline,
  type SpreadsheetTrendlineFit,
} from './work-spreadsheet-trendlines';
...
export function spreadsheetSeriesTrendlinesSvg(
  ...
  return spreadsheetSeriesTrendlineFits(series, xValues)
    .map(({ sourceIndex, fit }) => {
      const trendline = normalizeWorkSpreadsheetTrendline(
        series.trendlines?.[sourceIndex] ?? { type: 'linear' },
      );
      ...
      const path = points
        ...
        .join(' ');
      const equation =
        trendline.displayEquation && fit.equation ? fit.equation : undefined;
      const rSquared =
        trendline.displayRSquared && fit.rSquared !== undefined
          ? roundChartNumber(fit.rSquared)
          : undefined;
      const label = [equation, rSquared === undefined ? '' : `R² = ${rSquared}`]
        .filter(Boolean)
        .join(' · ');
      ...
      return [
        `<g data-trendline-series="${seriesIndex}:${sourceIndex}" data-trendline-type="${trendline.type}"${
          trendline.name
            ? ` data-trendline-name="${escapeChartXml(trendline.name)}"`
            : ''
        }${equation ? ` data-trendline-equation="${escapeChartXml(equation)}"` : ''}${
          rSquared === undefined
            ? ''
            : ` data-trendline-r-squared="${rSquared}"`
        }>`,
