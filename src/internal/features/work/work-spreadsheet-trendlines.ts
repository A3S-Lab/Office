...
export function fitSpreadsheetTrendline(
  ...
  if (trendline.type === 'movingAverage')
    return movingAverageFit(observed, trendline.period ?? 2);
  const model = regressionModel(observed, trendline);
  if (!model) return null;
  const minimum =
    Math.min(...observed.map((point) => point.x)) - (trendline.backward ?? 0);
  const maximum =
    Math.max(...observed.map((point) => point.x)) + (trendline.forward ?? 0);
...
function regressionModel(
  ...
  if (trendline.type === 'linear')
    return polynomialModel(observed, 1, trendline.intercept);
  if (trendline.type === 'polynomial') {
    return polynomialModel(
      observed,
      Math.min(trendline.order ?? 2, observed.length - 1),
      trendline.intercept,
    );
  }
  if (trendline.type === 'exponential') {
    const usable = observed.filter((point) => point.y > 0);
    const coefficients = linearCoefficients(
      usable.map((point) => ({ x: point.x, y: Math.log(point.y) })),
    );
  ...
  if (trendline.type === 'logarithmic') {
    ...
    const coefficients = linearCoefficients(
      usable.map((point) => ({ x: Math.log(point.x), y: point.y })),
    );
    if (!coefficients) return null;
    const [intercept, slope] = coefficients;
    return {
  ...
  if (trendline.type === 'power') {
    ...
    const coefficients = linearCoefficients(
      usable.map((point) => ({ x: Math.log(point.x), y: Math.log(point.y) })),
    );
    if (!coefficients) return null;
...
function polynomialModel(
  ...
  const matrix = Array.from({ length: coefficientCount }, (_, row) =>
    Array.from({ length: coefficientCount }, (_, column) =>
      observed.reduce(
        (sum, point) => sum + point.x ** (row + column + startPower * 2),
        0,
      ),
    ),
  );
  const vector = Array.from({ length: coefficientCount }, (_, row) =>
    observed.reduce(
      (sum, point) =>
        sum + point.x ** (row + startPower) * (point.y - (fixedIntercept ?? 0)),
      0,
    ),
  );
  ...
  const coefficients =
    fixedIntercept === undefined ? solved : [fixedIntercept, ...solved];
  ...
  const points = observed.slice(period - 1).map((point, index) => ({
    ...
    y:
      observed
        ...
        .reduce((sum, item) => sum + item.y, 0) / period,
  }));
...
function coefficientOfDetermination(
  ...
  const mean =
    usable.reduce((sum, point) => sum + point.actual, 0) / usable.length;
  const total = usable.reduce(
    (sum, point) => sum + (point.actual - mean) ** 2,
    0,
  );
  const residual = usable.reduce(
