...
export function spreadsheetXyChartAxesSvg(
  ...
  const xGridlines = spreadsheetChartAxisGridlinesVisible(
    xAxis,
    chart.type,
    'bottom',
  );
  const yGridlines = spreadsheetChartAxisGridlinesVisible(
    yAxis,
    chart.type,
    'left',
  );
  const xTicks = xBounds.ticks
    .map((value, index) => {
      const x =
        plot.x +
        spreadsheetChartAxisValueRatio(value, xBounds, xAxis) * plot.width;
  ...
  const yTicks = yBounds.ticks
    .map((value, index) => {
      const y =
        plot.y +
        (1 - spreadsheetChartAxisValueRatio(value, yBounds, yAxis)) *
          plot.height;
      const label = spreadsheetChartAxisLabelLayout(
        yAxis,
        chart.type,
