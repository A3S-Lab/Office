...
export function parseXlsxChartAxes(
  ...
  for (const axis of directChildren(plotArea).filter(
    (node) =>
      node.localName === 'catAx' ||
      node.localName === 'valAx' ||
      node.localName === 'dateAx',
  )) {
    ...
    const valueSettings =
      axis.localName === 'valAx'
        ? parseXlsxValueAxisSettings(
            axis,
            chartType,
            key,
            preserveVisibleDefaultGridlines,
          )
        : undefined;
    if (!text && !titleReference && !displaySettings && !valueSettings)
      continue;
    axes[key] = {
      ...(text ? { title: text } : {}),
...
function parseXlsxAxisDisplaySettings(
  ...
  const orientation = attribute(
    directChild(directChild(axis, 'scaling') ?? axis, 'orientation') ?? axis,
    'val',
  );
  const rawLabelPosition = attribute(
    directChild(axis, 'tickLblPos') ?? axis,
    'val',
  );
  const labelPosition =
    ...
    rawLabelPosition === 'none'
      ? rawLabelPosition
      : undefined;
  const rawTickMark = attribute(
    directChild(axis, 'majorTickMark') ?? axis,
    'val',
  );
  const majorTickMark =
    rawTickMark === 'in'
      ...
      : rawTickMark === 'out'
        ...
        : rawTickMark === 'cross'
          ...
          : rawTickMark === 'none'
            ? 'none'
            : undefined;
  const rawInterval = workSpreadsheetChartAxisIsCategoryAxis(
    chartType,
    position,
  )
  ...
  const labelInterval =
    ...
    Number(rawInterval) >= 1 &&
    Number(rawInterval) <= 31_999
      ? Number(rawInterval)
      : undefined;
  const defaultLabelPosition = workSpreadsheetChartAxisDefaultLabelPosition(
    chartType,
    position,
  );
...
export function chartTextTitleXml(
  title: string,
...
function chartAxisScalingXml(
  axis: WorkSpreadsheetChartAxis | undefined,
  includeBounds: boolean,
): string {
  return [
    `<c:scaling><c:orientation val="${axis?.reverseOrder === true ? 'maxMin' : 'minMax'}"/>`,
    includeBounds && Number.isFinite(axis?.maximum)
      ? `<c:max val="${Number(axis?.maximum)}"/>`
      : '',
    includeBounds && Number.isFinite(axis?.minimum)
      ? `<c:min val="${Number(axis?.minimum)}"/>`
      : '',
    '</c:scaling>',
  ].join('');
}
...
function chartAxisMajorTickMarkXml(
  axis: WorkSpreadsheetChartAxis | undefined,
): string {
  const value = axis?.majorTickMark;
  if (!value || value === 'none') return '';
  const nativeValue =
    value === 'inside' ? 'in' : value === 'outside' ? 'out' : 'cross';
  return `<c:majorTickMark val="${nativeValue}"/>`;
}

function chartCategoryAxisLabelIntervalXml(
  axis: WorkSpreadsheetChartAxis | undefined,
): string {
  return Number.isInteger(axis?.labelInterval) &&
    Number(axis?.labelInterval) >= 1
...
export function chartAxesXml(
  ...
  if (chart.type === 'combination') {
    return combinationChartAxesXml(
      chart,
      categoryAxisId,
      valueAxisId,
  ...
  const categoryAxisPosition: WorkSpreadsheetChartAxisPosition = horizontalBars
    ? 'left'
    : 'bottom';
  const categoryAxisTitle = chartAxisTitleXml(categoryAxis);
  const valueAxisPosition: WorkSpreadsheetChartAxisPosition = horizontalBars
    ? 'bottom'
    : 'left';
...
function parseXlsxValueAxisSettings(
  ...
  const minimum = finiteAttributeValue(
    scaling ? directChild(scaling, 'min') : null,
  );
  const maximum = finiteAttributeValue(
    scaling ? directChild(scaling, 'max') : null,
  );
  const majorUnit = finiteAttributeValue(directChild(axis, 'majorUnit'));
  const showMajorGridlines =
    directChildren(axis, 'majorGridlines').length === 1;
  const defaultGridlines = workSpreadsheetChartAxisShowsMajorGridlinesByDefault(
    chartType,
    position,
  );
  const numberFormatNode = directChild(axis, 'numFmt');
  const numberFormat = attribute(
    numberFormatNode ?? axis,
    'formatCode',
  )?.trim();
  ...
  const settings: WorkSpreadsheetChartAxis = {
    ...
    ...(showMajorGridlines !== defaultGridlines ||
    (showMajorGridlines && preserveVisibleDefaultGridlines)
      ? { showMajorGridlines }
      : {}),
    ...(!defaultNumberFormat && numberFormat ? { numberFormat } : {}),
    ...(!defaultNumberFormat && numberFormatSourceLinked !== undefined
...
function chartValueAxisGridlinesXml(
  ...
  const visible =
    ...
    workSpreadsheetChartAxisShowsMajorGridlinesByDefault(chartType, position);
  return visible ? '<c:majorGridlines/>' : '';
}

function chartValueAxisNumberFormatXml(
  axis: WorkSpreadsheetChartAxis | undefined,
): string {
...
function chartValueAxisMajorUnitXml(
  axis: WorkSpreadsheetChartAxis | undefined,
): string {
...
function combinationChartAxesXml(
  ...
  const hasSecondary = chart.series.some(
    (series) =>
      normalizeWorkSpreadsheetChartAxisGroup(series.axisGroup) === 'secondary',
  );
  ...
  const secondaryCategoryLabelPosition =
    secondaryCategoryAxis?.labelPosition ??
    workSpreadsheetChartAxisDefaultLabelPosition(chart.type, 'top');
  const secondary = [
    '<c:catAx>',
