...
export async function diagnoseXlsxCharts(
  archive: OoxmlPackage,
): Promise<WorkCompatibilityIssue[]> {
  const chartPaths = archive
    .paths('xl/charts/')
    .filter((path) => /^xl\/charts\/chart\d+\.xml$/i.test(path));
  ...
  for (const path of chartPaths) {
    ...
    const chartNodes = plotArea
      ? directChildren(plotArea).filter((element) =>
          /Chart$/i.test(element.localName),
        )
      : [];
    const supportedCombination =
      isSupportedXlsxCombinationChartNodes(chartNodes);
    if (chartNodes.length > 1 && !supportedCombination) combinations += 1;
    if (
      ((chartNodes.length === 1 &&
        SUPPORTED_CHART_ELEMENTS.has(chartNodes[0].localName)) ||
    ...
    if (chartNodes.some((node) => !isSupportedXlsxChartPlotLayout(node)))
      advancedPlotLayout += 1;
    ...
    if (
      ...
      descendants(chart, 'f').some(
        (formula) => !isSupportedChartReference(formula.textContent ?? ''),
      )
    ) {
...
async function anchoredChartPaths(archive: OoxmlPackage): Promise<Set<string>> {
  ...
  for (const path of drawings) {
    ...
    for (const frame of descendants(document, 'graphicFrame')) {
      ...
      const relationshipId = reference
        ? (attribute(reference, 'r:id') ?? attribute(reference, 'id'))
        : null;
...
function hasAdvancedChartFormatting(
  document: Document,
  chartNodes: Element[],
): boolean {
  if (
    [
      ...
      'sideWall',
      'backWall',
    ].some((name) => descendants(document, name).length > 0) ||
  ...
  for (const chart of chartNodes) {
    for (const series of directChildren(chart, 'ser')) {
      if (!isSupportedXlsxChartSeriesFormatting(series, chart.localName))
        return true;
      for (const shape of xlsxChartSeriesFormattingShapeProperties(series))
        supportedShapeProperties.add(shape);
    }
  }
  return descendants(document, 'spPr').some(
    (shape) => !supportedShapeProperties.has(shape),
  );
}
...
function isSupportedChartReference(value: string): boolean {
  ...
  return /^(?:(?:'(?:(?:[^']|'')+)'|[^!]+)!)?\$?[A-Z]{1,3}\$?[1-9]\d*(?::\$?[A-Z]{1,3}\$?[1-9]\d*)?$/i.test(
    formula,
  );
}
...
function unsupportedTrendlineCount(chartNodes: Element[]): number {
  return chartNodes.reduce((count, chart) => {
    const supportsTrendlines =
      TRENDLINE_CHART_ELEMENTS.has(chart.localName) &&
      !xlsxChartNodeUsesStackedGrouping(chart);
    const trendlines = directChildren(chart, 'ser').flatMap((series) =>
      directChildren(series, 'trendline'),
    );
    return (
      count +
      trendlines.filter(
        (trendline) => !supportsTrendlines || !isSupportedTrendline(trendline),
      ).length
    );
  }, 0);
}
...
function unsupportedDataLabelCount(chartNodes: Element[]): number {
  return chartNodes.reduce((count, chart) => {
    const dataLabels = [
      ...
      ...directChildren(chart, 'ser').flatMap((series) =>
        directChildren(series, 'dLbls'),
      ),
    ];
    return (
      count +
      dataLabels.filter(
        (labels) => !isSupportedDataLabels(labels, chart.localName),
      ).length
    );
  }, 0);
}
...
function unsupportedErrorBarCount(chartNodes: Element[]): number {
  return chartNodes.reduce((count, chart) => {
    const supportsErrorBars =
      ERROR_BAR_CHART_ELEMENTS.has(chart.localName) &&
      !xlsxChartNodeUsesStackedGrouping(chart);
    const seriesNodes = directChildren(chart, 'ser');
    const directErrorBars = seriesNodes.flatMap((series) =>
      directChildren(series, 'errBars'),
    );
    const misplacedErrorBars = Math.max(
      0,
      descendants(chart, 'errBars').length - directErrorBars.length,
    );
    const unsupported = seriesNodes.reduce((seriesCount, series) => {
      ...
      return (
        ...
        directChildren(series, 'errBars').filter((errorBars) => {
          ...
          const duplicateDirection = Boolean(
          ...
          return (
            duplicateDirection ||
            !supportsErrorBars ||
            !isSupportedErrorBars(errorBars, chart.localName)
          );
        }).length
      );
    }, 0);
...
function isSupportedErrorBars(
  errorBars: Element,
  chartLocalName: string,
): boolean {
  const children = directChildren(errorBars);
  if (children.some((child) => !ERROR_BAR_CHILDREN.has(child.localName)))
    return false;
  const direction = errorBarDirection(errorBars);
  if (!direction) return false;
  if (
    direction === 'x' &&
    chartLocalName !== 'scatterChart' &&
    chartLocalName !== 'bubbleChart'
  )
    return false;
  ...
  if (barType !== 'both' && barType !== 'plus' && barType !== 'minus')
    return false;
  ...
  const usesScalarValue =
    valueType === 'fixedVal' ||
    valueType === 'percentage' ||
    valueType === 'stdDev';
  if (
    values.length &&
    (!usesScalarValue || !validNonNegativeNumberAttribute(values[0]))
  )
    return false;
  ...
  if (
    barType !== 'minus' &&
    (plus.length !== 1 || !isSupportedErrorBarSource(plus[0]))
  )
    return false;
  if (barType === 'minus' && plus.length) return false;
  if (
    barType !== 'plus' &&
    (minus.length !== 1 || !isSupportedErrorBarSource(minus[0]))
  )
    return false;
...
function isSupportedErrorBarSource(source: Element): boolean {
  ...
  if (
    children.length !== 1 ||
    (children[0].localName !== 'numRef' && children[0].localName !== 'numLit')
  ) {
    return false;
  }
  ...
  if (data.localName === 'numRef') {
    ...
    if (
      dataChildren.some(
        (child) => child.localName !== 'f' && child.localName !== 'numCache',
      )
    )
      return false;
...
function isSupportedErrorBarNumberCache(
  cache: Element,
  ...
  if (
    children.some(
      (child) => !['formatCode', 'ptCount', 'pt'].includes(child.localName),
    )
  )
    return false;
  if (
    directChildren(cache, 'formatCode').length > 1 ||
    directChildren(cache, 'ptCount').length > 1
  )
    return false;
...
function isSupportedDataLabels(
  labels: Element,
  chartLocalName: string,
): boolean {
  ...
  if (
    children.some(
      (child) => !SUPPORTED_DATA_LABEL_CHILDREN.has(child.localName),
    )
  )
    return false;
  if (
    ['spPr', 'txPr', 'separator'].some(
      (name) => directChildren(labels, name).length > 1,
    )
  )
    return false;
  ...
  if (
    positions.length > 1 ||
    (positions.length === 1 &&
      !SUPPORTED_DATA_LABEL_POSITIONS.has(attribute(positions[0], 'val') ?? ''))
  ) {
    return false;
  }
  const booleanNames = [
  ...
  if (booleanNames.some((name) => !validOptionalBooleanAttribute(labels, name)))
    return false;
  if (
    booleanElementIsTrue(labels, 'showLegendKey') ||
    booleanElementIsTrue(labels, 'showLeaderLines')
  )
    return false;
  if (
  ...
  return (
    !booleanElementIsTrue(labels, 'showBubbleSize') ||
    chartLocalName === 'bubbleChart'
  );
}
...
function isSupportedTrendline(trendline: Element): boolean {
  ...
  if (
    children.some((child) => !SUPPORTED_TRENDLINE_CHILDREN.has(child.localName))
  )
    return false;
  if (
    directChildren(trendline, 'name').length > 1 ||
    directChildren(trendline, 'spPr').length > 1
  )
    return false;
  ...
  if (type === 'poly') {
    if (order.length !== 1 || !validIntegerAttribute(order[0], 2, 6))
      return false;
  ...
  if (type === 'movingAvg') {
    if (period.length !== 1 || !validIntegerAttribute(period[0], 2, 255))
      return false;
  ...
  if (
    !validOptionalNumberAttribute(trendline, 'forward', (value) => value >= 0)
  )
    return false;
  if (
    !validOptionalNumberAttribute(trendline, 'backward', (value) => value >= 0)
  )
    return false;
  if (!validOptionalNumberAttribute(trendline, 'intercept', () => true))
    return false;
  ...
  return !labels.some((label) =>
    ['manualLayout', 'tx', 'numFmt'].some(
      (name) => descendants(label, name).length > 0,
    ),
  );
}

function validIntegerAttribute(
  element: Element,
  minimum: number,
  maximum: number,
): boolean {
...
function validOptionalBooleanAttribute(
  parent: Element,
  localName: string,
): boolean {
  const elements = directChildren(parent, localName);
...
function hasAdvancedAxes(document: Document, chartNodes: Element[]): boolean {
  if (
    descendants(document, 'serAx').length ||
    descendants(document, 'dateAx').length
  )
    return true;
  ...
  if (isSupportedXlsxCombinationChartNodes(chartNodes))
    return valueAxes > 2 || categoryAxes > 2;
...
function unsupportedAxisTitleCount(
  document: Document,
  chartNodes: Element[],
): number {
  ...
  return axes.reduce((count, axis) => {
    ...
    const position =
      positionElements.length === 1
        ? attribute(positionElements[0], 'val')
        : null;
    const positionSupported =
      position === 'b' ||
      position === 'l' ||
      (supportsSecondaryTitles && (position === 't' || position === 'r'));
...
function unsupportedValueAxisSettingsCount(document: Document): number {
  ...
  return directChildren(plotArea, 'valAx').filter(
    (axis) => !isSupportedValueAxisSettings(axis, hasCategoryAxes),
  ).length;
}

function isSupportedValueAxisSettings(
  axis: Element,
  hasCategoryAxes: boolean,
): boolean {
  ...
  if (scaling.length === 1) {
    ...
    if (
      children.some(
        (child) => !['orientation', 'max', 'min'].includes(child.localName),
      )
    )
      return false;
    ...
    if (
      ...
      orientation.some(
        (item) => !['minMax', 'maxMin'].includes(attribute(item, 'val') ?? ''),
      )
    ) {
      return false;
    }
  ...
  if (
    !majorUnit.valid ||
    (majorUnit.value !== undefined && majorUnit.value <= 0)
  )
    return false;
  ...
  if (
    ...
    majorTicks.some(
      (tick) =>
        !['none', 'in', 'out', 'cross'].includes(attribute(tick, 'val') ?? ''),
    )
  ) {
    return false;
  }
  ...
  if (
    minorTicks.length > 1 ||
    minorTicks.some((tick) => attribute(tick, 'val') !== 'none')
  )
    return false;
  const tickLabelPositions = directChildren(axis, 'tickLblPos');
  if (
    tickLabelPositions.length > 1 ||
    tickLabelPositions.some(
      (position) =>
        !['nextTo', 'high', 'low', 'none'].includes(
          attribute(position, 'val') ?? '',
        ),
    )
  ) {
    return false;
  ...
  const axisPosition = attribute(
    directChildren(axis, 'axPos')[0] ?? axis,
    'val',
  );
  ...
  if (
    crossBetween.length > 1 ||
    crossBetween.some(
      (crossing) => attribute(crossing, 'val') !== expectedCrossBetween,
    )
  ) {
    return false;
  }
  const deletion = directChildren(axis, 'delete');
  if (deletion.length > 1 || deletion.some(booleanAttributeIsTrue))
    return false;
...
function unsupportedCategoryAxisSettingsCount(document: Document): number {
  const plotArea = firstDescendant(document, 'plotArea');
  if (!plotArea) return 0;
  return directChildren(plotArea, 'catAx').filter(
    (axis) => !isSupportedCategoryAxisSettings(axis),
  ).length;
}
...
function isSupportedCategoryAxisSettings(axis: Element): boolean {
  ...
  if (scaling.length === 1) {
    const children = directChildren(scaling[0]);
    if (children.some((child) => child.localName !== 'orientation'))
      return false;
  ...
  if (
    ...
    labelIntervals.some((item) => !validIntegerAttribute(item, 1, 31_999))
  ) {
    return false;
  }
  if (
    [
      ...
      'noMultiLvlLbl',
      'extLst',
    ].some((name) => directChildren(axis, name).length > 0)
  ) {
    return false;
  }
  const crosses = directChildren(axis, 'crosses');
  ...
  if (auto.length > 1 || auto.some((item) => !booleanAttributeIsTrue(item)))
    return false;
  const alignments = directChildren(axis, 'lblAlgn');
  if (
    alignments.length > 1 ||
    alignments.some((item) => attribute(item, 'val') !== 'ctr')
  )
    return false;
  const offsets = directChildren(axis, 'lblOffset');
  if (
    offsets.length > 1 ||
    offsets.some((item) => attribute(item, 'val') !== '100')
  )
    return false;
  const deletion = directChildren(axis, 'delete');
  if (
    deletion.length > 1 ||
    !deletion.every((item) => validBooleanAttributeValue(item))
  )
    return false;
...
function isSupportedAxisTitle(title: Element): boolean {
  ...
  if (
    children.some(
      (child) => !['tx', 'layout', 'overlay'].includes(child.localName),
    )
  )
    return false;
  ...
  if (
    layouts.length > 1 ||
    layouts.some((layout) => descendants(layout, 'manualLayout').length > 0)
  )
    return false;
  const overlays = directChildren(title, 'overlay');
  if (
    overlays.length > 1 ||
    overlays.some((overlay) => booleanAttributeIsTrue(overlay))
  )
    return false;
  ...
  if (source.localName === 'strRef') {
    ...
    if (
      sourceChildren.some(
        (child) => child.localName !== 'f' && child.localName !== 'strCache',
      )
    )
      return false;
  ...
  if (descendants(source, 'p').length !== 1 || !descendants(source, 't').length)
    return false;
  return !['br', 'fld', 'hlinkClick', 'hlinkMouseOver'].some(
    (name) => descendants(source, name).length > 0,
  );
}
