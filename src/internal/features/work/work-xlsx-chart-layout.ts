...
export interface ParsedXlsxChartLegend
  extends Pick<WorkSpreadsheetChart, 'showLegend'>,
    WorkSpreadsheetChartLayout {}

export function parseXlsxChartLegend(
  document: Document,
): ParsedXlsxChartLegend {
  const legend = firstDescendant(document, 'legend');
  if (!legend) return { showLegend: false };
  const position = xlsxLegendPosition(
    attribute(directChild(legend, 'legendPos') ?? legend, 'val'),
  );
  const overlayElement = directChild(legend, 'overlay');
  const overlay = overlayElement
    ? xlsxBooleanValue(attribute(overlayElement, 'val'))
    : false;
  return {
    showLegend: true,
...
export function xlsxChartLegendXml(chart: WorkSpreadsheetChart): string {
  ...
  const position = xlsxLegendPositionValue(
    normalizeWorkSpreadsheetChartLegendPosition(chart.legendPosition),
  );
  const overlay = normalizeWorkSpreadsheetChartLegendOverlay(
    chart.legendOverlay,
  )
    ? 1
    : 0;
...
export function parseXlsxChartPlotLayout(
  ...
  return {
    ...
    ...(workSpreadsheetChartSupportsBarSpacing(chartType)
      ? {
          gapWidth: normalizeWorkSpreadsheetChartGapWidth(
            gapWidth ? attribute(gapWidth, 'val') : undefined,
          ),
          overlap: normalizeWorkSpreadsheetChartOverlap(
            overlap ? attribute(overlap, 'val') : undefined,
            grouping,
          ),
        }
      : {}),
    ...(workSpreadsheetChartSupportsSmoothLines(chartType)
      ? {
          smoothLines: normalizeWorkSpreadsheetChartSmoothLines(
            consistentLineSmooth(chartNode),
          ),
        }
      : {}),
  };
...
export function xlsxChartGroupingValue(
...
export function xlsxChartSmoothLinesValue(
  chart: WorkSpreadsheetChart,
): boolean {
...
export function xlsxChartNodeUsesStackedGrouping(chartNode: Element): boolean {
  const grouping = attribute(
    directChild(chartNode, 'grouping') ?? chartNode,
    'val',
  );
...
export function isSupportedXlsxChartLegend(document: Document): boolean {
  ...
  if (children.some((child) => !LEGEND_CHILDREN.has(child.localName)))
    return false;
  const positions = directChildren(legend, 'legendPos');
  if (
    positions.length > 1 ||
    (positions[0] &&
      !LEGEND_POSITIONS.has(attribute(positions[0], 'val') ?? ''))
  ) {
    return false;
  }
  ...
  if (
    layouts.length > 1 ||
    layouts.some((layout) => directChildren(layout).length > 0)
  )
    return false;
...
export function isSupportedXlsxChartPlotLayout(chart: Element): boolean {
  ...
  if (chart.localName === 'barChart') {
    if (
      !grouping.length ||
      !BAR_GROUPINGS.has(attribute(grouping[0], 'val') ?? '')
    )
      return false;
    ...
    chart.localName === 'lineChart' ||
    chart.localName === 'areaChart'
  ) {
    if (
      !grouping.length ||
      !LINE_AREA_GROUPINGS.has(attribute(grouping[0], 'val') ?? '')
    )
      return false;
    if (
      directChildren(chart, 'gapWidth').length ||
      directChildren(chart, 'overlap').length
    )
      return false;
  } else if (
    grouping.length ||
  ...
  if (chart.localName === 'lineChart') {
    ...
    descendants(chart, 'smooth').some((element) =>
      xlsxBooleanValue(attribute(element, 'val')),
    )
  ) {
    return false;
  }
  ...
  if (
    descendants(chart, 'bubble3D').some((element) =>
      xlsxBooleanValue(attribute(element, 'val')),
    )
  )
    return false;
  if (
    descendants(chart, 'explosion').some(
      (element) => Number(attribute(element, 'val')) !== 0,
    )
  )
    return false;
  return !descendants(chart, 'firstSliceAng').some(
    (element) => Number(attribute(element, 'val')) !== 0,
  );
}
...
export function isSupportedXlsxCombinationPlotNode(chart: Element): boolean {
  ...
  if (chart.localName === 'lineChart' || chart.localName === 'areaChart') {
    return (
      grouping === 'standard' &&
      (chart.localName !== 'lineChart' || consistentLineSmooth(chart) === false)
    );
  }
  ...
  return (
    Number(attribute(gapWidth ?? chart, 'val') ?? 150) === 150 &&
    Number(attribute(overlap ?? chart, 'val') ?? 0) === 0
  );
}

export function xlsxChartGroupingIsStacked(
  chart: WorkSpreadsheetChart,
): boolean {
  return workSpreadsheetChartGroupingIsStacked(xlsxChartGroupingValue(chart));
}

function xlsxLegendPosition(
  value: string | null,
): WorkSpreadsheetChartLegendPosition {
...
function xlsxLegendPositionValue(
...
function hasConsistentLineSmooth(chart: Element): boolean {
  ...
  if (chartSmooth.length > 1 || !chartSmooth.every(validBooleanElement))
    return false;
  const series = directChildren(chart, 'ser');
  if (series.some((item) => directChildren(item, 'smooth').length > 1))
    return false;
  const explicit = [
    ...chartSmooth,
  ...
  const values = explicit.map((element) =>
    xlsxBooleanValue(attribute(element, 'val')),
  );
...
function validOptionalIntegerElement(
  parent: Element,
  localName: string,
  minimum: number,
  maximum: number,
...
function validOptionalBooleanElement(
  parent: Element,
  localName: string,
): boolean {
  const elements = directChildren(parent, localName);
