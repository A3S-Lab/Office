...
const DASH_VALUES = new Set<WorkSpreadsheetChartLineDash>([
  'solid',
  'dash',
  'dot',
  'dashDot',
...
export function parseXlsxChartSeriesStyle(
  series: Element,
...
export function xlsxChartSeriesShapePropertiesXml(
  style: WorkSpreadsheetChartSeriesStyle | undefined,
): string {
  const normalized = normalizeWorkSpreadsheetChartSeriesStyle(style);
  if (!normalized) return '';
  const fill = normalized.fillColor
    ? xlsxSolidFillXml(normalized.fillColor, normalized.fillTransparency)
    : '';
...
export function isSupportedXlsxChartSeriesFormatting(
  series: Element,
  chartLocalName: string,
): boolean {
  const shapeProperties = directChildren(series, 'spPr');
  if (
    shapeProperties.length > 1 ||
    shapeProperties.some((shape) => !isSupportedSeriesShapeProperties(shape))
  ) {
    return false;
  }
...
export function xlsxChartSeriesFormattingShapeProperties(
  series: Element,
): Element[] {
  return [
    ...directChildren(series, 'spPr'),
    ...directChildren(series, 'marker').flatMap((marker) =>
      directChildren(marker, 'spPr'),
    ),
  ];
}
...
function parseShapeProperties(
  ...
  const lineFill = line
    ? parseSolidFill(directChild(line, 'solidFill'))
    : undefined;
...
function parseMarkerStyle(
  marker: Element,
): WorkSpreadsheetChartMarkerStyle | undefined {
  ...
  const fill = shape
    ? parseSolidFill(directChild(shape, 'solidFill'))
    : undefined;
...
function parseSolidFill(fill: Element | undefined): XlsxSolidColor | undefined {
  ...
  const normalized = normalizeWorkSpreadsheetChartColor(
    attribute(color ?? fill, 'val'),
  );
...
function xlsxLineWidth(line: Element): number | undefined {
  ...
  return Number.isFinite(width)
    ? Math.round((width / EMUS_PER_POINT) * 100) / 100
    : undefined;
}
...
function xlsxLineDash(line: Element): WorkSpreadsheetChartLineDash | undefined {
  ...
  return DASH_VALUES.has(value as WorkSpreadsheetChartLineDash)
    ? (value as WorkSpreadsheetChartLineDash)
    : undefined;
}
...
function xlsxSolidFillXml(color: string, transparency?: number): string {
  ...
  if (transparency === undefined)
    return `<a:solidFill><a:srgbClr val="${value}"/></a:solidFill>`;
...
function xlsxLineXml(style: WorkSpreadsheetChartSeriesStyle): string {
  if (!style.lineColor && style.lineWidth === undefined && !style.lineDash)
    return '';
  const width =
    style.lineWidth === undefined
      ? ''
      : ` w="${Math.round(style.lineWidth * EMUS_PER_POINT)}"`;
...
function xlsxMarkerShapePropertiesXml(
  marker: WorkSpreadsheetChartMarkerStyle,
): string {
  const fill = marker.fillColor ? xlsxSolidFillXml(marker.fillColor) : '';
  const line = marker.lineColor
    ? `<a:ln>${xlsxSolidFillXml(marker.lineColor)}</a:ln>`
    : '';
  return fill || line ? `<c:spPr>${fill}${line}</c:spPr>` : '';
}
...
function isSupportedSeriesShapeProperties(shape: Element): boolean {
  ...
  if (
    children.some(
      (child) => child.localName !== 'solidFill' && child.localName !== 'ln',
    )
  )
    return false;
...
function isSupportedSeriesLine(line: Element): boolean {
  ...
  if (
    children.some(
      (child) =>
        child.localName !== 'solidFill' && child.localName !== 'prstDash',
    )
  )
    return false;
  ...
  if (
    fills.length > 1 ||
    dashes.length > 1 ||
    fills.some((fill) => !isSupportedSolidFill(fill, false))
  ) {
    return false;
  }
  ...
  if (widthSource !== null) {
    ...
    if (
      ...
      width < 0.25 * EMUS_PER_POINT ||
      width > 20 * EMUS_PER_POINT
    )
      return false;
  }
  return (
    !dashes.length ||
    DASH_VALUES.has(attribute(dashes[0], 'val') as WorkSpreadsheetChartLineDash)
  );
}
...
function isSupportedMarker(marker: Element): boolean {
  ...
  if (
    children.some(
      (child) => !['symbol', 'size', 'spPr'].includes(child.localName),
    )
  )
    return false;
...
function isSupportedMarkerShapeProperties(shape: Element): boolean {
  ...
  return (
    ...
    lineChildren.every(
      (child) =>
        child.localName === 'solidFill' && isSupportedSolidFill(child, false),
    )
  );
}
...
function isSupportedSolidFill(fill: Element, allowAlpha: boolean): boolean {
  ...
  if (children.length !== 1 || children[0].localName !== 'srgbClr')
    return false;
  ...
  if (
    transforms.some((transform) => transform.localName !== 'alpha') ||
    transforms.length > (allowAlpha ? 1 : 0)
  ) {
    return false;
  }
...
function hasOnlyAttributes(element: Element, allowed: string[]): boolean {
  return Array.from(element.attributes).every(
    (item) =>
      item.name === 'xmlns' ||
      item.prefix === 'xmlns' ||
      allowed.includes(item.localName),
  );
}
