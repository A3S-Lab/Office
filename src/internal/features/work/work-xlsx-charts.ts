...
export const XLSX_CHART_RELATIONSHIP =
...
export function isSupportedXlsxCombinationChartNodes(
  chartNodes: readonly Element[],
): boolean {
  if (
    chartNodes.length < 2 ||
    !chartNodes.every(
      (node) =>
        xlsxCombinationSeriesType(node) !== null &&
        isSupportedXlsxCombinationPlotNode(node),
    )
  ) {
    return false;
  }
  ...
  if (!categoryNodes.length || categoryNodes.some((category) => !category))
    return false;
  const signatures = categoryNodes.map((category) =>
    categorySourceSignature(category!),
  );
...
function categorySourceSignature(category: Element): string {
  ...
  return reference
    ? `reference:${reference}`
    : `literal:${JSON.stringify(cachedValues(category))}`;
}

export interface XlsxWorksheetChart
  extends XlsxDrawingAnchor,
...
export async function readXlsxWorksheetCharts(
  ...
  const drawingReference = directChildren(
  ...
  for (const [anchorIndex, anchor] of directChildren(
    ...
    const chartRelationshipId = chartReference
      ...
      : null;
    const relationship = chartRelationshipId
    ...
    const sourceId =
      attribute(properties ?? frame, 'id') ?? String(anchorIndex + 1);
    charts.push({
      id: `xlsx-chart-${partNumber(drawingRelationship.target)}-${sourceId}`,
      name:
        attribute(properties ?? frame, 'name')?.trim() ||
        `Worksheet chart ${anchorIndex + 1}`,
...
function parseXlsxChart(
  ...
  const chartNodes = directChildren(plotArea).filter((element) =>
  ...
  const chartDataLabels = parseXlsxDataLabels(
    directChild(chartNode, 'dLbls'),
    type,
  );
  const parsedSeries = seriesNodes.map((node, index) =>
    parseXlsxChartSeries(node, index, type, chartDataLabels),
  );
  const series = xlsxChartNodeUsesStackedGrouping(chartNode)
    ? parsedSeries.map(withoutStackedChartAnalysis)
    : parsedSeries;
  const categoryNode = workSpreadsheetChartUsesNumericXAxis(type)
    ? null
    : directChild(seriesNodes[0], 'cat');
...
function parseXlsxCombinationChart(
  ...
  const entries = chartNodes
    .flatMap((chartNode) => {
      ...
      return directChildren(chartNode, 'ser').map(
        (seriesNode, sourceIndex) => ({
          seriesNode,
          chartType,
          axisGroup,
          chartDataLabels,
          order: finiteNumber(
            attribute(directChild(seriesNode, 'order') ?? seriesNode, 'val') ??
              sourceIndex,
            sourceIndex,
          ),
        }),
  ...
  const title = xlsxChartTitle(document);
  const hasSecondaryAxes = entries.some(
    (entry) => entry.axisGroup === 'secondary',
  );
  ...
  return {
    ...
    series: entries.map((entry, index) => ({
      ...parseXlsxChartSeries(
        entry.seriesNode,
        index,
        entry.chartType,
        entry.chartDataLabels,
      ),
...
function withoutStackedChartAnalysis(
  series: WorkSpreadsheetChartSeries,
...
function xlsxChartTitle(document: Document): Element | undefined {
  const chart =
    directChild(document.documentElement, 'chart') ??
    firstDescendant(document, 'chart');
  return chart ? directChild(chart, 'title') : undefined;
}

function xlsxCombinationSeriesType(
  node: Element,
): WorkSpreadsheetCombinationSeriesType | null {
  if (node.localName === 'lineChart') return 'line';
  if (node.localName === 'areaChart') return 'area';
  if (
    node.localName === 'barChart' &&
    attribute(firstDescendant(node, 'barDir') ?? node, 'val') !== 'bar'
  ) {
    return 'column';
  }
  return null;
...
function xlsxCombinationAxisGroup(
  document: Document,
  chartNode: Element,
...
function parseXlsxChartSeries(
  ...
  const values = directChild(
    node,
    workSpreadsheetChartUsesNumericXAxis(type) ? 'yVal' : 'val',
  );
  const xValues = workSpreadsheetChartUsesNumericXAxis(type)
    ? directChild(node, 'xVal')
    : null;
  const bubbleSizes =
    type === 'bubble' ? directChild(node, 'bubbleSize') : null;
  const dataLabelsNode = directChild(node, 'dLbls');
  const dataLabels = dataLabelsNode
    ? parseXlsxDataLabels(dataLabelsNode, type)
    : inheritedDataLabels;
  ...
  return {
    name:
      (title ? cachedText(title) || richText(title) : '') ||
      `Series ${index + 1}`,
    nameReference: title ? formulaReference(title) : undefined,
    values: values
      ? cachedValues(values).map((value) => finiteNumber(value))
      : [],
    ...
    ...(bubbleSizes
      ? {
          bubbleSizes: cachedValues(bubbleSizes).map((value) =>
            finiteNumber(value),
          ),
...
function parseXlsxDataLabels(
  ...
  const position = xlsxDataLabelPosition(
    attribute(directChild(node, 'dLblPos') ?? node, 'val'),
  );
...
function xlsxDataLabelPosition(
  value: string | null,
): WorkSpreadsheetDataLabelPosition | undefined {
...
export function parseXlsxErrorBars(
  ...
  const direction =
    directionValue === 'x' ? 'x' : directionValue === 'y' ? 'y' : null;
  if (
    !direction ||
    (direction === 'x' && !workSpreadsheetChartUsesNumericXAxis(chartType))
  )
    return null;

  const barTypeValue = attribute(
    directChild(node, 'errBarType') ?? node,
    'val',
  );
  const barType =
    barTypeValue === 'plus' ||
    ...
    barTypeValue === 'both'
      ? barTypeValue
      : null;
  const valueType = workSpreadsheetErrorBarValueType(
    attribute(directChild(node, 'errValType') ?? node, 'val'),
  );
...
function workSpreadsheetErrorBarValueType(
  value: string | null,
): WorkSpreadsheetErrorBarValueType | null {
...
export function parseXlsxTrendline(
  node: Element,
): WorkSpreadsheetTrendline | null {
  const type = xlsxTrendlineType(
    attribute(firstDescendant(node, 'trendlineType') ?? node, 'val'),
  );
  if (!type) return null;
...
function xlsxTrendlineType(
  value: string | null,
): WorkSpreadsheetTrendlineType | null {
...
function xlsxChartType(node: Element): WorkSpreadsheetChartType {
  ...
  return attribute(firstDescendant(node, 'barDir') ?? node, 'val') === 'bar'
    ? 'bar'
    : 'column';
}

function readDoughnutHoleSize(node: Element): number {
  return normalizeWorkSpreadsheetDoughnutHoleSize(
    attribute(firstDescendant(node, 'holeSize') ?? node, 'val'),
  );
}

function readRadarStyle(node: Element): WorkSpreadsheetRadarStyle {
  return normalizeWorkSpreadsheetRadarStyle(
    attribute(firstDescendant(node, 'radarStyle') ?? node, 'val'),
  );
}

function readScatterStyle(node: Element): WorkSpreadsheetScatterStyle {
  return normalizeWorkSpreadsheetScatterStyle(
    attribute(firstDescendant(node, 'scatterStyle') ?? node, 'val'),
  );
}

function readBubbleScale(node: Element): number {
  return normalizeWorkSpreadsheetBubbleScale(
    attribute(firstDescendant(node, 'bubbleScale') ?? node, 'val'),
  );
}

function readShowNegativeBubbles(node: Element): boolean {
  return readBooleanValue(
    attribute(firstDescendant(node, 'showNegBubbles') ?? node, 'val'),
  );
}

function readBubbleSizeRepresents(
  node: Element,
...
function chartPlotXml(
  ...
  if (chart.type === 'combination') {
    return combinationChartPlotXml(
      chart,
      categoryAxisId,
      valueAxisId,
      secondaryCategoryAxisId,
  ...
  if (chart.type === 'doughnut') {
    const holeSize = normalizeWorkSpreadsheetDoughnutHoleSize(
  ...
  return Array.from(groups.values())
    .map((group) => {
      ...
      const series = group.series
        .map(({ item, index }) => chartSeriesXml(item, seriesChart, index))
        .join('');
      if (group.type === 'line') {
...
function chartSeriesXml(
  ...
  const showMarkers =
    chart.type === 'line' ||
    (chart.type === 'radar' &&
      normalizeWorkSpreadsheetRadarStyle(chart.radarStyle) === 'marker');
  ...
  const trendlines =
    supportsAnalysis && workSpreadsheetChartSupportsTrendlines(chart.type)
      ? chartTrendlinesXml(series)
      : '';
  const errorBars = supportsAnalysis
    ? chartErrorBarsXml(series, chart.type)
    : '';
  return [
    '<c:ser>',
    ...
    chart.type === 'line'
      ? `<c:smooth val="${xlsxChartSmoothLinesValue(chart) ? 1 : 0}"/>`
      : '',
    '</c:ser>',
  ].join('');
}

function xyChartSeriesXml(
  series: WorkSpreadsheetChartSeries,
  chart: WorkSpreadsheetChart,
  index: number,
): string {
  ...
  const xValuesXml = numberReferenceOrLiteralXml(
    series.xValuesReference,
    xValues,
  );
  const yValuesXml = numberReferenceOrLiteralXml(
  ...
  const shapeProperties = xlsxChartSeriesShapePropertiesXml(series.style);
  if (chart.type === 'bubble') {
    const bubbleSizesXml = numberReferenceOrLiteralXml(
  ...
  const style = normalizeWorkSpreadsheetScatterStyle(chart.scatterStyle);
  const showMarkers =
    style === 'marker' || style === 'lineMarker' || style === 'smoothMarker';
...
function chartDataLabelsXml(
  series: WorkSpreadsheetChartSeries,
  chartType: WorkSpreadsheetChartType,
): string {
  if (!series.dataLabels) return '';
  const labels = normalizeWorkSpreadsheetDataLabels(
    series.dataLabels,
    chartType,
  );
  return [
    '<c:dLbls>',
    labels.position
      ? `<c:dLblPos val="${xlsxDataLabelPositionValue(labels.position)}"/>`
      : '',
...
function xlsxDataLabelPositionValue(
...
function chartTrendlineXml(source: WorkSpreadsheetTrendline): string {
  ...
  return [
    ...
    trendline.type === 'polynomial'
      ? `<c:order val="${trendline.order ?? 2}"/>`
      : '',
    trendline.type === 'movingAverage'
      ? `<c:period val="${trendline.period ?? 2}"/>`
      : '',
    trendline.forward
      ? `<c:forward val="${finiteNumber(trendline.forward)}"/>`
      : '',
    trendline.backward
      ? `<c:backward val="${finiteNumber(trendline.backward)}"/>`
      : '',
    trendline.intercept !== undefined
      ? `<c:intercept val="${finiteNumber(trendline.intercept)}"/>`
      : '',
...
function numberReferenceOrLiteralXml(
  reference: string | undefined,
