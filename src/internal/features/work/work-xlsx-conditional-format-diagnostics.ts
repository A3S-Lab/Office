...
export function diagnoseXlsxConditionalFormatting(
  ...
  const dxfs = styles
    ? directChildren(firstDescendant(styles, 'dxfs') ?? styles, 'dxf')
    : [];
  ...
  if (allContainers.length > containers.length)
    unsupported.add('extension-based rules');
  if (descendants(worksheet, 'dataBar').some(hasAdvancedDataBarVisuals)) {
    unsupported.add(
      'x14 data-bar direction, axis, border, gradient, or negative-color options',
    );
  }
  ...
  for (const container of containers) {
    for (const rule of directChildren(container, 'cfRule')) {
      ...
      const formulas = directChildren(rule, 'formula').map(
        (formula) => formula.textContent?.trim() ?? '',
      );
      ...
      if (type === 'cellIs') {
        ...
        const formulaCount =
          supportedOperator &&
          spreadsheetConditionalComparisonNeedsUpperValue(operator)
            ? 2
            : 1;
        if (supportedOperator && formulas.length >= formulaCount)
          supported += 1;
        ...
        if (
          type === 'aboveAverage' &&
          (attribute(rule, 'equalAverage') !== null ||
            attribute(rule, 'stdDev') !== null)
        ) {
          optionNormalization = true;
        }
        ...
        const supportedThresholds =
          ...
          (thresholds.length === 0 ||
            (thresholds.length === colors.length &&
              thresholds.every(isSupportedVisualThreshold)));
        if (
          colors.length >= 2 &&
          colors.length <= 3 &&
          colors.every(hasRgbColor) &&
          supportedThresholds
        ) {
          supported += 1;
        ...
        const supportedThresholds =
          ...
          (thresholds.length === 2 &&
            thresholds.every(isSupportedVisualThreshold));
        const supportedOptions = Boolean(
          dataBar && hasSupportedDataBarOptions(dataBar),
        );
        if (
          colors.length === 1 &&
          colors.every(hasRgbColor) &&
        ...
        if (
          ...
          Array.from(dataBar.attributes).some(
            (item) =>
              !['showValue', 'minLength', 'maxLength'].includes(item.localName),
          )
        ) {
          optionNormalization = true;
        }
        ...
        const name = iconSet
          ? (attribute(iconSet, 'iconSet') ?? '3TrafficLights1')
          : '';
        if (
          iconSet &&
          (booleanAttribute(iconSet, 'custom') ||
            directChildren(iconSet, 'cfIcon').length > 0)
        ) {
          ...
          if (
            Array.from(iconSet.attributes).some(
              (item) =>
                !['iconSet', 'showValue', 'reverse', 'percent'].includes(
                  item.localName,
                ),
            ) ||
            ['showValue', 'reverse', 'percent'].some(
              (name) => !validBooleanAttribute(iconSet, name),
            )
          ) {
            optionNormalization = true;
          }
  ...
  if (colorNormalization) {
    diagnostics.push({
      ...
      message:
        'Theme, indexed, automatic, or tinted conditional-format colors are normalized to supported RGB colors.',
      severity: 'warning',
    });
  }
  if (formulaNormalization) {
    diagnostics.push({
      ...
      message:
        'External, structured, cross-sheet, or advanced conditional formulas may be recalculated differently.',
      severity: 'warning',
    });
  }
...
function isAdvancedFormula(value: string): boolean {
  return (
    /(?:^|[^A-Z0-9_.])(?:INDIRECT|OFFSET|LET|LAMBDA|_XLFN\.)\s*\(/i.test(
      value,
    ) || /(?:\[[^\]]+\]|!|[#@])/.test(value)
  );
}
...
function hasAdvancedDifferentialStyle(dxf: Element): boolean {
  return directChildren(dxf).some((child) => {
    if (child.localName === 'font')
      return directChildren(child).some((item) => item.localName !== 'color');
...
function validBooleanAttribute(element: Element, name: string): boolean {
  ...
  return (
    value === null || ['0', '1', 'false', 'true'].includes(value.toLowerCase())
  );
}
...
function hasSupportedDataBarOptions(element: Element): boolean {
  ...
  const minimum = percentageAttribute(
    element,
    'minLength',
    DEFAULT_DATA_BAR_MIN_LENGTH,
  );
  const maximum = percentageAttribute(
    element,
    'maxLength',
    DEFAULT_DATA_BAR_MAX_LENGTH,
  );
...
function hasAdvancedDataBarVisuals(element: Element): boolean {
  return (
    ...
    ].some((name) => attribute(element, name) !== null) ||
    [
      'borderColor',
      'negativeFillColor',
      'negativeBorderColor',
