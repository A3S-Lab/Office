...
export class XlsxDifferentialFormatWriter {
  ...
  private updateCount() {
    this.container.setAttribute(
      'count',
      String(directChildren(this.container, 'dxf').length),
    );
  }
}
...
export function writeXlsxConditionalFormats(
  ...
  for (const existing of directChildren(root, 'conditionalFormatting'))
    existing.remove();
  ...
  for (const value of source) {
    ...
    if (!rule?.cellrange.length) continue;
    const ruleElement = writeRule(
      worksheet,
      rule,
      priority,
      differentialFormats,
    ...
    const container = worksheet.createElementNS(
      namespace,
...
function writeRule(
  ...
  if (rule.type === 'colorGradation') {
    const colors = Array.isArray(rule.format)
      ? rule.format.slice(0, 3).reverse()
      : [];
    ...
    for (const threshold of options.thresholds)
      scale.append(writeThreshold(document, threshold));
  ...
  if (rule.type === 'dataBar') {
    ...
    for (const threshold of options.thresholds)
      dataBar.append(writeThreshold(document, threshold));
    const color = document.createElementNS(namespace, 'color');
    color.setAttribute('rgb', rgb);
  ...
  if (rule.type === 'icons') {
    const format = Array.isArray(rule.format)
      ? null
      : normalizeSpreadsheetConditionalIconSetFormat(rule.format);
    ...
    for (const threshold of format.thresholds) {
      ...
      if (threshold.value !== undefined)
        value.setAttribute('val', String(threshold.value));
  ...
  const finish = () => {
    const dxfId = differentialFormats?.append(
      rule.format as FortuneConditionalFormatStyle,
    );
    if (dxfId !== null && dxfId !== undefined)
      element.setAttribute('dxfId', String(dxfId));
    return element;
  };
  ...
  if (isSpreadsheetConditionalComparisonOperator(rule.conditionName)) {
    ...
    if (!values.length || (needsUpperValue && values[1] === undefined))
      return null;
    ...
    if (needsUpperValue)
      appendFormula(document, element, formulaForCellRule(values[1]));
    return finish();
  }
  if (rule.conditionName === 'textContains' && values[0]) {
    element.setAttribute('type', 'containsText');
    element.setAttribute('text', values[0]);
    const firstCell = encodeCell(
      rule.cellrange[0].row[0],
      rule.cellrange[0].column[0],
    );
    appendFormula(
      document,
      element,
      `NOT(ISERROR(SEARCH("${values[0].replaceAll('"', '""')}",${firstCell})))`,
    );
    return finish();
  }
  if (rule.conditionName === 'duplicateValue') {
    element.setAttribute(
      'type',
      values[0] === '1' ? 'uniqueValues' : 'duplicateValues',
    );
    return finish();
  }
  if (
    ['top10', 'top10_percent', 'last10', 'last10_percent'].includes(
      rule.conditionName,
    )
  ) {
    element.setAttribute('type', 'top10');
    element.setAttribute('rank', values[0] || '10');
    if (rule.conditionName.startsWith('last'))
      element.setAttribute('bottom', '1');
    if (rule.conditionName.endsWith('_percent'))
      element.setAttribute('percent', '1');
    return finish();
  }
  if (
    rule.conditionName === 'aboveAverage' ||
    rule.conditionName === 'belowAverage'
  ) {
    element.setAttribute('type', 'aboveAverage');
    if (rule.conditionName === 'belowAverage')
      element.setAttribute('aboveAverage', '0');
    return finish();
  }
...
function conditionalFormatRule(
  ...
  if (
    !['default', 'colorGradation', 'dataBar', 'icons'].includes(
      String(rule.type),
    ) ||
  ...
  const cellrange = rule.cellrange.filter(validRange).map((range) => ({
    ...
    column: [Math.min(...range.column), Math.max(...range.column)] as [
      number,
      number,
    ],
  }));
  ...
  if (rule.type === 'colorGradation') {
    ...
    const visualOptions = optionalVisualOptions(
      rule.visualOptions,
      format.length,
      'colorScale',
    );
  ...
  const visualOptions =
    rule.type === 'dataBar'
      ? optionalVisualOptions(rule.visualOptions, 2, 'dataBar')
      : undefined;
  if (
    rule.type === 'dataBar' &&
    rule.visualOptions !== undefined &&
    !visualOptions
  )
    return null;
  return {
    ...
    ...metadata,
    conditionName:
      typeof rule.conditionName === 'string' ? rule.conditionName : undefined,
    conditionRange: Array.isArray(rule.conditionRange)
      ? rule.conditionRange.filter(validRange)
      : [],
    conditionValue: Array.isArray(rule.conditionValue)
      ? rule.conditionValue.filter((item): item is string | number =>
...
function conditionalFormatStyle(
  value: unknown,
): FortuneConditionalFormatStyle | null {
...
function appendFormula(
  ...
  const formula = document.createElementNS(
...
function optionalVisualOptions(
  value: unknown,
  ...
  return value === undefined
    ? undefined
    : normalizeSpreadsheetConditionalVisualOptions(value, thresholdCount, kind);
}
