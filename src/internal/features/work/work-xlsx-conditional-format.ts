...
} from './work-xlsx-conditional-format-write';
export type {
...
export function readXlsxDifferentialFormats(
  ...
  return directChildren(container, 'dxf').map((dxf) => ({
    ...
    cellColor: colorFromContainer(
      directChild(directChild(dxf, 'fill') ?? dxf, 'patternFill'),
    ),
  }));
}
...
export function readXlsxConditionalFormats(
  ...
  let sourceOrder = 0;
  for (const container of directChildren(
...
function readRule(
  ...
  const formulas = directChildren(element, 'formula').map((formula) =>
    parseFormulaValue(formula.textContent ?? ''),
  );
  if (type === 'cellIs') {
  ...
  if (type === 'duplicateValues' || type === 'uniqueValues') {
    return defaultRule(cellrange, style, 'duplicateValue', [
      type === 'duplicateValues' ? '0' : '1',
    ]);
  }
  if (type === 'top10') {
    ...
    return defaultRule(cellrange, style, conditionName, [
      attribute(element, 'rank') ?? '10',
    ]);
  }
  if (type === 'aboveAverage') {
    return defaultRule(
      cellrange,
      style,
      booleanAttribute(element, 'aboveAverage', true)
        ? 'aboveAverage'
        : 'belowAverage',
      [''],
    );
  ...
  if (type === 'colorScale') {
    ...
    return {
      ...
      ...(spreadsheetConditionalThresholdsEqual(thresholds, defaults)
        ? {}
  ...
  if (type === 'dataBar') {
    ...
    const showValue = booleanAttribute(
      dataBar,
      'showValue',
      defaults.showValue,
    );
    const minLength = boundedPercentageAttribute(
      dataBar,
      'minLength',
      defaults.minLength,
    );
    const maxLength = boundedPercentageAttribute(
      dataBar,
      'maxLength',
      defaults.maxLength,
    );
  ...
  if (type === 'iconSet') {
    ...
    if (
      ...
      directChildren(iconSet, 'cfIcon').length
    )
      return null;
    ...
    if (
      ...
      thresholds.some((threshold) => !threshold)
    )
      return null;
    return {
      type: 'icons',
...
function parseRange(value: string): FortuneConditionalFormatRange | null {
  ...
  const end = match[3]
    ? { row: Number(match[4]) - 1, column: decodeColumn(match[3]) }
    : start;
  return {
...
function colorFromContainer(container: Element | undefined): string | null {
  return colorFromElement(
    firstDescendant(container, 'color') ??
      firstDescendant(container, 'fgColor'),
  );
}
...
function readVisualThresholds(
  ...
  return thresholds.some((threshold) => !threshold)
    ? null
    : (thresholds as SpreadsheetConditionalThreshold[]);
}
...
function boundedPercentageAttribute(
  element: Element,
  name: string,
  defaultValue: number,
): number | null {
