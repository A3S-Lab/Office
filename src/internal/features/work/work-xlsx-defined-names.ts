...
export function importXlsxDefinedNames(
  ...
  for (const definedName of workbook.Workbook?.Names ?? []) {
    const sheetIndex =
      typeof definedName.Sheet === 'number' ? definedName.Sheet : undefined;
    ...
    if (definedName.Name.toLowerCase() === '_xlnm.print_area') {
      ...
      const reference = normalizeSpreadsheetPrintArea(
        stripSpreadsheetSheetQualifier(definedName.Ref, sheet.name),
      );
    ...
    if (definedName.Name.toLowerCase() === '_xlnm.print_titles') {
      if (!sheet?.id) continue;
      const parsed = parseSpreadsheetPrintTitles(
        stripSpreadsheetSheetQualifier(definedName.Ref, sheet.name),
      );
      if (!parsed) continue;
      const rows = parsed.rows
        ? normalizeSpreadsheetPrintTitleRows(
            `$${parsed.rows[0] + 1}:$${parsed.rows[1] + 1}`,
          )
        : null;
...
export function exportXlsxDefinedNames(
  content: WorkSpreadsheetContent,
): DefinedName[] {
  const names: DefinedName[] = [];
  for (const namedRange of content.namedRanges ?? []) {
    if (
      !isValidSpreadsheetDefinedName(namedRange.name) ||
      !namedRange.reference.trim()
    )
      continue;
    const sheetIndex = namedRange.scopeSheetId
      ? content.sheets.findIndex(
          (sheet) => sheet.id === namedRange.scopeSheetId,
        )
      : -1;
  ...
  for (const printArea of content.printAreas ?? []) {
    const sheetIndex = content.sheets.findIndex(
      (sheet) => sheet.id === printArea.sheetId,
    );
  ...
  for (const printTitle of content.printTitles ?? []) {
    const sheetIndex = content.sheets.findIndex(
      (sheet) => sheet.id === printTitle.sheetId,
    );
    if (sheetIndex < 0) continue;
    const rows = printTitle.rows
      ? normalizeSpreadsheetPrintTitleRows(printTitle.rows)
      : null;
    const columns = printTitle.columns
      ? normalizeSpreadsheetPrintTitleColumns(printTitle.columns)
      : null;
