...
export function xlsxDrawingAnchorToBounds(
  ...
  const left = anchor.from
    ? markerPosition(anchor.from.column, anchor.from.columnOffsetEmu, (index) =>
        spreadsheetColumnWidth(config, index),
      )
    : emuToPixels(anchor.leftEmu);
  const top = anchor.from
    ? markerPosition(anchor.from.row, anchor.from.rowOffsetEmu, (index) =>
        spreadsheetRowHeight(config, index),
      )
    : emuToPixels(anchor.topEmu);
  const right = anchor.to
    ? markerPosition(anchor.to.column, anchor.to.columnOffsetEmu, (index) =>
        spreadsheetColumnWidth(config, index),
      )
    : left + emuToPixels(anchor.widthEmu);
  const bottom = anchor.to
    ? markerPosition(anchor.to.row, anchor.to.rowOffsetEmu, (index) =>
        spreadsheetRowHeight(config, index),
      )
...
export function xlsxTwoCellAnchorMarkers(
  bounds: SpreadsheetDrawingBounds,
  sheet: WorkSpreadsheetSheet,
): string {
  const fromColumn = axisMarker(bounds.left, (index) =>
    spreadsheetColumnWidth(sheet.config, index),
  );
  const toColumn = axisMarker(bounds.left + bounds.width, (index) =>
    spreadsheetColumnWidth(sheet.config, index),
  );
  const fromRow = axisMarker(bounds.top, (index) =>
    spreadsheetRowHeight(sheet.config, index),
  );
  const toRow = axisMarker(bounds.top + bounds.height, (index) =>
    spreadsheetRowHeight(sheet.config, index),
  );
  return `${markerXml('from', fromColumn, fromRow)}${markerXml('to', toColumn, toRow)}`;
}

export function spreadsheetColumnWidth(
  config: WorkSpreadsheetSheet['config'],
  index: number,
): number {
...
export function spreadsheetRowHeight(
  config: WorkSpreadsheetSheet['config'],
  index: number,
): number {
  const key = String(index);
...
function readMarker(
  ...
  return {
    ...
    columnOffsetEmu: nonNegativeNumber(
      directChild(element, 'colOff')?.textContent,
    ),
    rowOffsetEmu: nonNegativeNumber(
      directChild(element, 'rowOff')?.textContent,
    ),
  };
}
