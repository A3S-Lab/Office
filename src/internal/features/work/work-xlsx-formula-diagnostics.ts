...
export async function diagnoseXlsxFormulas(
  ...
  const ranges = Array.from(features.sheets.entries()).flatMap(
    ([sheetName, sheet]) =>
  ...
  const dynamicRanges = ranges.filter(
    (range) => range.type === 'dynamic-array',
  ).length;
  const dataTables = ranges.filter(
    (range) => range.type === 'data-table',
  ).length;
  const cachedErrors = formulas.filter((formula) => formula.cachedError).length;
  const externalReferences = formulas.filter((formula) =>
    formulaHasExternalReference(formula.formula),
  );
  const structuredReferences = formulas.filter((formula) =>
    formulaHasStructuredReference(formula.formula),
  );
  const volatileFormulas = formulas.filter(
    (formula) => volatileSpreadsheetFormulaFunctions(formula.formula).length,
  );
  const unsupportedFunctions = new Set(
    formulas.flatMap((formula) =>
      unsupportedSpreadsheetFormulaFunctions(formula.formula),
    ),
  );
  const unsupportedFormulaAttributes = new Set(
    Array.from(features.sheets.values()).flatMap(
      (sheet) => sheet.unsupportedFormulaAttributes,
    ),
  );
