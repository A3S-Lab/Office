...
export async function readXlsxFormulaFeatures(
  buffer: ArrayBuffer,
...
export function createSpreadsheetFormulaMetadata(
  ...
  const metadata: WorkSpreadsheetFormulaMetadata = {
    ...
    sourceFormulas: Object.keys(sourceFormulas).length
      ? sourceFormulas
      : undefined,
...
export function createXlsxFormulaCell(
  ...
  const sourceFormula =
    sheet.formulaMetadata?.sourceFormulas?.[address] ?? range?.formula;
  ...
  if (
    range &&
    range.type !== 'data-table' &&
    !spreadsheetFormulaRangeConflict(sheet, range)
  ) {
...
function readWorksheetFormulaFeatures(
  ...
  for (const cell of descendants(document, 'c')) {
    ...
    const cachedError =
      attribute(cell, 't') === 'e'
        ? (directChild(cell, 'v')?.textContent ?? '')
        : undefined;
    ...
    if (type === 'array' && reference) {
      ...
      ranges.push({
        type:
          metadataIndex && dynamicMetadataIndexes.has(metadataIndex)
            ? 'dynamic-array'
            : 'array',
        anchor: address,
        reference,
  ...
  return {
    ...
    sharedFormulaCells,
    unsupportedFormulaAttributes: Array.from(
...
function readCalculationSettings(
  ...
  return effectiveSpreadsheetCalculationSettings({
    mode: calculationMode(
      attribute(calculation ?? workbook.documentElement, 'calcMode'),
    ),
    fullCalculationOnLoad: booleanAttribute(
      calculation,
      'fullCalcOnLoad',
      false,
    ),
...
function writeCalculationSettings(
  workbook: Document,
  source: WorkSpreadsheetCalculationSettings | undefined,
): void {
  ...
  calculation.setAttribute(
    'fullCalcOnLoad',
    booleanValue(settings.fullCalculationOnLoad),
  );
  calculation.setAttribute(
    'forceFullCalc',
    booleanValue(settings.forceFullCalculation),
  );
  calculation.setAttribute(
    'iterate',
    booleanValue(settings.iterativeCalculation),
  );
  ...
  calculation.setAttribute(
    'fullPrecision',
    booleanValue(settings.fullPrecision),
  );
}
...
async function readDynamicArrayMetadata(
  archive: OoxmlPackage,
): Promise<Set<number>> {
  if (!archive.has('xl/metadata.xml')) return new Set();
  const metadata = await archive.xml('xl/metadata.xml');
  const metadataTypes = directChildren(
    firstDescendant(metadata, 'metadataTypes') ?? metadata.documentElement,
  );
  const dynamicTypeIndexes = new Set(
    metadataTypes.flatMap((item, index) =>
      item.localName === 'metadataType' &&
      attribute(item, 'name')?.toUpperCase() === 'XLDAPR'
        ? [index + 1]
        : [],
    ),
  );
...
function readDataTableOptions(
  formula: Element,
): WorkSpreadsheetDataTableOptions | undefined {
...
function unsupportedFormulaAttributesForElement(
  formula: Element,
  type: string,
): string[] {
  const common = ['t'];
...
function writeDataTableFormula(
  document: Document,
  ...
  const cell = descendants(document, 'c').find(
    (candidate) =>
      (attribute(candidate, 'r') ?? '').toUpperCase().replaceAll('$', '') ===
      anchor,
  );
  if (!cell) return;
  let formula = directChild(cell, 'f');
  if (!formula) {
    formula = document.createElementNS(
      document.documentElement.namespaceURI,
      'f',
    );
    cell.insertBefore(formula, directChild(cell, 'v') ?? cell.firstChild);
  }
  for (const item of Array.from(formula.attributes))
    formula.removeAttribute(item.name);
...
function optionalBooleanAttribute(
  ...
  return attribute(element, name) === null
    ? undefined
    : booleanAttribute(element, name, false);
}
