...
import type {
  WorkSpreadsheetHeaderFooterSections,
...
const FRIENDLY_TO_OOXML_TOKEN: Record<string, string> = Object.fromEntries(
  Object.entries(OOXML_TO_FRIENDLY_TOKEN).map(([ooxml, friendly]) => [
    friendly,
    `&${ooxml}`,
  ]),
);
...
export function parseXlsxHeaderFooterSections(
  ...
  const sections: Required<WorkSpreadsheetHeaderFooterSections> = {
    left: '',
    center: '',
    right: '',
  };
...
export function serializeXlsxHeaderFooterSections(
  ...
  return (
    ...
    .flatMap(([code, template]) =>
      template ? [`&${code}${serializeTemplate(template)}`] : [],
    )
    .join('');
}

export function readXlsxHeaderFooter(
  element: Element | undefined,
): XlsxHeaderFooterSettings {
  if (!element) return {};
  const settings: XlsxHeaderFooterSettings = {};
  const header = parseXlsxHeaderFooterSections(
    directChild(element, 'oddHeader')?.textContent,
  );
  const footer = parseXlsxHeaderFooterSections(
    directChild(element, 'oddFooter')?.textContent,
  );
...
export function writeXlsxHeaderFooter(
  document: Document,
  pageSetup: WorkSpreadsheetPageSetup,
): Element | null {
  ...
  if (
    !header &&
    !footer &&
    pageSetup.scaleWithDocument === undefined &&
    pageSetup.alignWithMargins === undefined
  ) {
    return null;
  }
  ...
  if (pageSetup.scaleWithDocument !== undefined) {
    element.setAttribute(
      'scaleWithDoc',
      pageSetup.scaleWithDocument ? '1' : '0',
    );
  }
  if (pageSetup.alignWithMargins !== undefined) {
    element.setAttribute(
      'alignWithMargins',
      pageSetup.alignWithMargins ? '1' : '0',
    );
  }
...
export function inspectXlsxHeaderFooterText(
  source: string | null | undefined,
): XlsxHeaderFooterInspection {
  const inspection: XlsxHeaderFooterInspection = {
    hasFormatting: false,
    hasImage: false,
  };
  ...
  for (let index = 0; index < source.length - 1; index += 1) {
    ...
    if (formattingCodeEnd(source, index) !== null)
      inspection.hasFormatting = true;
  }
  return inspection;
}
...
function formattingCodeEnd(
  source: string,
  ampersandIndex: number,
): number | null {
  ...
  if (code === 'K') {
    ...
    while (
      ...
      end < ampersandIndex + 7 &&
      /^[0-9A-Fa-f+-]$/.test(source[end + 1])
    ) {
      end += 1;
    }
    return end;
