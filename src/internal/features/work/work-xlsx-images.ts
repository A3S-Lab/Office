...
export function isSupportedXlsxWorksheetImageContentType(
  contentType: string,
): boolean {
...
export async function readXlsxWorksheetImages(
  ...
  for (const [anchorIndex, anchor] of directChildren(
    ...
    const imageRelationship = imageRelationshipId
      ? relationships.get(imageRelationshipId)
      : undefined;
    if (
      !picture ||
      !imageRelationship ||
      !readableImageRelationship(archive, imageRelationship)
    )
      continue;
    ...
    if (imageBudget.bytes + bytes.byteLength > MAX_XLSX_WORKSHEET_IMAGE_BYTES)
      continue;
    ...
    images.push({
      ...
      name:
        attribute(properties ?? picture, 'name')?.trim() ||
        `Worksheet image ${anchorIndex + 1}`,
...
export async function patchXlsxWorksheetDrawings(
  ...
  if (
    !content.sheets.some(
      (sheet) =>
        sheet.images?.some((image) => serializableImage(image)) ||
  ...
  for (const sheet of content.sheets) {
    ...
    const chartParts = charts.map((chart, index) => {
      ...
      zip.file(
        path,
        xlsxChartPartXml(chart, content, sheet, currentChartIndex),
      );
      return {
        chart,
        path,
    ...
    zip.file(
      ...
      drawingRelationshipsXml(media, chartParts),
    );
    drawingPaths.push(drawingPath);
    await bindDrawingToWorksheet(
      zip,
      worksheetPart,
      worksheetEntry,
      drawingPath,
...
function readableImageRelationship(
  archive: OoxmlPackage,
...
function serializableImage(
  image: WorkSpreadsheetImage,
): SerializableImage | null {
  const match = /^data:([^;,]+);base64,([a-z0-9+/=\s]+)$/i.exec(image.src);
  if (!match) return null;
  const contentType =
    image.contentType?.toLowerCase() || match[1].toLowerCase();
  const extension = imageExtension(contentType);
  if (!extension || !isSupportedXlsxWorksheetImageContentType(contentType))
    return null;
  try {
...
async function exportedWorksheetParts(
  archive: OoxmlPackage,
    ...
    ...media.map(({ image, relationshipId, objectId }) => {
      return [
        ...
        `<xdr:cNvPr id="${objectId}" name="${escapeXml(image.name?.trim() || `Image ${objectId}`)}"${
          image.altText?.trim()
            ? ` descr="${escapeXml(image.altText.trim())}"`
            : ''
        }/>`,
...
async function bindDrawingToWorksheet(
  ...
  for (const existing of directChildren(worksheet.documentElement, 'drawing'))
    existing.remove();
...
async function updateContentTypes(
  ...
  for (const path of drawingPaths) {
    ...
    const override =
      existing ?? document.createElementNS(namespace, 'Override');
