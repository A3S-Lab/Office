...
export function readXlsxManualPageBreaks(
  document: Document,
): XlsxManualPageBreaks {
...
export function writeXlsxManualPageBreaks(
  document: Document,
  pageBreaks: WorkSpreadsheetPageBreaks | undefined,
): void {
  const root = document.documentElement;
  for (const existing of directChildren(root).filter(
    (element) =>
      element.localName === 'rowBreaks' || element.localName === 'colBreaks',
  )) {
    existing.remove();
  }
  ...
  if (rows.length)
    root.insertBefore(
      createBreakContainer(document, 'rowBreaks', rows, 16_383),
      anchor,
    );
  if (columns.length)
    root.insertBefore(
      createBreakContainer(document, 'colBreaks', columns, 1_048_575),
      anchor,
    );
}
