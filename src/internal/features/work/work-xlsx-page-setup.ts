...
import type {
  WorkSpreadsheetPageMargins,
  WorkSpreadsheetPageSetup,
  WorkSpreadsheetPaperSize,
} from './work-types';
import {
  readXlsxHeaderFooter,
  writeXlsxHeaderFooter,
} from './work-xlsx-header-footer';
...
const PAPER_SIZES_BY_CODE = new Map(
  Object.entries(PAPER_SIZE_CODES).map(([paperSize, code]) => [
    code,
    paperSize as WorkSpreadsheetPaperSize,
  ]),
);

export function readXlsxPageSetup(
  document: Document,
): XlsxPageSetup | undefined {
  ...
  const setupProperties = sheetProperties
    ? directChild(sheetProperties, 'pageSetUpPr')
    : undefined;
  if (
    !pageSetupElement &&
    !marginsElement &&
  ...
  const paperSize = pageSetupElement
    ? attribute(pageSetupElement, 'paperSize')
    : null;
  const editablePaperSize = paperSize
    ? PAPER_SIZES_BY_CODE.get(paperSize)
    : undefined;
  if (editablePaperSize) pageSetup.paperSize = editablePaperSize;
  const orientation = pageSetupElement
    ? attribute(pageSetupElement, 'orientation')?.toLowerCase()
    : undefined;
  if (orientation === 'portrait' || orientation === 'landscape')
    pageSetup.orientation = orientation;
  const scale = boundedIntegerAttribute(pageSetupElement, 'scale', 10, 400);
  if (scale !== undefined) pageSetup.scale = scale;
  const fitToWidth = boundedIntegerAttribute(
    pageSetupElement,
    'fitToWidth',
    0,
    32_767,
  ...
  const fitToHeight = boundedIntegerAttribute(
    pageSetupElement,
    'fitToHeight',
    0,
    32_767,
  ...
  const pageOrder = pageSetupElement
    ? attribute(pageSetupElement, 'pageOrder')
    : null;
  if (pageOrder === 'downThenOver' || pageOrder === 'overThenDown')
    pageSetup.pageOrder = pageOrder;
  const pageNumberStart = boundedIntegerAttribute(
    pageSetupElement,
    'firstPageNumber',
    1,
    32_767,
  );
  const useFirstPageNumber = pageSetupElement
    ? attribute(pageSetupElement, 'useFirstPageNumber')
    : null;
  if (
    pageNumberStart !== undefined &&
    (useFirstPageNumber === null ||
      booleanAttribute(pageSetupElement, 'useFirstPageNumber'))
  ) {
  ...
  if (printOptions && attribute(printOptions, 'horizontalCentered') !== null) {
    pageSetup.horizontalCentered = booleanAttribute(
      printOptions,
      'horizontalCentered',
    );
  }
  if (printOptions && attribute(printOptions, 'verticalCentered') !== null) {
    pageSetup.verticalCentered = booleanAttribute(
      printOptions,
      'verticalCentered',
    );
  }
...
export function writeXlsxPageSetup(
  ...
  if (
    pageSetup.horizontalCentered !== undefined ||
    pageSetup.verticalCentered !== undefined
  ) {
    const printOptions = document.createElementNS(
      root.namespaceURI,
      'printOptions',
    );
    if (pageSetup.horizontalCentered !== undefined) {
      printOptions.setAttribute(
        'horizontalCentered',
        pageSetup.horizontalCentered ? '1' : '0',
      );
    }
    if (pageSetup.verticalCentered !== undefined) {
      printOptions.setAttribute(
        'verticalCentered',
        pageSetup.verticalCentered ? '1' : '0',
      );
    }
    elements.push(printOptions);
  }
  if (pageSetup.margins)
    elements.push(writeMargins(document, pageSetup.margins));

  const setup = document.createElementNS(root.namespaceURI, 'pageSetup');
  if (pageSetup.paperSize) {
    setup.setAttribute(
      'paperSize',
      PAPER_SIZE_CODES[normalizeSpreadsheetPaperSize(pageSetup.paperSize)],
    );
  }
  if (pageSetup.orientation)
    setup.setAttribute('orientation', pageSetup.orientation);
  if (pageSetup.scale !== undefined)
    setup.setAttribute('scale', String(pageSetup.scale));
  if (pageSetup.fitToWidth !== undefined)
    setup.setAttribute('fitToWidth', String(pageSetup.fitToWidth));
  if (pageSetup.fitToHeight !== undefined)
    setup.setAttribute('fitToHeight', String(pageSetup.fitToHeight));
...
function writeFitToPage(
  ...
  if (fitToPage !== undefined) {
    ...
    if (!setupProperties) {
      setupProperties = document.createElementNS(
        root.namespaceURI,
        'pageSetUpPr',
      );
  ...
  if (
    setupProperties &&
    !setupProperties.attributes.length &&
    !setupProperties.children.length
  ) {
...
): WorkSpreadsheetPageMargins | null {
  if (!element) return null;
  const values = ['top', 'right', 'bottom', 'left', 'header', 'footer'].map(
    (name) => nonnegativeNumberAttribute(element, name),
  );
