...
export function createXlsxPivotXmlParts(
  ...
  const cacheFields: PivotCacheField[] = fields.map((field) => {
    ...
    for (
      let row = sourceRange.row[0] + 1;
      row <= sourceRange.row[1];
      row += 1
    ) {
      values.push(
        pivotSourceValue(
          sourceSheet.data?.[row]?.[sourceRange.column[0] + field.index],
        ),
      );
    }
    return {
      name: field.name,
      values,
      sharedItems: dimensionIndexes.has(field.index)
        ? uniqueSortedPivotValues(values)
        : undefined,
    };
  });
  return {
...
export function xlsxPivotAggregation(
...
export function workPivotAggregation(
  value: string | null,
): WorkSpreadsheetPivotAggregation | null {
...
function pivotTableXml(
  ...
  const dataFieldIndexes = new Set(
    pivot.values.map((value) => value.fieldIndex),
  );
  const reportFilterIndexes = new Set(
    (pivot.reportFilters ?? []).map((filter) => filter.fieldIndex),
  );
  ...
  return [
    ...
    columnFieldIndexes.length
      ? `<colFields count="${columnFieldIndexes.length}">${columnFields}</colFields>`
      : '',
    '<colItems count="1"><i t="grand"><x/></i></colItems>',
    pageFields
      ? `<pageFields count="${pivot.reportFilters!.length}">${pageFields}</pageFields>`
      : '',
...
function pivotTableReference(pivot: WorkSpreadsheetPivotTable): string {
  ...
  const reportFilterRows = pivot.reportFilters?.length
    ? pivot.reportFilters.length + 1
    : 0;
  if (ranges?.length !== 1 || !reportFilterRows)
    return pivot.outputReference ?? pivot.anchor;
  const range = ranges[0];
  const tableStartRow = range.row[0] + reportFilterRows;
  if (tableStartRow > range.row[1])
    return pivot.outputReference ?? pivot.anchor;
...
function pivotCacheRecordsXml(fields: PivotCacheField[]): string {
  ...
  const sharedIndexes = fields.map((field) =>
    field.sharedItems
      ? new Map(
          field.sharedItems.map(
            (value, index) => [pivotValueKey(value), index] as const,
          ),
        )
      : undefined,
  );
...
function pivotSourceValue(cell: Cell | null | undefined): PivotSourceValue {
  ...
  if (cell?.ct?.t === 'e')
    return { value: String(cell.m ?? cell.v ?? '#VALUE!'), type: 'error' };
...
function uniqueSortedPivotValues(
  values: PivotSourceValue[],
): PivotSourceValue[] {
...
function pivotCacheValueXml(value: PivotSourceValue): string {
  ...
  if (value.type === 'error')
    return `<e v="${escapeXml(String(value.value))}"/>`;
  return `<s v="${escapeXml(String(value.value))}"/>`;
}

function pivotValueKey(value: PivotSourceValue): string {
  const raw =
    value.value instanceof Date ? value.value.toISOString() : value.value;
...
function pivotSourceFilterKey(value: PivotSourceValue): string {
  ...
  if (value.type === 'boolean')
    return `boolean:${String(Boolean(value.value))}`;
...
function pivotValueStatistics(values: PivotSourceValue[]) {
  const types = new Set(
    values.filter((value) => value.type !== 'blank').map((value) => value.type),
  );
  return {
    ...
    integer:
      ...
      values
        ...
        .every((value) => Number.isInteger(Number(value.value))),
    date: types.has('date'),
