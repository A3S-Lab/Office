...
export async function patchXlsxPivotTables(
  buffer: ArrayBuffer,
  ...
  const candidates = content.sheets.flatMap((ownerSheet) =>
    (ownerSheet.pivotTables ?? []).flatMap((pivot) => {
      if (
        !ownerSheet.id ||
        !spreadsheetPivotValidation(content, ownerSheet.id, pivot).valid
      )
        return [];
      const worksheetPart = worksheetParts.get(
        ownerSheet.name.slice(0, 31) || '工作表',
      );
  ...
  const workbookRelationships = parseXml(
    await workbookRelationshipsEntry.async('text'),
    'xl/_rels/workbook.xml.rels',
  );
  removeRelationships(workbookRelationships, '/pivotCacheDefinition');
  for (const existing of directChildren(
    workbook.documentElement,
    'pivotCaches',
  ))
    existing.remove();
  const pivotCaches = workbook.createElementNS(
  ...
  for (const [index, candidate] of candidates.entries()) {
    ...
    const pivotCache = workbook.createElementNS(
      workbook.documentElement.namespaceURI,
      'pivotCache',
    );
    pivotCache.setAttribute('cacheId', String(cacheId));
    pivotCache.setAttributeNS(
    ...
    const worksheetRelationshipDocument =
      ...
      (await readRelationshipsDocument(
        zip,
        relationshipsPartPath(candidate.worksheetPart),
      ));
  ...
  zip.file(
    'xl/_rels/workbook.xml.rels',
    new XMLSerializer().serializeToString(workbookRelationships),
  );
  for (const [worksheetPart, relationships] of worksheetRelationships) {
    zip.file(
      relationshipsPartPath(worksheetPart),
...
export async function readXlsxPivotTables(
  buffer: ArrayBuffer,
): Promise<XlsxPivotReadResult> {
  return inspectXlsxPivotTables(await OoxmlPackage.load(buffer));
}

export async function inspectXlsxPivotTables(
  archive: OoxmlPackage,
): Promise<XlsxPivotReadResult> {
  if (!archive.has('xl/workbook.xml'))
    return { tables: [], unsupported: [], detected: 0 };
  ...
  for (const cache of directChildren(
    ...
    const relationship = workbookRelationships.get(
      attribute(cache, 'r:id') ?? attribute(cache, 'id') ?? '',
    );
    if (cacheId === null || !relationship || !archive.has(relationship.target))
      continue;
    cacheById.set(
      cacheId,
      parsePivotCache(await archive.xml(relationship.target)),
    );
  }
  ...
  if (
    !detected &&
    (archive.paths('xl/pivotTables/').length ||
      archive.paths('xl/pivotCache/').length)
  ) {
    unsupported.push({
      code: 'xlsx.pivots.orphaned',
      message:
        'Pivot-table package parts are present without a readable worksheet relationship.',
    });
  }
  if (
    archive.paths('xl/slicers/').length ||
    archive.paths('xl/slicerCaches/').length
  ) {
    unsupported.push({
...
async function workbookWorksheetParts(
  ...
  for (const sheet of directChildren(
    firstDescendant(workbook, 'sheets') ?? workbook.documentElement,
    'sheet',
  )) {
    const name = attribute(sheet, 'name');
    const relationship = relationships.get(
      attribute(sheet, 'r:id') ?? attribute(sheet, 'id') ?? '',
    );
...
function parsePivotCache(document: Document): XlsxPivotCache {
  ...
  const sourceSheetName =
    attribute(worksheetSource ?? root, 'sheet')?.trim() || undefined;
  const sourceReference =
    attribute(worksheetSource ?? root, 'ref')?.trim() || undefined;
  const cacheFields = directChildren(
    directChild(root, 'cacheFields') ?? root,
    'cacheField',
  );
  const fields = cacheFields.map(
    (field, index) => attribute(field, 'name')?.trim() || `Field ${index + 1}`,
  );
  const fieldItems = cacheFields.map((field) =>
    directChildren(directChild(field, 'sharedItems') ?? field).flatMap(
      (item) => {
  ...
  if (
    ...
    descendants(root, 'calculatedMembers').length
  ) {
    unsupported = {
...
function pivotCacheItemValue(
  element: Element,
): { value: WorkSpreadsheetPivotFilterValue } | null {
  const source = attribute(element, 'v');
  if (element.localName === 'm') return { value: null };
  if (element.localName === 'b')
    return { value: source === '1' || source?.toLowerCase() === 'true' };
...
function parsePivotReportFilters(
  ...
  if (pageWrap > 0 || booleanAttribute(root, 'pageOverThenDown')) {
    return {
      unsupported: {
        ...
        message:
          'Wrapped or horizontal report-filter layouts remain cached values only.',
        location,
      },
    };
  }
  const pivotFields = directChildren(
    directChild(root, 'pivotFields') ?? root,
    'pivotField',
  );
  ...
  for (const pageField of directChildren(pageFields, 'pageField')) {
    ...
    const pivotField =
      fieldIndex === null ? undefined : pivotFields[fieldIndex];
    ...
    const items = directChildren(
      directChild(pivotField, 'items') ?? pivotField,
      'item',
    );
    if (
    ...
    if (
      sharedItemIndex === null ||
      sharedItemIndex < 0 ||
      sharedItemIndex >= sharedItems.length
    ) {
      return {
        unsupported: {
          code: 'xlsx.pivots.report-filter-item',
...
function parsePivotTable(
  ...
  const locationLabel = destinationSheetName
    ? `${destinationSheetName} · ${name}`
    : name;
  ...
  if (
    directChildren(root, 'filters').length ||
    booleanAttribute(root, 'dataOnRows')
  ) {
    return {
      unsupported: {
        code: 'xlsx.pivots.filters-layout',
        message:
          'Label or value filters and values-on-rows layouts remain cached values only.',
        location: locationLabel,
      },
    };
  }
  const parsedReportFilters = parsePivotReportFilters(
    root,
    cache,
    locationLabel,
  );
  ...
  const columnFields = readPivotFieldIndexes(
    directChild(root, 'colFields'),
  ...
  for (const dataField of directChildren(
    directChild(root, 'dataFields') ?? root,
    'dataField',
  )) {
    ...
    if (fieldIndex === null || !aggregation || !cache.fields[fieldIndex]) {
      return {
        unsupported: {
          ...
          message:
            'The pivot uses an unsupported value calculation or field reference.',
          location: locationLabel,
        },
      };
    }
  ...
  if (
    ...
    [...rowFields, ...columnFields, ...reportFilterFields].some(
      (index) => index < 0 || !cache.fields[index],
    ) ||
    ...
    return {
      unsupported: {
        ...
        message:
          'The pivot layout cannot be represented by the editable Work pivot model.',
        location: locationLabel,
      },
    };
  }
  ...
  if (outputRange[0].row[0] < reportFilterRows) {
    return {
      unsupported: {
        code: 'xlsx.pivots.report-filter-layout',
        message:
          'The report-filter placement cannot be represented by the Work report layout.',
        location: locationLabel,
      },
    };
  }
  const ownedRange = {
    row: [outputRange[0].row[0] - reportFilterRows, outputRange[0].row[1]] as [
      number,
      number,
    ],
    column: [outputRange[0].column[0], outputRange[0].column[1]] as [
      number,
      number,
    ],
  };
...
function removeRelationships(document: Document, typeSuffix: string): void {
  for (const relationship of directChildren(
    ...
    if (attribute(relationship, 'Type')?.endsWith(typeSuffix))
      relationship.remove();
  }
}
...
function insertWorkbookPivotCaches(
  document: Document,
  pivotCaches: Element,
): void {
  ...
  const insertionPoint = directChildren(document.documentElement).find(
    (element) => laterElements.has(element.localName),
  );
...
async function updateContentTypes(
  zip: JSZip,
  parts: Array<{ path: string; contentType: string }>,
): Promise<void> {
  ...
  for (const override of directChildren(document.documentElement, 'Override')) {
    ...
    if (
      partName.startsWith('/xl/pivotTables/') ||
