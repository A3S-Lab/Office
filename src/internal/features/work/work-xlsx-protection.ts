...
export function readXlsxProtection(
  worksheet: Document,
  styles: Document | null,
): XlsxProtectionFeatures {
...
export function writeXlsxProtection(
  worksheet: Document,
  sheet: Sheet,
  styles?: XlsxCellProtectionWriter,
): void {
  const root = worksheet.documentElement;
  for (const element of directChildren(root, 'sheetProtection'))
    element.remove();
  for (const element of directChildren(root, 'protectedRanges'))
    element.remove();

  const authority = normalizeSheetProtectionAuthority(sheet.config?.authority);
  if (authority.sheet === 1) {
    const element = worksheet.createElementNS(
      root.namespaceURI,
      'sheetProtection',
    );
...
export class XlsxCellProtectionWriter {
  ...
  constructor(private readonly styles: Document) {
    ...
    this.cellXfs =
      directChild(root, 'cellXfs') ??
      styles.createElementNS(root.namespaceURI, 'cellXfs');
    if (!this.cellXfs.parentElement) {
      const anchor = directChildren(root).find((child) =>
        ['cellStyles', 'dxfs', 'tableStyles', 'colors', 'extLst'].includes(
          child.localName,
        ),
      );
  ...
  styleId(baseStyleId: number, locked: boolean, hidden: boolean): number {
    ...
    if (
      sameProtection(readProtectionStyle(styles[baseIndex]), { locked, hidden })
    )
      return baseIndex;
    ...
    const protection = this.styles.createElementNS(
...
export function diagnoseXlsxProtection(
  worksheet: Document,
  styles: Document | null,
): XlsxProtectionDiagnostic[] {
  const protection = directChild(worksheet.documentElement, 'sheetProtection');
  const protectedRanges = readProtectedRanges(worksheet);
...
function authorityFromElement(
  ...
  return {
    ...
    selectLockedCells: booleanAttribute(element, 'selectLockedCells', false)
      ? 0
      : 1,
    selectunLockedCells: booleanAttribute(element, 'selectUnlockedCells', false)
      ? 0
      : 1,
    ...
    insertHyperlinks: booleanAttribute(element, 'insertHyperlinks', true)
      ? 0
      : 1,
    ...
    usePivotTablereports: booleanAttribute(element, 'pivotTables', true)
      ? 0
      : 1,
...
function readProtectedRanges(worksheet: Document): FortuneSheetEditableRange[] {
  ...
  if (!container) return [];
  return directChildren(container, 'protectedRange').flatMap(
    (element, index) => {
...
function readCellProtectionRanges(
  worksheet: Document,
  styles: Document | null,
): SpreadsheetCellProtectionRange[] {
  const xfs = styles
    ? directChildren(
        firstDescendant(styles, 'cellXfs') ?? styles.documentElement,
        'xf',
      )
    : [];
  ...
  for (const cell of descendants(worksheet, 'c')) {
    ...
    ranges.push({
      range: {
        row: [coordinate.row, coordinate.row],
        column: [coordinate.column, coordinate.column],
      },
      ...style,
    });
  }
...
function writeProtectionAttributes(
  element: Element,
  authority: FortuneSheetProtectionAuthority,
): void {
  ...
  element.setAttribute(
    'selectLockedCells',
    authority.selectLockedCells ? '0' : '1',
  );
  element.setAttribute(
    'selectUnlockedCells',
    authority.selectunLockedCells ? '0' : '1',
  );
  ...
  for (const [key, name] of mappings)
    element.setAttribute(name, authority[key] ? '0' : '1');
}

function writeProtectedRanges(
  document: Document,
...
function writeCellProtectionStyles(
  worksheet: Document,
  sheet: Sheet,
  styles: XlsxCellProtectionWriter,
): void {
...
function ensureCellElement(
  ...
  let row = directChildren(sheetData, 'row').find(
    (element) => attribute(element, 'r') === String(rowNumber),
  );
  if (!row) {
    row = document.createElementNS(
    ...
    const anchor = directChildren(sheetData, 'row').find(
      (element) =>
        (positiveInteger(attribute(element, 'r')) ?? Number.MAX_SAFE_INTEGER) >
        rowNumber,
    );
    sheetData.insertBefore(row, anchor ?? null);
  }
  const reference = encodeCell(rowIndex, columnIndex);
  let cell = directChildren(row, 'c').find(
    (element) => attribute(element, 'r') === reference,
  );
  if (!cell) {
...
function worksheetBounds(document: Document): SpreadsheetCellRange {
  const reference = attribute(
    directChild(document.documentElement, 'dimension') ??
      document.documentElement,
    'ref',
  );
  const parsed = reference
    ? parseSpreadsheetCellRanges(reference)?.[0]
    : undefined;
  if (parsed) {
    return {
      row: [0, parsed.row[1]],
  ...
  const coordinates = descendants(document, 'c')
    ...
    .filter((value): value is { row: number; column: number } =>
      Boolean(value),
    );
  return {
...
function expandWorksheetDimension(
  document: Document,
  row: number,
  column: number,
): void {
...
function hasHiddenFormula(
  worksheet: Document,
  styles: Document | null,
): boolean {
  const formulas = descendants(worksheet, 'c').filter((cell) =>
    Boolean(directChild(cell, 'f')),
  );
...
function insertWorksheetFeature(
  root: Element,
  element: Element,
  anchors: string[],
): void {
...
function parseSqref(value: string | null): SpreadsheetCellRange[] {
  return value
    ? (parseSpreadsheetCellRanges(value.trim().replace(/\s+/g, ',')) ?? [])
    : [];
}

function indexedStyle(
  styles: CellProtectionStyle[],
  value: string | null,
): CellProtectionStyle | null {
...
function attributes(element: Element): Record<string, string> {
  return Object.fromEntries(
    Array.from(element.attributes).map((item) => [item.name, item.value]),
  );
}
...
function sameProtection(
  left: CellProtectionStyle,
  right: CellProtectionStyle,
): boolean {
