...
1. Import → light/no-op edit path → export → reopen does not silently drop
   ...
   content controls, PAGE/PAGEREF/NUMPAGES/SECTION fields, XE/INDEX index
...
bookmark-backed PAGEREF, NUMPAGES, and SECTION fields, XE index entries and
INDEX field columns, TOC level/hyperlink options and cached entry titles,
contract tables, duplicate-bookmark identity diagnostics, and active-content
fail-closed export.
