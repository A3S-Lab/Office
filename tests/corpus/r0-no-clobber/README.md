...
1. Import → light/no-op edit path → export → reopen does not silently drop
   ...
   content controls,
   PAGE/PAGEREF/NUMPAGES/SECTION/SECTIONPAGES/NUMWORDS/NUMCHARS/DATE fields,
   XE/INDEX index identities, TOC options/entries, table cells).
...
and DATE fields, XE index entries and INDEX field columns, TOC level/hyperlink
