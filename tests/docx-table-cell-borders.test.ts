...
import { applyDocumentTableSharedBorderPaint } from '../src/internal/features/work/work-document-table-borders';
...
describe('DOCX table cell borders', () => {
  ...
  test('projects shared-edge winners onto CSS without rewriting stored edges', () => {
    const document = parseXml(`
      <w:document xmlns:w="${WORD_NAMESPACE}">
        <w:body>
          <w:tbl>
    ...
    const cells = Array.from(html.querySelectorAll<HTMLTableCellElement>('td'));
    expect(cells[0]?.dataset.officeCellBorderRightStyle).toBe('dotted');
    expect(cells[1]?.dataset.officeCellBorderLeftStyle).toBe('none');
    expect(cells[0]?.style.borderRightStyle).toBe('dotted');
    expect(cells[1]?.style.borderLeftStyle).toBe('dotted');
  });
