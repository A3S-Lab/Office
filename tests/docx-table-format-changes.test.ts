...
describe('DOCX table-formatting revisions', () => {
  ...
  test('imports tblCaption-only w:tblPrChange as a reviewable table-formatting change', async () => {
    const source = await tableDocxWithCaptionChange({
      prior: 'Quarterly summary',
      current: 'Annual summary',
    });
    const imported = await importOfficeFile(
      new File([source], 'table-caption-formatting.docx'),
    );
    if (imported.content.type !== 'document') {
      throw new Error('Expected an imported document artifact.');
    }
    ...
    expect(table?.dataset.officeTableCaption).toBe('Annual summary');
  });

  test('pending tblCaption table-formatting change round-trips as native w:tblPrChange', async () => {
    const source = await tableDocxWithCaptionChange({
      prior: 'Quarterly summary',
      current: 'Annual summary',
    });
    const imported = await importOfficeFile(
      new File([source], 'table-caption-roundtrip.docx'),
    );
    if (imported.content.type !== 'document') {
      throw new Error('Expected an imported document artifact.');
    }
    ...
    expect(change).toBeTruthy();
    expect(
      directChild(directChild(change!, 'tblPr'), 'tblCaption'),
    ).toBeTruthy();
    expect(directChild(properties, 'tblCaption')).toBeTruthy();
  });

  test('imports tblDescription-only w:tblPrChange as a reviewable table-formatting change', async () => {
    const source = await tableDocxWithDescriptionChange({
      prior: 'Quarterly summary',
      current: 'Annual summary',
    });
    const imported = await importOfficeFile(
      new File([source], 'table-description-formatting.docx'),
    );
    if (imported.content.type !== 'document') {
      throw new Error('Expected an imported document artifact.');
    }
    ...
    expect(table?.dataset.officeTableDescription).toBe('Annual summary');
  });

  test('pending tblDescription table-formatting change round-trips as native w:tblPrChange', async () => {
    const source = await tableDocxWithDescriptionChange({
      prior: 'Quarterly summary',
      current: 'Annual summary',
    });
    const imported = await importOfficeFile(
      new File([source], 'table-description-roundtrip.docx'),
    );
    if (imported.content.type !== 'document') {
      throw new Error('Expected an imported document artifact.');
    }
    ...
    expect(change).toBeTruthy();
    expect(
      directChild(directChild(change!, 'tblPr'), 'tblDescription'),
    ).toBeTruthy();
    expect(directChild(properties, 'tblDescription')).toBeTruthy();
  });

  test('imports tblStyleColBandSize-only w:tblPrChange as a reviewable table-formatting change', async () => {
    const source = await tableDocxWithColBandSizeChange({
      prior: 1,
      current: 2,
    });
    const imported = await importOfficeFile(
      new File([source], 'table-col-band-size-formatting.docx'),
    );
    if (imported.content.type !== 'document') {
      throw new Error('Expected an imported document artifact.');
    }
    ...
    expect(table?.dataset.officeTableColBandSize).toBe('2');
  });

  test('pending tblStyleColBandSize table-formatting change round-trips as native w:tblPrChange', async () => {
    const source = await tableDocxWithColBandSizeChange({
      prior: 1,
      current: 2,
    });
    const imported = await importOfficeFile(
      new File([source], 'table-col-band-size-roundtrip.docx'),
    );
    if (imported.content.type !== 'document') {
      throw new Error('Expected an imported document artifact.');
    }
    ...
    expect(change).toBeTruthy();
    expect(
      directChild(directChild(change!, 'tblPr'), 'tblStyleColBandSize'),
    ).toBeTruthy();
    expect(directChild(properties, 'tblStyleColBandSize')).toBeTruthy();
  });

  test('live table colBandSize edits become reviewable when track changes is on', () => {
    const editor = new Editor({
      extensions: createWorkDocumentExtensions({
        isTracking: () => true,
      }),
      content: [
        ...
        ' data-office-table-width="100"',
        ' data-office-table-bidi-visual="false"',
        ' data-office-table-col-band-size="1">',
        '<tbody><tr><td><p>Cell</p></td></tr></tbody></table>',
      ].join(''),
    });
    try {
      ...
      expect(table?.dataset.changeKind).toBe('table-formatting');
      expect(
        parseDocumentTableFormatting(table?.dataset.changeBefore)?.colBandSize,
      ).toBe(1);
      expect(table?.dataset.officeTableColBandSize).toBe('2');
    } finally {
      editor.destroy();
  ...
  test('imports tblStyleRowBandSize-only w:tblPrChange as a reviewable table-formatting change', async () => {
    const source = await tableDocxWithRowBandSizeChange({
      prior: 1,
      current: 2,
    });
    const imported = await importOfficeFile(
      new File([source], 'table-row-band-size-formatting.docx'),
    );
    if (imported.content.type !== 'document') {
      throw new Error('Expected an imported document artifact.');
    }
    ...
    expect(table?.dataset.officeTableRowBandSize).toBe('2');
  });

  test('pending tblStyleRowBandSize table-formatting change round-trips as native w:tblPrChange', async () => {
    const source = await tableDocxWithRowBandSizeChange({
      prior: 1,
      current: 2,
    });
    const imported = await importOfficeFile(
      new File([source], 'table-row-band-size-roundtrip.docx'),
    );
    if (imported.content.type !== 'document') {
      throw new Error('Expected an imported document artifact.');
    }
    ...
    expect(change).toBeTruthy();
    expect(
      directChild(directChild(change!, 'tblPr'), 'tblStyleRowBandSize'),
    ).toBeTruthy();
    expect(directChild(properties, 'tblStyleRowBandSize')).toBeTruthy();
  });

  test('live table rowBandSize edits become reviewable when track changes is on', () => {
    const editor = new Editor({
      extensions: createWorkDocumentExtensions({
        isTracking: () => true,
      }),
      content: [
        ...
        ' data-office-table-width="100"',
        ' data-office-table-bidi-visual="false"',
        ' data-office-table-row-band-size="1">',
        '<tbody><tr><td><p>Cell</p></td></tr></tbody></table>',
      ].join(''),
    });
    try {
      ...
      expect(table?.dataset.changeKind).toBe('table-formatting');
      expect(
        parseDocumentTableFormatting(table?.dataset.changeBefore)?.rowBandSize,
      ).toBe(1);
      expect(table?.dataset.officeTableRowBandSize).toBe('2');
    } finally {
      editor.destroy();
  ...
  test('live table description edits become reviewable when track changes is on', () => {
    const editor = new Editor({
      ...
      content: [
        ...
        ' data-office-table-width="100"',
        ' data-office-table-bidi-visual="false"',
        ' data-office-table-description="Quarterly summary">',
        '<tbody><tr><td><p>Cell</p></td></tr></tbody></table>',
      ].join(''),
    });
    try {
      ...
      expect(table?.dataset.changeKind).toBe('table-formatting');
      expect(
        parseDocumentTableFormatting(table?.dataset.changeBefore)?.description,
      ).toBe('Quarterly summary');
      expect(table?.dataset.officeTableDescription).toBe('Annual summary');
    } finally {
      editor.destroy();
    }
  ...
  test('live table caption edits become reviewable when track changes is on', () => {
    const editor = new Editor({
      extensions: createWorkDocumentExtensions({
        isTracking: () => true,
      }),
      content: [
        ...
        ' data-office-table-width="100"',
        ' data-office-table-bidi-visual="false"',
        ' data-office-table-caption="Quarterly summary">',
        '<tbody><tr><td><p>Cell</p></td></tr></tbody></table>',
      ].join(''),
    });
    try {
      ...
      expect(table?.dataset.changeKind).toBe('table-formatting');
      expect(
        parseDocumentTableFormatting(table?.dataset.changeBefore)?.caption,
      ).toBe('Quarterly summary');
      expect(table?.dataset.officeTableCaption).toBe('Annual summary');
    } finally {
      editor.destroy();
...
async function tableDocxWithColBandSizeChange(options: {
  prior: number;
  current: number | null;
}): Promise<ArrayBuffer> {
  const artifact = createArtifact('blank-document');
  ...
  if (options.current !== null) {
    ...
    current.setAttributeNS(WORD_NAMESPACE, 'w:val', String(options.current));
    properties.append(current);
  }
  const change = document.createElementNS(WORD_NAMESPACE, 'w:tblPrChange');
  change.setAttributeNS(WORD_NAMESPACE, 'w:id', '66');
  change.setAttributeNS(WORD_NAMESPACE, 'w:author', 'Reviewer');
  change.setAttributeNS(WORD_NAMESPACE, 'w:date', '2026-09-09T00:00:00Z');
  const prior = document.createElementNS(WORD_NAMESPACE, 'w:tblPr');
  const priorColBandSize = document.createElementNS(
    WORD_NAMESPACE,
    'w:tblStyleColBandSize',
  );
  priorColBandSize.setAttributeNS(
    WORD_NAMESPACE,
    'w:val',
    String(options.prior),
  );
  prior.append(priorColBandSize);
  change.append(prior);
  properties.append(change);
  archive.file(
    'word/document.xml',
...
async function tableDocxWithRowBandSizeChange(options: {
  prior: number;
  current: number | null;
}): Promise<ArrayBuffer> {
  const artifact = createArtifact('blank-document');
  ...
  for (const existing of Array.from(properties.children).filter(
    (child) =>
      child.localName === 'tblStyleRowBandSize' ||
      child.localName === 'tblPrChange',
  )) {
    existing.remove();
  }
  ...
  const change = document.createElementNS(WORD_NAMESPACE, 'w:tblPrChange');
  change.setAttributeNS(WORD_NAMESPACE, 'w:id', '67');
  change.setAttributeNS(WORD_NAMESPACE, 'w:author', 'Reviewer');
  change.setAttributeNS(WORD_NAMESPACE, 'w:date', '2026-09-09T00:00:00Z');
  const prior = document.createElementNS(WORD_NAMESPACE, 'w:tblPr');
  const priorRowBandSize = document.createElementNS(
    WORD_NAMESPACE,
    'w:tblStyleRowBandSize',
  );
  priorRowBandSize.setAttributeNS(
    WORD_NAMESPACE,
    'w:val',
    String(options.prior),
  );
  prior.append(priorRowBandSize);
  change.append(prior);
  properties.append(change);
  archive.file(
    'word/document.xml',
...
async function tableDocxWithDescriptionChange(options: {
  ...
  for (const existing of Array.from(properties.children).filter(
    (child) =>
      child.localName === 'tblDescription' || child.localName === 'tblPrChange',
  )) {
    existing.remove();
  }
  if (options.current) {
  ...
  const change = document.createElementNS(WORD_NAMESPACE, 'w:tblPrChange');
  change.setAttributeNS(WORD_NAMESPACE, 'w:id', '65');
  change.setAttributeNS(WORD_NAMESPACE, 'w:author', 'Reviewer');
  change.setAttributeNS(WORD_NAMESPACE, 'w:date', '2026-09-09T00:00:00Z');
  const prior = document.createElementNS(WORD_NAMESPACE, 'w:tblPr');
  const priorDescription = document.createElementNS(
    WORD_NAMESPACE,
    'w:tblDescription',
  );
  priorDescription.setAttributeNS(WORD_NAMESPACE, 'w:val', options.prior);
  prior.append(priorDescription);
  change.append(prior);
  properties.append(change);
  archive.file(
    'word/document.xml',
...
async function tableDocxWithCaptionChange(options: {
  prior: string;
  current: string | null;
}): Promise<ArrayBuffer> {
  const artifact = createArtifact('blank-document');
  ...
  for (const existing of Array.from(properties.children).filter(
    (child) =>
      child.localName === 'tblCaption' || child.localName === 'tblPrChange',
  )) {
    existing.remove();
  }
  if (options.current) {
    const current = document.createElementNS(WORD_NAMESPACE, 'w:tblCaption');
    current.setAttributeNS(WORD_NAMESPACE, 'w:val', options.current);
    properties.append(current);
  }
  const change = document.createElementNS(WORD_NAMESPACE, 'w:tblPrChange');
  ...
  change.setAttributeNS(WORD_NAMESPACE, 'w:date', '2026-09-09T00:00:00Z');
  const prior = document.createElementNS(WORD_NAMESPACE, 'w:tblPr');
  const priorCaption = document.createElementNS(WORD_NAMESPACE, 'w:tblCaption');
  priorCaption.setAttributeNS(WORD_NAMESPACE, 'w:val', options.prior);
  prior.append(priorCaption);
  change.append(prior);
  properties.append(change);
  archive.file(
    'word/document.xml',
