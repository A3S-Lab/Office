...
describe('DOCX table property-revision preservation', () => {
  ...
  test('preserves relationship-free w:tcPrChange as opaque cell metadata on untouched round-trip', async () => {
    ...
    // vAlign/solid-shd/tcMar/tcW/noWrap/textDirection/tcFitText/hideMark/cnfStyle subset
    // (hMerge stays opaque).
    ...
    hMerge.setAttributeNS(WORD_NAMESPACE, 'w:val', 'restart');
    prior.append(hMerge);
    change.append(prior);
    properties.append(change);
    archive.file(
    ...
    expect(directChild(priorProperties, 'hMerge')).toBeTruthy();
  });
});
