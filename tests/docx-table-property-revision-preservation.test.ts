...
describe('DOCX table property-revision preservation', () => {
  ...
  test('preserves relationship-free w:trPrChange as opaque row metadata on untouched round-trip', async () => {
    ...
    // hidden/jc/gridBefore alone are reviewable as row-formatting.
    ...
    gridAfter.setAttributeNS(WORD_NAMESPACE, 'w:val', '1');
    prior.append(cantSplit, header, height, gridAfter);
    change.append(prior);
    properties.append(change);
    archive.file(
      'word/document.xml',
    ...
    expect(
      directChild(directChild(exportedChange!, 'trPr'), 'gridAfter'),
    ).toBeTruthy();
  });
