...
test('routes the concise README and documentation homes to the current release story', async () => {
  ...
  expect(readme).toContain('Version `0.172.0`');
  expect(readme).toContain('nests page-matched vector-run Span MCIDs');
  expect(readme).toContain('under outline roles');
  expect(readme).toContain('Earlier releases stay on the product');
  expect(englishHome).toContain("## What's new on `main` (0.172.0)");
  expect(englishHome).toContain(
    'nests page-matched Span MCIDs under outline roles',
  );
  expect(chineseHome).toContain('## `main` 更新内容（0.172.0）');
  expect(chineseHome).toContain(
    '浏览器文档 PDF 导出现将同页匹配的 Span MCID 嵌套到大纲角色下',
  );
...
test('publishes the WPS VML connector boundary across code, docs, and A3S Test evidence', async () => {
  ...
  expect(packageManifest).toContain('"version": "0.172.0"');
});
...
test('publishes the WPS connector-kind A3S Test matrix in the 0.57.0 release story', async () => {
  ...
  for (const document of [latestEnglish, latestChinese]) {
    ...
    expect(document).toContain('0.172.0');
  }
...
test('publishes Writer move revisions across implementation, native collaboration, docs, and release evidence', async () => {
  ...
  expect(packageManifest).toContain('0.172.0');
});
