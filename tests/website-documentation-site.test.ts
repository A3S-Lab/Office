...
test('routes the concise README and documentation homes to the current release story', async () => {
  ...
  expect(readme).toContain('Version `0.230.0`');
  expect(readme).toContain('对象对齐');
  expect(readme).toContain('accessible name');
  expect(readme).toContain('focus steal');
  expect(englishHome).toContain("## What's new on `main` (0.230.0)");
  expect(englishHome).toContain('Presentation and Document L2 toolbar focus');
  expect(chineseHome).toContain('## `main` 更新内容（0.230.0）');
  expect(chineseHome).toContain('演示文稿与文档二级工具条焦点');
...
test('publishes the WPS VML connector boundary across code, docs, and A3S Test evidence', async () => {
  ...
  expect(packageManifest).toContain('"version": "0.230.0"');
});
...
test('publishes the WPS connector-kind A3S Test matrix in the 0.57.0 release story', async () => {
  ...
  for (const document of [latestEnglish, latestChinese]) {
    ...
    expect(document).toContain('0.230.0');
  }
...
test('publishes Writer move revisions across implementation, native collaboration, docs, and release evidence', async () => {
  ...
  expect(packageManifest).toContain('0.230.0');
});
