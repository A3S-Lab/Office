...
test('routes the concise README and documentation homes to the current release story', async () => {
  ...
  expect(readme).toContain('Version `0.291.0`');
  expect(readme).toContain('AVERAGEIF');
  expect(readme).toContain('AVERAGEIF(A1:A3,"apple",C1)');
  expect(readme).toContain('fail closed');
  expect(englishHome).toContain("## What's new on `main` (0.291.0)");
  expect(englishHome).toContain('top-left');
  expect(englishHome).toContain('AVERAGEIF(A1:A3,"apple",C1)');
  expect(chineseHome).toContain('## `main` 更新内容（0.291.0）');
  expect(chineseHome).toContain('AVERAGEIF');
  expect(chineseHome).toContain('结果是 10');
...
test('publishes the WPS VML connector boundary across code, docs, and A3S Test evidence', async () => {
  ...
  expect(packageManifest).toContain('"version": "0.291.0"');
});
...
test('publishes the WPS connector-kind A3S Test matrix in the 0.57.0 release story', async () => {
  ...
  for (const document of [latestEnglish, latestChinese]) {
    ...
    expect(document).toContain('0.291.0');
  }
...
test('publishes Writer move revisions across implementation, native collaboration, docs, and release evidence', async () => {
  ...
  expect(packageManifest).toContain('0.291.0');
});
