...
test('routes the concise README and documentation homes to the current release story', async () => {
  ...
  expect(readme).toContain('Version `0.168.0`');
  expect(readme).toContain('paints Writer `basicWideInline`');
  expect(readme).toContain('thick geometric rails');
  expect(readme).toContain('Earlier releases stay on the product');
  expect(englishHome).toContain("## What's new on `main` (0.168.0)");
  expect(englishHome).toContain(
    'paints Writer basicWideInline / basicWideMidline / basicWideOutline art borders as thick geometric rails',
  );
  expect(chineseHome).toContain('## `main` 更新内容（0.168.0）');
  expect(chineseHome).toContain(
    '浏览器文档 PDF 导出现将 Writer 的 basicWideInline / basicWideMidline / basicWideOutline 艺术边框绘制为粗几何轨线',
  );
...
test('publishes the WPS VML connector boundary across code, docs, and A3S Test evidence', async () => {
  ...
  expect(packageManifest).toContain('"version": "0.168.0"');
});
...
test('publishes the WPS connector-kind A3S Test matrix in the 0.57.0 release story', async () => {
  ...
  for (const document of [latestEnglish, latestChinese]) {
    ...
    expect(document).toContain('0.168.0');
  }
...
test('publishes Writer move revisions across implementation, native collaboration, docs, and release evidence', async () => {
  ...
  expect(packageManifest).toContain('0.168.0');
});
