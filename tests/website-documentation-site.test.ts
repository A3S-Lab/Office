...
test('routes the concise README and documentation homes to the current release story', async () => {
  ...
  expect(readme).toContain('Version `0.218.0`');
  expect(readme).toContain(
    'keeps Spreadsheet Custom Sort **次序** closed labels honest',
  );
  expect(readme).toContain('Closed label is state');
  expect(readme).toContain('新建自定义序列… stays a command');
  expect(englishHome).toContain("## What's new on `main` (0.218.0)");
  expect(englishHome).toContain(
    'Spreadsheet Custom Sort **次序** closed labels stay honest',
  );
  expect(chineseHome).toContain('## `main` 更新内容（0.218.0）');
  expect(chineseHome).toContain(
    '电子表格自定义排序「次序」关闭标签如实表示状态',
  );
...
test('publishes the WPS VML connector boundary across code, docs, and A3S Test evidence', async () => {
  ...
  expect(packageManifest).toContain('"version": "0.218.0"');
});
...
test('publishes the WPS connector-kind A3S Test matrix in the 0.57.0 release story', async () => {
  ...
  for (const document of [latestEnglish, latestChinese]) {
    ...
    expect(document).toContain('0.218.0');
  }
...
test('publishes Writer move revisions across implementation, native collaboration, docs, and release evidence', async () => {
  ...
  expect(packageManifest).toContain('0.218.0');
});
