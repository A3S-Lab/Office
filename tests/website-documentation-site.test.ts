...
test('routes the concise README and documentation homes to the current release story', async () => {
  ...
  expect(readme).toContain('Version `0.299.0`');
  expect(readme).toContain('Sheet1:Sheet2!Name');
  expect(readme).toContain('single-sheet');
  expect(readme).toContain('recalculate-spreadsheet-formulas');
  expect(englishHome).toContain("## What's new on `main` (0.299.0)");
  expect(englishHome).toContain('Sheet1:Sheet2!Name');
  expect(englishHome).toContain('single-sheet');
  expect(chineseHome).toContain('## `main` 更新内容（0.299.0）');
  expect(chineseHome).toContain('三维名称');
  expect(chineseHome).toContain('单工作表');
...
test('publishes the WPS VML connector boundary across code, docs, and A3S Test evidence', async () => {
  ...
  expect(packageManifest).toContain('"version": "0.299.0"');
});
...
test('publishes the WPS connector-kind A3S Test matrix in the 0.57.0 release story', async () => {
  ...
  for (const document of [latestEnglish, latestChinese]) {
    ...
    expect(document).toContain('0.299.0');
  }
...
test('publishes Writer move revisions across implementation, native collaboration, docs, and release evidence', async () => {
  ...
  expect(packageManifest).toContain('0.299.0');
});
