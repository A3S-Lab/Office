...
test('routes the concise README and documentation homes to the current release story', async () => {
  ...
  expect(englishHome).toContain("## What's new on `main` (0.53.1)");
  ...
  expect(chineseHome).toContain('## `main` 更新内容（0.53.1）');
...
test('publishes the WPS VML connector boundary across code, docs, and A3S Test evidence', async () => {
  const [
    ...
    readFile(
      path.join(documentationRoot, '0.53.1/en/components/document.mdx'),
      'utf8',
    ),
    readFile(
      path.join(documentationRoot, '0.53.1/zh/components/document.mdx'),
      'utf8',
    ),
  ...
  expect(changelog).toContain('## 0.53.1 - 2026-09-05');
...
test('publishes Writer move revisions across implementation, native collaboration, docs, and release evidence', async () => {
  ...
  expect(packageManifest).toContain('0.53.1');
});
