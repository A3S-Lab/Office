...
test('keeps public documentation on the neutral Traditional Office baseline', async () => {
  ...
  const wpsReferenceDocumentation = new Set([
    ...[
      ...
      'ROADMAP.md',
      'tests/e2e/README.md',
      'docs/latest/en/wps-editor-parity-plan.md',
      'docs/latest/zh/wps-editor-parity-plan.md',
    ].map((file) => path.join(repositoryRoot, file)),
    ...DOCUMENTATION_VERSIONS.flatMap((version) =>
      DOCUMENTATION_LOCALES.flatMap(({ lang }) =>
        [
          ...
          'wps-editor-parity-plan.md',
        ].map((route) =>
          path.join(documentationRoot, version, lang, route),
        ),
      ),
    ),
  ]);
...
test('routes the concise README and documentation homes to the current release story', async () => {
  const [readme, englishHome, chineseHome, packageManifest] = await Promise.all([
  ...
  expect(readme).toContain('Version `0.283.0`');
  ...
  expect(readme).toContain('fldChar');
  expect(readme).toContain('fail-closed');
  expect(readme).toContain('test:corpus:r0');
  expect(readme).toContain('test:corpus:r2');
  expect(readme).toContain('test:corpus:r2-pivot');
  expect(readme).toContain('test:corpus:r2-cf');
  expect(packageManifest).toContain('test:corpus:r2');
  expect(packageManifest).toContain('test:corpus:r2-pivot');
  expect(packageManifest).toContain('test:corpus:r2-cf');
  expect(englishHome).toContain("## What's new on `main` (0.283.0)");
  ...
  expect(englishHome).toContain('complex');
  expect(chineseHome).toContain('## `main` 更新内容（0.283.0）');
...
test('publishes the WPS VML connector boundary across code, docs, and A3S Test evidence', async () => {
  ...
  expect(packageManifest).toContain('"version": "0.311.0"');
});
...
test('publishes the WPS connector-kind A3S Test matrix in the 0.57.0 release story', async () => {
  ...
  for (const document of [latestEnglish, latestChinese]) {
    ...
    expect(document).toContain('0.283.0');
  }
...
test('publishes Writer move revisions across implementation, native collaboration, docs, and release evidence', async () => {
  ...
  expect(packageManifest).toContain('0.311.0');
});
