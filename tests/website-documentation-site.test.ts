...
test('routes the concise README and documentation homes to the current release story', async () => {
  ...
  expect(readme).toContain('Version `0.289.0`');
  expect(readme).toContain('SUMIF');
  expect(readme).toContain('C1:C3');
  expect(readme).toContain('fail closed');
  expect(englishHome).toContain("## What's new on `main` (0.289.0)");
  expect(englishHome).toContain('criteria range shape');
  expect(englishHome).toContain('SUMIF(A1:A3,"apple",C1)');
  expect(chineseHome).toContain('## `main` 更新内容（0.289.0）');
  expect(chineseHome).toContain('SUMIF');
  expect(chineseHome).toContain('C1:C3');
...
test('publishes the WPS VML connector boundary across code, docs, and A3S Test evidence', async () => {
  ...
  expect(packageManifest).toContain('"version": "0.289.0"');
});
...
test('publishes the WPS connector-kind A3S Test matrix in the 0.57.0 release story', async () => {
  ...
  for (const document of [latestEnglish, latestChinese]) {
    ...
    expect(document).toContain('0.289.0');
  }
...
test('publishes Writer move revisions across implementation, native collaboration, docs, and release evidence', async () => {
  ...
  expect(packageManifest).toContain('0.289.0');
});
