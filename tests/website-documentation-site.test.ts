...
test('routes the concise README and documentation homes to the current release story', async () => {
  ...
  expect(readme).toContain('Version `0.239.0`');
  expect(readme).toContain('color-picker');
  expect(readme).toContain('chain().focus()');
  expect(readme).toContain('Popover');
  expect(readme).toContain('L2');
  expect(englishHome).toContain("## What's new on `main` (0.239.0)");
  expect(englishHome).toContain('Document color picker L2 focus restore');
  expect(chineseHome).toContain('## `main` 更新内容（0.239.0）');
  expect(chineseHome).toContain('颜色选择器');
  expect(chineseHome).toContain('单元格底纹');
...
test('publishes the WPS VML connector boundary across code, docs, and A3S Test evidence', async () => {
  ...
  expect(packageManifest).toContain('"version": "0.239.0"');
});
...
test('publishes the WPS connector-kind A3S Test matrix in the 0.57.0 release story', async () => {
  ...
  for (const document of [latestEnglish, latestChinese]) {
    ...
    expect(document).toContain('0.239.0');
  }
...
test('publishes Writer move revisions across implementation, native collaboration, docs, and release evidence', async () => {
  ...
  expect(packageManifest).toContain('0.239.0');
});
