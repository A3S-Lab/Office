...
test('routes the concise README and documentation homes to the current release story', async () => {
  ...
  expect(readme).toContain('Version `0.248.0`');
  expect(readme).toContain('Document');
  expect(readme).toContain('Quick Access');
  expect(readme).toContain('undo/redo');
  expect(readme).toContain('remove-link');
  expect(readme).toContain('chain().focus()');
  expect(readme).toContain('QAT');
  expect(readme).toContain('L2');
  expect(englishHome).toContain("## What's new on `main` (0.248.0)");
  expect(englishHome).toContain(
    'Document Quick Access and Insert link/bookmark stay-mounted focus restore',
  );
  expect(chineseHome).toContain('## `main` 更新内容（0.248.0）');
  expect(chineseHome).toContain('文档');
  expect(chineseHome).toContain('撤销/重做');
  expect(chineseHome).toContain('快速访问');
...
test('publishes the WPS VML connector boundary across code, docs, and A3S Test evidence', async () => {
  ...
  expect(packageManifest).toContain('"version": "0.248.0"');
});
...
test('publishes the WPS connector-kind A3S Test matrix in the 0.57.0 release story', async () => {
  ...
  for (const document of [latestEnglish, latestChinese]) {
    ...
    expect(document).toContain('0.248.0');
  }
...
test('publishes Writer move revisions across implementation, native collaboration, docs, and release evidence', async () => {
  ...
  expect(packageManifest).toContain('0.248.0');
});
