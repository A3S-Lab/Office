...
test('routes the concise README and documentation homes to the current release story', async () => {
  ...
  expect(readme).toContain('Version `0.226.0`');
  expect(readme).toContain('PDF page-number');
  expect(readme).toContain('data-office-escape-consumer');
  expect(readme).toContain('sheet rename');
  expect(englishHome).toContain("## What's new on `main` (0.226.0)");
  expect(englishHome).toContain(
    'PDF page-number and Spreadsheet sheet-rename Escape ownership',
  );
  expect(chineseHome).toContain('## `main` 更新内容（0.226.0）');
  expect(chineseHome).toContain('PDF 页码与表格工作表重命名的 Escape 归属');
...
test('publishes the WPS VML connector boundary across code, docs, and A3S Test evidence', async () => {
  ...
  expect(packageManifest).toContain('"version": "0.226.0"');
});
...
test('publishes the WPS connector-kind A3S Test matrix in the 0.57.0 release story', async () => {
  ...
  for (const document of [latestEnglish, latestChinese]) {
    ...
    expect(document).toContain('0.226.0');
  }
...
test('publishes Writer move revisions across implementation, native collaboration, docs, and release evidence', async () => {
  ...
  expect(packageManifest).toContain('0.226.0');
});
