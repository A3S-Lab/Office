...
test('routes the concise README and documentation homes to the current release story', async () => {
  ...
  expect(readme).toContain('Version `0.298.0`');
  expect(readme).toContain('External-workbook');
  expect(readme).toContain('ListObject');
  expect(readme).toContain('recalculate-spreadsheet-formulas');
  expect(englishHome).toContain("## What's new on `main` (0.298.0)");
  expect(englishHome).toContain('External-workbook');
  expect(englishHome).toContain('ListObject');
  expect(chineseHome).toContain('## `main` 更新内容（0.298.0）');
  expect(chineseHome).toContain('外部工作簿');
  expect(chineseHome).toContain('ListObject');
...
test('publishes the WPS VML connector boundary across code, docs, and A3S Test evidence', async () => {
  ...
  expect(packageManifest).toContain('"version": "0.298.0"');
});
...
test('publishes the WPS connector-kind A3S Test matrix in the 0.57.0 release story', async () => {
  ...
  for (const document of [latestEnglish, latestChinese]) {
    ...
    expect(document).toContain('0.298.0');
  }
...
test('publishes Writer move revisions across implementation, native collaboration, docs, and release evidence', async () => {
  ...
  expect(packageManifest).toContain('0.298.0');
});
