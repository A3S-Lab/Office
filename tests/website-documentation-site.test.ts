...
test('routes the concise README and documentation homes to the current release story', async () => {
  ...
  expect(readme).toContain('Version `0.296.0`');
  expect(readme).toContain('cached');
  expect(readme).toContain('office_collaboration_mutate');
  expect(readme).toContain('recalculate-spreadsheet-formulas');
  expect(englishHome).toContain("## What's new on `main` (0.296.0)");
  expect(englishHome).toContain('cached');
  expect(englishHome).toContain('office_collaboration_mutate');
  expect(chineseHome).toContain('## `main` 更新内容（0.296.0）');
  expect(chineseHome).toContain('缓存');
  expect(chineseHome).toContain('office_collaboration_mutate');
...
test('publishes the WPS VML connector boundary across code, docs, and A3S Test evidence', async () => {
  ...
  expect(packageManifest).toContain('"version": "0.296.0"');
});
...
test('publishes the WPS connector-kind A3S Test matrix in the 0.57.0 release story', async () => {
  ...
  for (const document of [latestEnglish, latestChinese]) {
    ...
    expect(document).toContain('0.296.0');
  }
...
test('publishes Writer move revisions across implementation, native collaboration, docs, and release evidence', async () => {
  ...
  expect(packageManifest).toContain('0.296.0');
});
