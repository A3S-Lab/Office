...
test('routes the concise README and documentation homes to the current release story', async () => {
  ...
  expect(readme).toContain('Version `0.256.0`');
  expect(readme).toContain('Spreadsheet');
  expect(readme).toContain('row');
  expect(readme).toContain('translucent');
  expect(readme).toContain('vertical');
  expect(readme).toContain('middle');
  expect(englishHome).toContain("## What's new on `main` (0.256.0)");
  expect(englishHome).toContain(
    'Spreadsheet row indexes stay readable and in-cell edit centers vertically',
  );
  expect(chineseHome).toContain('## `main` 更新内容（0.256.0）');
  expect(chineseHome).toContain('表格');
  expect(chineseHome).toContain('行号');
  expect(chineseHome).toContain('垂直居中');
  expect(chineseHome).toContain('半透明');
...
test('publishes the WPS VML connector boundary across code, docs, and A3S Test evidence', async () => {
  ...
  expect(packageManifest).toContain('"version": "0.256.0"');
});
...
test('publishes the WPS connector-kind A3S Test matrix in the 0.57.0 release story', async () => {
  ...
  for (const document of [latestEnglish, latestChinese]) {
    ...
    expect(document).toContain('0.256.0');
  }
...
test('publishes Writer move revisions across implementation, native collaboration, docs, and release evidence', async () => {
  ...
  expect(packageManifest).toContain('0.256.0');
});
