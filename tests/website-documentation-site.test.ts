...
test('routes the concise README and documentation homes to the current release story', async () => {
  ...
  expect(readme).toContain('Version `0.224.0`');
  expect(readme).toContain(
    'restores Escape draft cancel on Spreadsheet table totals',
  );
  expect(readme).toContain('Totals label');
  expect(readme).toContain('Custom formula');
  expect(englishHome).toContain("## What's new on `main` (0.224.0)");
  expect(englishHome).toContain(
    'Spreadsheet Table Design **汇总行** Escape restores dirty drafts',
  );
  expect(chineseHome).toContain('## `main` 更新内容（0.224.0）');
  expect(chineseHome).toContain('表格「表格设计」汇总行 Escape 先还原脏草稿');
...
test('publishes the WPS VML connector boundary across code, docs, and A3S Test evidence', async () => {
  ...
  expect(packageManifest).toContain('"version": "0.224.0"');
});
...
test('publishes the WPS connector-kind A3S Test matrix in the 0.57.0 release story', async () => {
  ...
  for (const document of [latestEnglish, latestChinese]) {
    ...
    expect(document).toContain('0.224.0');
  }
...
test('publishes Writer move revisions across implementation, native collaboration, docs, and release evidence', async () => {
  ...
  expect(packageManifest).toContain('0.224.0');
});
