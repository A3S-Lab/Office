...
test('documentation changelog stays scannable, localized, and version-aware', async ({
  ...
  await expect(cards).toHaveCount(86);
  await expect(cards.first()).toHaveAttribute('data-version', '0.105.0');
  await expect(cards.first()).toContainText(
    'Writer 让单元格 hMerge 修订可审阅',
  );
  ...
  await expect(releaseCard('0.105.0')).toContainText('实时修订跟踪');
