...
test('documentation changelog stays scannable, localized, and version-aware', async ({
  ...
  await expect(cards.first()).toHaveAttribute('data-version', '0.85.0');
  await expect(cards.first()).toContainText(
    'Writer 让行 wBefore 修订可审阅',
  );
  const releaseCard = (version: string) =>
    page.locator(`.office-release-card[data-version="${version}"]`);
  await expect(releaseCard('0.85.0')).toContainText('行前置宽度');
  await expect(releaseCard('0.85.0')).toContainText('实时修订跟踪');
  await expect(releaseCard('0.85.0')).toContainText('不透明路径保持失败闭合');
